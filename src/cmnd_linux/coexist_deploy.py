...
def deploy(i:CoexistInputs,*,execute=False,accept_legacy=False):
    ...
    try:
        ...
        write_new(ETC/'native-tomcat.env',(native/'computername.env').read_text()+'\nCATALINA_OPTS="-Xms512m -Xmx4096m -Djava.awt.headless=true -Djavax.net.ssl.trustStore=/etc/cmnd/java-cacerts"\n',0o640); shutil.copyfile(i.java_home/'lib/security/cacerts',ETC/'java-cacerts'); write_new(STATE/'truststore-password','changeit'); run(i.java_home/'bin/keytool','-importcert','-noprompt','-alias','linux-cmnd','-file',certs.ca_pem,'-keystore',ETC/'java-cacerts','-storepass:file',STATE/'truststore-password'); write_new(Path('/usr/local/share/ca-certificates/linux-cmnd.crt'),certs.ca_pem.read_bytes(),0o644); run('update-ca-certificates')
        ...
        adminpass=_admin(i); write_new(STATE/'initial-admin.json',json.dumps({'username':'admin','password':adminpass})); from .update_gui import password_record; admin=pwd.getpwnam('cmnd-admin'); MANAGEMENT.mkdir(mode=0o750); os.chown(MANAGEMENT,0,admin.pw_gid); Path('/var/lib/cmnd-updates').mkdir(mode=0o750); queue=Path('/var/lib/cmnd-update-requests'); queue.mkdir(mode=0o700); os.chown(queue,admin.pw_uid,admin.pw_gid); write_new(MANAGEMENT/'admin.json',json.dumps({'password':password_record(adminpass),'origin':https_origin(public,c.apache_https),'check_on_startup':not i.apt_managed,'applications':{'TV management':f'https://{public}:{c.tomcat_https}/SmartInstall/','Site and content editor':f'https://{public}:{c.apache_https}/SmartCMS/'}}),0o640); write_new(MANAGEMENT/'updates.json',json.dumps({'enabled':not i.apt_managed,'channel':'apt' if i.apt_managed else 'preview','token_file':None}),0o640); validate_baseline(); shutil.copyfile(BASELINE,'/var/lib/cmnd-updates/current.deb')
