...
public class WorkingDayMapper {
   ...
   public static List<WorkingDayViewModel> toWorkingDayViewModelList(final Page.WorkingDays workingDays) {
      Assert.notNull(workingDays, ToWorkingDayViewModelListMessages.WORKING_DAYS_CAN_NOT_BE_NULL);
      Map<Page.WorkingDay, Boolean> domainWorkingDays = workingDays.getWorkingDays();
      Assert.state(domainWorkingDays != null, ToWorkingDayViewModelListMessages.DOMAIN_WORKING_DAYS_CAN_NOT_BE_NULL);
      List<WorkingDayViewModel> viewModelWorkingDays = new ArrayList<>();
      domainWorkingDays.entrySet().stream().forEach(entry -> {
         ...
         Page.WorkingDay domainWorkingDay = entry.getKey();
         String viewModelWorkingDay = String.valueOf(domainWorkingDay);
   ...
   public static Page.WorkingDays toWorkingDays(final List<WorkingDayViewModel> workingDayViewModelList) {
