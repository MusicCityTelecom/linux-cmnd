...
public class HardwareMapper {
   ...
   public static HardwareViewModel toHardwareViewModel(final Hardware hardware) {
      Assert.notNull(hardware, ToHardwareViewModelMessages.HARDWARE_CAN_NOT_BE_NULL);
   ...
   public static List<HardwareViewModel> toHardwareViewModelList(final Collection<Hardware> hardwareCollection) {
      Assert.notNull(hardwareCollection, ToHardwareViewModelListMessages.HARDWARE_COLLECTION_CAN_NOT_BE_NULL);
      return hardwareCollection.stream()
         .filter(Objects::nonNull)
         .map(HardwareMapper::toHardwareViewModel)
         .filter(Objects::nonNull)
