...
public class MetricsServiceImpl implements MetricsService {
   ...
   public void writeMetricsLog(JSONObject metricDetails) {
      ...
      if (!StringUtils.isBlank(this.fileBeatServerId)) {
         ...
         File logFile = new File("C:\\Program Files\\Apache Software Foundation\\Tomcat 9.0\\logs\\metrics\\" + logFileName);
         ...
         try {
            ...
            FileUtils.writeStringToFile(
               logFile,
               new SimpleDateFormat("yyyy-MM-dd HH:mm:ss,SSS").format(new Date()) + " " + metricDetails.toString() + "\r\n",
   ...
   private synchronized void initFileBeatServerId() {
      if (!this.iSInitServerId) {
         ...
         try {
            File fileBeatServerConfigFile = new File("C:\\Philips\\filebeat\\server.json");
