...
public class HTNGHelper {
   ...
   public static FolioManagementClient getFolioClient(String wsdl) {
      return isValidURL(wsdl) ? new FolioManagementClient(wsdl) : null;
   }

   public static RoomStatusManagementClient getRoomStatusClient(String wsdl) {
      return isValidURL(wsdl) ? new RoomStatusManagementClient(wsdl) : null;
   }

   public static GuestCommunicationsClient getMessageClient(String wsdl) {
      return isValidURL(wsdl) ? new GuestCommunicationsClient(wsdl) : null;
   }
   ...
   public static List<HTNGProfileMessageRS.ProfileMessages.ProfileMessage> getMessages(String roomId) {
      List<HTNGProfileMessageRS.ProfileMessages.ProfileMessage> profileMessageList = new ArrayList<>();

      for (Message msg : JpaManager.getMessageManager().findMessageByGuestIds(roomId)) {
         ...
         JAXBElement<FormattedTextTextType> content = new JAXBElement<>(new QName("Text"), FormattedTextTextType.class, text);
         ...
         profileMessage.setStatus(messageStatusToHTNG(msg.getStatus()));
   ...
   public static List<CostingItemType> getBillItems(String roomId) {
      List<CostingItemType> costingItemList = new ArrayList<>();

      for (Billitem item : JpaManager.getBillitemManager().findBillitemsByRoomId(roomId)) {
   ...
   public static ResGuestType generateResGuestType(String guestId) {
      ...
      UniqueIDType uniqueId = generateUniqueTypeID("1", PmsUtils.extractGuestId(guestId));
   ...
   public static ResGuestsType generateResGuests(String guestId) {
      ...
      List<ResGuestType> resGuestList = new ArrayList<>();
      ResGuestType resGuest = generateResGuestType(guestId);
   ...
   public static HotelReservationsType.HotelReservation generateHoltelReservation(String resId) {
      ...
      hotelReservationVal1.setResGuests(generateResGuests(guestId));
      UniqueIDType resUniqueid = generateUniqueTypeID("1", resId);
   ...
   public static HotelReservationsType generateHoltelReservations(String resId) {
      ...
      HotelReservationsType.HotelReservation hotelReservationVal1 = generateHoltelReservation(resId);
   ...
   public static ErrorsType getErrorsType(String message) {
      ...
      List<ErrorType> returnErrorsError = new ArrayList<>();
