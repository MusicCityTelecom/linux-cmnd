...
public class iptv extends HttpServlet {
   private static final long serialVersionUID = 1L;
   private static final Logger LOG = LoggerFactory.getLogger(iptv.class);
   private static final Map<String, Class<? extends WebServiceCommandHandler>> COMMAND_HANDLER_MAPPING = new HashMap<>();
   private static final Map<String, Object> TV_HANDLE_LOCKS = new ConcurrentHashMap<>();
   ...
   private String getClientIp(HttpServletRequest request) {
      ...
      if (StringUtils.isNotBlank(requestIp)) {
         LOG.info("clientip ={} from request parameter ", requestIp);
         return requestIp;
      } else {
   ...
   private boolean checkSecurityHeader(HttpServletRequest request, String requestData, String requestSchema) {
      ...
      boolean compareSuccess = hashValue != null && hashValue.equalsIgnoreCase(securityToken);
      if (!compareSuccess) {
   ...
   private String getTVUniqueID(JSONObject commandDetails, JSONObject webServiceParameters) {
      if (webServiceParameters != null) {
         String tvUniqueId = webServiceParameters.optString("TVUniqueID", "");
         if (StringUtils.isNotBlank(tvUniqueId)) {
            return tvUniqueId;
         }
      }
   ...
   private String handleWebServiceCommand(String requestData, String clientIp, String requestSchema) throws IOException {
      ...
      if (handler == null) {
         LOG.warn("received <{}> command TODO implement!", function);
         return "";
      }
      ...
      synchronized (this.getDevicesHandleLock(tvUniqueId)) {
         Devices device = this.getDevicesObjectForHandler(clientIp, tvUniqueId);
         if (isNotDiscoveryJapit && device == null) {
            LOG.warn("submit get device info task for client ip : {}", clientIp);
         ...
         if (device != null) {
            if (device.getPmsSyncStatus() == PmsUtils.PmsSyncStatus.NeedSync.ordinal()) {
               LOG.info("submit sync pms status task for device : {}", device.getTvuniqueid());
            ...
            if (isNotDiscoveryJapit && ("offline".equalsIgnoreCase(device.getPowerstatus()) || "REBOOT".equalsIgnoreCase(device.getPowerstatus()))) {
               LOG.info("submit refresh power status task for device : {}", device.getTvuniqueid());
               TVDiscoveryManager.getDevices(clientIp);
            }
         }
   ...
   private Devices getDevicesObjectForHandler(String clientIp, String tvUniqueId) {
      ...
      if (iptv != null) {
         ...
         if ("on".equalsIgnoreCase(incomingTvAdr)) {
            iptv.setTvipaddress(clientIp);
         }

         iptv.setLastonline(TpvDateUtils.getDateTime());
   ...
   private static void findWebServiceCommandHandlerRoute() {
      try {
         List<Class<? extends WebServiceCommandHandler>> list = TpvClassUtils.getAllAssignedClass(WebServiceCommandHandler.class);
         LOG.info("find {} subclass::{}", WebServiceCommandHandler.class.getName(), list);

         for (Class<? extends WebServiceCommandHandler> handlerClz : list) {
         ...
         LOG.info("COMMAND_HANDLER_MAPPING==>{}", COMMAND_HANDLER_MAPPING);
      } catch (Exception ex) {
   ...
   private Object getDevicesHandleLock(String tvUniqueId) {
      ...
      if (lock == null) {
         ...
         return currentLock == null ? newLock : currentLock;
      } else {
         return lock;
      }
   }
   ...
   private String error(String func, String msg) {
      return "{\t\"Svc\" : \"WebServices\",\t\"SvcVer\" : \"3.0\",\t\"Cookie\" : 295,   \"CmdType\": \"error\",\t\"Fun\" : \""
         + func
   ...
   static {
      findWebServiceCommandHandlerRoute();
   }
}
