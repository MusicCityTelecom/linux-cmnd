...
public class CloneItemUtils {
   ...
   private static Map<String, String> japitNameToShowNameMap = new HashMap<>();
   private static Map<String, String> showNameToJapitNameMap = new HashMap<>();

   private CloneItemUtils() {
   }

   public static CloneItemUtils.CloneItemInfo getCloneItemInfo(CommonConstants.CloneItemType itemType, int id) {
      CloneItemUtils.CloneItemInfo info = new CloneItemUtils.CloneItemInfo();
      info.setId(id);
      info.setClone(false);
      ...
      switch (itemType) {
         ...
         case Banner:
            ...
            if (null != banners) {
               ...
               info.version = getBannerIdentifier(banners);
   ...
   private static String getMasterClonePath(String cloneName, String platform) {
      String rootPath = getCloneRootPath(cloneName);
      if (rootPath != null) {
   ...
   public static String getCloneItemPath(Setting setting, String itemName) {
      String masterPath = getMasterClonePath(setting.getName(), setting.getPlatform());
      if (masterPath != null) {
         String itemPath = masterPath + itemName + "/";
   ...
   public static int getAssignedId(CommonConstants.CloneItemType cloneType, Setting setting) {
      ...
      switch (cloneType) {
         ...
         case UiCustomizations:
            ...
            break;
         default:
            LOG.error("not support assign clone item:{}", cloneType);
      }
   ...
   public static String getCloneItemPlatform(CommonConstants.CloneItemType cloneType, int cloneId) {
      ...
      CloneItemUtils.CloneItemInfo info = getCloneItemInfo(cloneType, cloneId);
   ...
   public static String getUniqueCloneName(List<String> currentExistsName, String baseName) {
      String rename = baseName;
      if (null != currentExistsName && !currentExistsName.isEmpty()) {
         int j = 0;
         ...
         while (true) {
            ...
            if (!currentExistsName.contains(name)) {
               rename = name;
               break;
            }
   ...
   public static CloneItemUtils.CloneItemInfo getCloneItemInfo(String selectCloneType, int cloneId) {
      CommonConstants.CloneItemType type = IPUpgradeManager.convertUpgradeTypeToCloneItemType(selectCloneType);
      return getCloneItemInfo(type, cloneId);
   }
   ...
   static {
      ...
      for (Entry<String, String> entry : japitNameToShowNameMap.entrySet()) {
   ...
   public static class CloneItemInfo {
      ...
      public String getCachedPath() {
         return this.itemType == CommonConstants.CloneItemType.Firmware
            ? String.format(Locale.ENGLISH, "%s/Profile/UPG/%d/", CommonConstants.servletContextPath, this.id)
            : String.format(Locale.ENGLISH, "%s/Profile/Clone/%s", CommonConstants.servletContextPath, this.getClonePath());
      }
   }
}
