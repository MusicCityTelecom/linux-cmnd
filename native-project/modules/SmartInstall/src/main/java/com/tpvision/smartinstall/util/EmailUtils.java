...
public class EmailUtils {
   ...
   private static final String AMAZON_SECRET_KEY = EncryptionDecryptionUtility.decryptEmailParam(
      "5vuzJp+i866jd+vTcVMM+SM/Xw3VHHLBmw/i8tci6P5B5s9rIatvhABER6QUrxCf"
   );
   ...
   public static boolean sendEmailByAmazonService(String subject, String content, String mailAddress) {
      try {
         ...
         AmazonSimpleEmailService client = AmazonSimpleEmailServiceClientBuilder.standard().withRegion(Regions.fromName(AMAZON_MAIL_REGION)).build();
         SendEmailRequest request = new SendEmailRequest()
            .withDestination(new Destination().withToAddresses(emails))
            .withMessage(
               new Message()
                  .withBody(
                     new Body().withHtml(new Content().withCharset(charset).withData(content)).withText(new Content().withCharset(charset).withData(content))
                  )
                  .withSubject(new Content().withCharset(charset).withData(subject))
            )
            .withSource(AMAZON_MAIL_FROM);
         client.sendEmail(request);
         LOG.info("Email sent success to <{}>!", mailAddress);
         return true;
      } catch (Exception e) {
