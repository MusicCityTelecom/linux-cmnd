...
public class DetectOfflineTask extends TpvTimerTask {
   ...
   public void tryRun() {
      ...
      for (Devices device : iptvmanager.loadAll()) {
         if (!device.isRFDevice() && device.isOnline() && this.isExceedThirtyMinites(device.getLastonline())) {
   ...
   private void checkDeviceSecureCmdStatus(int validPort, Devices device, DevicesManager iptvmanager) {
      if ("Transiting".equalsIgnoreCase(device.getSecureCmdSupport())) {
         LOG.warn("skip to check secure mode as device {} is in transiting status", device.getTvuniqueid());
      } else {
         ...
         if (!device.getSecureCmdSupport().equalsIgnoreCase(String.valueOf(isSecureMode))) {
            LOG.info("fix secure mode to {} for device : {}", isSecureMode, device.getTvuniqueid());
   ...
   private void handleOfflineDevice(Devices device, DevicesManager iptvmanager) {
      LOG.info("offline tv:{}", device.getId());
