...
@WebServlet("/downloadDeviceData")
public class DownloadDeviceDataServlet extends HttpServlet {
      ...
      try (FileWriter fileWriter = new FileWriter(f)) {
         ...
         if (!tvs.isEmpty()) {
            for (int i = 0; i < tvs.size(); i++) {
               ...
               if (null != vsecureKey && null != vsecureID) {
            ...
            try (
               FileInputStream fis = new FileInputStream(overviewFile);
   ...
   private void downloadDeviceData(HttpServletRequest request, HttpServletResponse response) {
      try {
         ...
         File directoryPath = new File(CommonConstants.DOWNLOAD_LOCATION + "/Overview/DeviceData.xml");
         if (directoryPath.exists()) {
            directoryPath.delete();
         }
      } catch (Exception ex) {
   ...
   protected void proccessDeviceDataXml(DevicesInf listOfDevice) {
      if (null != listOfDevice) {
         ...
         try {
            ...
            marshaller.marshal(listOfDevice, sw);
