...
public class TVDiscoveryServiceHandler extends WebServiceCommandHandler {
   ...
   public String execute() {
      ...
      if (this.device != null && "Transiting".equalsIgnoreCase(this.device.getSecureCmdSupport())) {
         ...
         OnlineDevices onlineTvInfo = TVDiscoveryManager.initOnlineTvInfoDiscoveryParameter(
            tvIpAddress, port, this.tvUniqueId, tvDiscoveryParameters, isUsingHttps, true
         );
   ...
   private int detectSecureStatusAndWlsPort(boolean isUsingHttps, String tvIp) {
      LOG.info("start to detect tv's japit port info,isUsingHttps-> {}", isUsingHttps);
      if (this.device != null && isUsingHttps && this.device.getWlsSecurePort() != 0) {
         LOG.info("tv is not new and already has the https port<{}>,skip detect ports job", this.device.getWlsSecurePort());
         return this.device.getWlsSecurePort();
      }

      if (this.device != null && !isUsingHttps && this.device.getWlsPort() != 0) {
         LOG.info("tv is not new and already has the http port<{}>,skip detect ports job", this.device.getWlsPort());
         return this.device.getWlsPort();
      }

      List<Integer> possiblePorts = new ArrayList<>();
      if (isUsingHttps) {
      ...
      LOG.info("detect tv's wls port=>{}, isSecure=>{}", port, isUsingHttps);
      return port;
   }

   private int detectJapitPossiblePorts(String tvIp, List<Integer> possiblePorts) {
      LOG.info("detect target possiblePorts=>{}", possiblePorts);
      int port = 0;
      List<Integer> validPorts = NetworkUtils.filterSocketPorts(tvIp, possiblePorts);
      if (validPorts.size() == 1) {
         port = validPorts.get(0);
         LOG.info("only one port is OK ==>{}", port);
      } else if (validPorts.size() > 1) {
         int validPort = this.filterJapitPort(tvIp, validPorts);
         if (validPort != -1) {
            port = validPort;
            LOG.info("multiple ports are OK, use japit to test result=> {}", port);
         }
      }
   ...
   private int filterJapitPort(String tvIp, List<Integer> validPorts) {
      for (int port : validPorts) {
         try {
            ...
            if (response.contains("CommandDetails")) {
               return port;
            }
         } catch (Exception e) {
   ...
   private String getPollingFrequency(String pollingFrequency, String responseUniqueId) {
      return "{  \"Svc\": \"WebServices\",  \"SvcVer\": \"1.0\",  \"Cookie\": 294,  \"CmdType\": \"Change\",  \"Fun\": \"TVDiscoveryService\",  \"CommandDetails\": {    \"WebServiceParameters\": {      \"PollingFrequency\": "
         + pollingFrequency
         + ",      \"TVUniqueID\": \""
         + responseUniqueId
         + "\"    }  }}";
