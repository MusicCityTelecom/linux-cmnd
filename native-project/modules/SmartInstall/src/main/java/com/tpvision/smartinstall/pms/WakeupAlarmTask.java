...
public class WakeupAlarmTask extends TpvTimerTask {
   ...
   private boolean setAlarmToTV(String roomId, String wakeupTime) {
      ...
      if (devices.isEmpty()) {
         LOG.error("RoomId:{} No Found TV", roomId);
         return false;
      }
      ...
      for (Devices tv : devices) {
         if (PmsUtils.updatePmsAlarmTime(tv, wakeupTime.split(" ")[1])) {
            LOG.info("Set Alarm To TV Pass:{},{}", tv.getId(), wakeupTime);
         } else {
            LOG.error("Set Alarm To TV Fail:{},{}", tv.getId(), wakeupTime);
            flag = false;
         }
      }
