...
public class UserLogFilter implements Filter {
   ...
   public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
      ...
      boolean isDownloadUpgradePackage = req.getRequestURL().toString().contains("Profile") || req.getRequestURL().toString().contains("Cert");
      if (isDownloadUpgradePackage) {
         DownloadLimiter.getInstance().increaseIpDownloadCount(request.getRemoteAddr(), serverIP);
         stopWatch = new StopWatch();
         stopWatch.start();
         response = new UserLogFilter.SimpleFileDownloadResponseWrapper((HttpServletResponse)response);
         LOG.info("TV >>>>>>>> SI IP = {} file:{} start to download.", req.getRemoteAddr(), req.getRequestURL());
      }
      ...
      if (isDownloadUpgradePackage) {
         ...
         long writtenLength = ((UserLogFilter.SimpleFileDownloadResponseWrapper)response).getContentWritten();
         ...
         LOG.info(
            "SI >>>>>>>> TV IP = {} file:{} {},file-length:{} response-length:{} used time:{} download speed:{}",
            req.getRemoteAddr(),
            req.getRequestURL(),
            downloadFullFileResult,
            fileLength,
            writtenLength,
            stopWatch.getTime(),
            downloadSpeed
   ...
   private class SimpleDownloadOutputStream extends ServletOutputStream {
      private final ServletOutputStream delegate;
      private UserLogFilter.SimpleFileDownloadResponseWrapper simpleFileDownloadResponseWrapper;

      public SimpleDownloadOutputStream(ServletOutputStream delegate, UserLogFilter.SimpleFileDownloadResponseWrapper simpleFileDownloadResponseWrapper) {
   ...
   private class SimpleFileDownloadResponseWrapper extends HttpServletResponseWrapper {
      ...
      public ServletOutputStream getOutputStream() throws IOException {
         return UserLogFilter.this.new SimpleDownloadOutputStream(super.getOutputStream(), this);
      }
   }
}
