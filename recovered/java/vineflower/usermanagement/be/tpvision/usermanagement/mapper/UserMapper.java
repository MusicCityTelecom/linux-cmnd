...
public class UserMapper {
   public static User toUser(final CreateOrUpdateUserViewModel createOrUpdateUserViewModel) {
      Assert.notNull(createOrUpdateUserViewModel, MessageUtilities.CREATE_OR_UPDATE_USER_VIEW_MODEL_NOT_NULL_MESSAGE);
   ...
   public static List<User> toUserList(final Collection<CreateOrUpdateUserViewModel> createOrUpdateUserViewModelCollection) {
      ...
      return createOrUpdateUserViewModelCollection.stream()
         .filter(createOrUpdateUserViewModel -> createOrUpdateUserViewModel != null)
         .map(UserMapper::toUser)
         .filter(user -> user != null)
         .collect(Collectors.toList());
   }

   public static UserViewModel toUserViewModel(final User user) {
   ...
   public static List<UserViewModel> toUserViewModelList(final Collection<User> userCollection) {
      Assert.notNull(userCollection, MessageUtilities.USER_COLLECTION_NOT_NULL_MESSAGE);
      return userCollection.stream()
         .filter(user -> user != null)
         .map(UserMapper::toUserViewModel)
         .filter(userViewModel -> userViewModel != null)
         .collect(Collectors.toList());
   }
}
