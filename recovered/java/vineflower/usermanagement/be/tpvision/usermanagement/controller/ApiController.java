...
public class ApiController {
   ...
   public Map<String, Object> updateUserPassword(final String username, final String password) {
   ...
   public Map<String, Object> checkUserPassword(final String username, final String password) {
      this.assertUserServiceStateNotNull();
      User user = this.userService.getUser(username);
      Objects.requireNonNull(password, MessageUtilities.USER_PASSWORD_NOT_NULL_MESSAGE);
      String encodedPassword = DigestUtils.md5Hex(password);
      return user != null && StringUtils.equalsIgnoreCase(encodedPassword, user.getPassword())
         ? this.getSuccessResult()
         : this.getFailureResult(100, "invalid password");
   }
