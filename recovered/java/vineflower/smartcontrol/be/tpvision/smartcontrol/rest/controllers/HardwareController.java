...
public class HardwareController {
   ...
   public HardwareController(final HardwareService hardwareService) {
      Assert.notNull(hardwareService, ConstructorMessages.HARDWARE_SERVICE_CAN_NOT_BE_NULL);
      this.hardwareService = hardwareService;
   }

   private Hardware getHardware(final String hardwareKey) {
   ...
   @PostMapping("/{hardwareKey}")
   public void addHardware(@PathVariable("hardwareKey") final String hardwareKey) {
      Assert.notNull(hardwareKey, AddHardwareMessages.HARDWARE_KEY_CAN_NOT_BE_NULL);
      Hardware hardware = new Hardware();
      hardware.setHardwareKey(hardwareKey);
      this.hardwareService.addHardware(hardware);
   }

   @PutMapping("/{hardwareKey}/{contentId}")
   public void updateHardware(@PathVariable("hardwareKey") final String hardwareKey, @PathVariable("contentId") final String contentId) {
      Assert.notNull(hardwareKey, UpdateHardwareMessages.HARDWARE_KEY_CAN_NOT_BE_NULL);
   ...
   @DeleteMapping("/{hardwareKey}")
   public void deleteHardware(@PathVariable("hardwareKey") final String hardwareKey) {
