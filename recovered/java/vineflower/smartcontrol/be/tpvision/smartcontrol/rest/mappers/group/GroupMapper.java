...
public class GroupMapper {
   ...
   public static GroupViewModel toGroupViewModel(final Group group) {
      Assert.notNull(group, ToGroupViewModelMessages.GROUP_CAN_NOT_BE_NULL);
      Long groupId = group.getId();
      String groupName = group.getName();
      Set<Device> deviceSet = group.getDevices();
      List<GroupDeviceViewModel> groupDeviceViewModelList = deviceSet.stream()
         .map(DeviceMapper::toGroupDeviceViewModel)
         .filter(Objects::nonNull)
         .collect(Collectors.toList());
      return new GroupViewModel(groupId, groupName, groupDeviceViewModelList);
   }

   public static Group toGroup(final GroupViewModel groupViewModel) {
      Assert.notNull(groupViewModel, ToGroupMessages.GROUP_VIEW_MODEL_CAN_NOT_BE_NULL);
      ...
      Set<Device> deviceSet = groupDeviceViewModelList.stream()
         .filter(Objects::nonNull)
   ...
   public static DetectGroupsViewModel toDetectGroupsViewModel(final Group group) {
      Assert.notNull(group, ToDetectGroupsViewModelMessages.GROUP_CAN_NOT_BE_NULL);
      String groupName = group.getName();
      Set<Device> deviceSet = group.getDevices();
      List<DetectDevicesViewModel> detectDevicesViewModelList = deviceSet.stream()
         .map(DeviceMapper::toDetectDevicesViewModel)
         .filter(Objects::nonNull)
         .collect(Collectors.toList());
      return new DetectGroupsViewModel(groupName, detectDevicesViewModelList);
   }

   public static Group toGroup(final DetectGroupsViewModel detectGroupsViewModel) {
      Assert.notNull(detectGroupsViewModel, ToGroupMessages.DETECT_GROUPS_VIEW_MODEL_CAN_NOT_BE_NULL);
      ...
      Set<Device> deviceSet = detectDevicesViewModelList.stream()
         .filter(Objects::nonNull)
         .map(DeviceMapper::toDevice)
         .filter(Objects::nonNull)
         .collect(Collectors.toSet());
   ...
   public static ImportExportGroupViewModel toImportExportGroupViewModel(final Group group) {
      Assert.notNull(group, ToImportExportGroupViewModelMessages.GROUP_CAN_NOT_BE_NULL);
      String groupName = group.getName();
      Set<Device> deviceSet = group.getDevices();
      List<ImportExportDeviceViewModel> importExportDeviceViewModelList = deviceSet.stream()
   ...
   public static Group toGroup(final ImportExportGroupViewModel importExportGroupViewModel) {
      Assert.notNull(importExportGroupViewModel, ToGroupMessages.IMPORT_EXPORT_GROUP_VIEW_MODEL_CAN_NOT_BE_NULL);
      ...
      Set<Device> deviceSet = importExportDeviceViewModelList.stream()
         .filter(Objects::nonNull)
         .map(DeviceMapper::toDevice)
         .filter(Objects::nonNull)
