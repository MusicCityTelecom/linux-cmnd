...
public class SicpDecoder extends ByteToMessageDecoder {
   ...
   public SicpDecoder(final SicpCommandFactory sicpCommandFactory) {
      Assert.notNull(sicpCommandFactory, ConstructorMessages.SICP_COMMAND_FACTORY_CAN_NOT_BE_NULL);
      this.sicpCommandFactory = sicpCommandFactory;
   }

   @Override
   public void decode(final ChannelHandlerContext channelHandlerContext, final ByteBuf input, final List<Object> output) {
      logger.debug("Decoding server message...");
      if (input != null && input.isReadable()) {
         int messageSize = input.getByte(0);
         if (input.readableBytes() >= messageSize) {
            ...
            logger.debug("Decoded bytes: {}", data);
            Object decoded = codec.toDomain(data);
            Response response = new ResponseWrapper(decoded);
