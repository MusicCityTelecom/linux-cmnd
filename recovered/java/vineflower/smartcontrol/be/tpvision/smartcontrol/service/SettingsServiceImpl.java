...
public class SettingsServiceImpl implements SettingsService {
   ...
   @Autowired
   public SettingsServiceImpl(final SettingsRepository settingsRepository) {
      Assert.notNull(settingsRepository, ConstructorMessages.SETTINGS_REPOSITORY_CAN_NOT_BE_NULL);
   ...
   public void setSettings(final Settings settings) {
      Assert.notNull(settings, SetSettingsMessages.SETTINGS_CAN_NOT_BE_NULL);
      this.setSettings(settings, false);
   }

   @Transactional
   @Override
   public void setSettings(final Settings settings, final boolean encodeDefaultFtpSettingsPassword) {
      Assert.notNull(settings, SetSettingsMessages.SETTINGS_CAN_NOT_BE_NULL);
   ...
   public List<NetworkAdapter> getNetworkAdapters() {
      List<NetworkAdapter> networkAdapterList = new ArrayList<>();
   ...
   private List<NetworkAdapter> getNetworkAdapters(NetworkInterface networkInterface) {
      Assert.notNull(networkInterface, "Network interface can not be null.");
      List<NetworkAdapter> networkAdapterList = new ArrayList<>();
