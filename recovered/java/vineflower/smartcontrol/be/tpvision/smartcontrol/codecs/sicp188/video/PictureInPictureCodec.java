...
import be.tpvision.smartcontrol.messages.codecs.sicp188.video.picture_in_picture.ToDomainMessages;
import be.tpvision.smartcontrol.messages.codecs.sicp188.video.picture_in_picture.ToProtocolMessages;
import be.tpvision.smartcontrol.util.MapUtilities;
import java.util.EnumMap;
import java.util.Map;
import org.springframework.util.Assert;

public class PictureInPictureCodec extends Codec<PictureInPicture> {
   static final byte STATUS_OFF_BYTE = 0;
   static final byte STATUS_ON_BYTE = 1;
   private static final Map<PictureInPicture.Status, Byte> domainStatuses = new EnumMap<>(PictureInPicture.Status.class);
   private static final Map<Byte, PictureInPicture.Status> protocolStatuses = MapUtilities.inverse(domainStatuses);
   ...
   private static final Map<PictureInPicture.WindowPosition, Byte> domainWindowPositions = new EnumMap<>(PictureInPicture.WindowPosition.class);
   private static final Map<Byte, PictureInPicture.WindowPosition> protocolWindowPositions = MapUtilities.inverse(domainWindowPositions);
   ...
   public byte[] toProtocol(final PictureInPicture pictureInPicture) {
      Assert.notNull(pictureInPicture, ToProtocolMessages.PICTURE_IN_PICTURE_CAN_NOT_BE_NULL);
      ...
      Byte protocolWindowPosition = domainWindowPositions.get(domainWindowPosition);
   ...
   public PictureInPicture toDomain(final byte[] bytes) {
