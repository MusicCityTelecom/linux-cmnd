...
public class TilingCodec extends Codec<Tiling> {
   static final int MAXIMUM_NUMBER_OF_H_MONITORS = 15;
   static final byte ENABLE_NO_BYTE = 0;
   static final byte ENABLE_YES_BYTE = 1;
   private static final Map<Tiling.Enable, Byte> domainEnables = new EnumMap<>(Tiling.Enable.class);
   private static final Map<Byte, Tiling.Enable> protocolEnables = MapUtilities.inverse(domainEnables);
   ...
   private static final Map<Tiling.FrameComp, Byte> domainFrameComps = new EnumMap<>(Tiling.FrameComp.class);
   private static final Map<Byte, Tiling.FrameComp> protocolFrameComps = MapUtilities.inverse(domainFrameComps);
   ...
   public byte[] toProtocol(Tiling tiling) {
      Assert.notNull(tiling, ToProtocolMessages.TILING_CAN_NOT_BE_NULL);
      ...
      Byte protocolEnable = domainEnables.get(domainEnable);
      ...
      Byte protocolFrameComp = domainFrameComps.get(domainFrameComp);
