...
public class QueryDatabaseAuthenticationHandler extends AbstractJdbcUsernamePasswordAuthenticationHandler {
   ...
   public QueryDatabaseAuthenticationHandler(
      final QueryJdbcAuthenticationProperties properties,
      ...
      if (StringUtils.isBlank(properties.getFieldPassword())) {
         LOGGER.warn(
            "When the password field is left undefined, CAS will skip comparing database and user passwords for equality , (specially if the query results do not contain the password field),and will instead only rely on a successful query execution with returned results in order to verify credentials"
         );
      }
   }
   ...
   protected AuthenticationHandlerExecutionResult authenticateUsernamePasswordInternal(
      ...
      try {
         ...
         if (!dbFields.containsKey(this.properties.getFieldPassword())) {
            ...
            boolean originalPasswordEquals = StringUtils.isBlank(originalPassword) && !StringUtils.equals(password, dbPassword);
         ...
         if (StringUtils.isNotBlank(this.properties.getFieldDisabled()) && dbFields.containsKey(this.properties.getFieldDisabled())) {
            String dbDisabled = dbFields.get(this.properties.getFieldDisabled()).toString();
            if (BooleanUtils.toBoolean(dbDisabled) || "1".equals(dbDisabled)) {
               throw new AccountDisabledException("Account has been disabled");
            }
         }

         if (StringUtils.isNotBlank(this.properties.getFieldExpired()) && dbFields.containsKey(this.properties.getFieldExpired())) {
            String dbExpired = dbFields.get(this.properties.getFieldExpired()).toString();
   ...
   private void collectPrincipalAttributes(final Map<String, List<Object>> attributes, final Map<String, Object> dbFields) {
      this.principalAttributeMap.forEach((key, names) -> {
         ...
         if (attribute != null) {
            ...
            Collection<String> attributeNames = (Collection<String>)names;
