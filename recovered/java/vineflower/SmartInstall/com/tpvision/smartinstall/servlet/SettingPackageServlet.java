...
@WebServlet("/settingpackage")
public class SettingPackageServlet extends BaseHttpServlet {
   ...
   protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      switch (mode) {
         ...
         case "DELETE_TERM_AND_CONDITIONS":
            ...
            break;
         default:
            LOG.error("not support mode:{}", mode);
      }
   }
   ...
   private String saveTermIntoDB(String value, String termId, String settingPackageId, String valueName) {
      ...
      } else if ("delete".equalsIgnoreCase(valueName)) {
         int seq = Integer.parseInt(termId);
         if (seq >= 0 && seq < messages.size()) {
            messages.remove(seq);
         }
      }
   ...
   private void initEditTerm(HttpServletRequest request, HttpServletResponse response) {
      ...
      if (termAndConditions == null) {
         termAndConditions = "{\"Svc\":\"offlineServices\",\"SvcVer\":\"4.0\",\"Cookie\":"
            + JAPITUtils.getJapitRandomCookieValue()
            + ",\"CmdType\":\"Change\",\"Fun\":\"ApplicationControl\",\"CommandDetails\":{\"ApplicationDetails\":{\"ApplicationName\":\"Googlecast\",\"ApplicationAttributes\":{\"TermsAndConditions\":[{\"Language\":\"eng\",\"TermsAndConditionsTitle\":\"Please enter a title.\",\"TermsAndConditionsBody\":\"Please enter a message.\"}]}}}}";
         settingPackage.setTermAndConditions(termAndConditions);
         JpaManager.getSettingPackageManager().save(settingPackage);
      }

      TermAndConditionsBean termAndConditionsBean = new Gson().fromJson(termAndConditions, TermAndConditionsBean.class);
      List<TermAndConditionsBean.CommandDetails.ApplicationDetails.ApplicationAttributes.TermsAndConditions> messages = termAndConditionsBean.getCommandDetails()
      ...
      Gson gson = new Gson();
      String status = "{\"languageList\":"
         + gson.toJson(sbLanguage.toString())
         + ", \"bodyList\":"
         + gson.toJson(sbMessageBody.toString())
         + ", \"titleList\":"
         + gson.toJson(sbMessageTitle.toString())
         + "}";
   ...
   private void updateCloneIn(HttpServletRequest request, HttpServletResponse response) {
      ...
      try {
         ...
         SettingPackage settingPackage = JpaManager.getSettingPackageManager().loadByKey(Integer.valueOf(settingPackageId));
         LOG.info("update setting clonein:{} to {}", key, value);
         ...
         if (settingItem == null) {
            LOG.info("Setting name not found in database:{}", key);
   ...
   private Setting getSettingItem(String settingName, SettingChannelBean scb) {
      for (Setting s : scb.getSetttings().getSetting()) {
         if (s.getItem().equals(settingName)) {
            return s;
         }
      }
   ...
   private void updateSettings(HttpServletRequest request, HttpServletResponse response) {
      ...
      try {
         ...
         LOG.info("update setting:{} to {}", settingName, value);
         SettingChannelBean scb = new Gson().fromJson(settingPackage.getValue(), SettingChannelBean.class);
         Setting settingItem = this.getSettingItem(settingName, scb);
         if (settingItem == null) {
            LOG.info("Setting name not found in database:{}", settingName);
            settingItem = new Setting();
   ...
   private void settingEdit(HttpServletRequest request, HttpServletResponse response) {
      ...
      try {
         ...
         String welcomelogoImageFileName = request.getAttribute("WELCOMELOGO_IMAGE_FILE_NAME") != null
            ? (String)request.getAttribute("WELCOMELOGO_IMAGE_FILE_NAME")
            : null;
            ...
            if (null == s.getItem()) {
               ...
               if (null == value) {
                  ...
                  if ("Wireless and Networks.Settings.Network Configuration".equalsIgnoreCase(s.getItem())) {
                     if ("Static IP".equalsIgnoreCase(value)) {
                        bStaticIP = true;
                     } else {
                        bStaticIP = false;
                     }
                  ...
                  if ("Wireless and Networks.Settings.Static IP Configuration.IP Address".equalsIgnoreCase(s.getItem())
                     || "Wireless and Networks.Settings.Static IP Configuration.Netmask".equalsIgnoreCase(s.getItem())
                     || "Wireless and Networks.Settings.Static IP Configuration.Gateway".equalsIgnoreCase(s.getItem())
                     || "Wireless and Networks.Settings.Static IP Configuration.DNS 1".equalsIgnoreCase(s.getItem())
                     || "Wireless and Networks.Settings.Static IP Configuration.DNS 2".equalsIgnoreCase(s.getItem())) {
                     if (bStaticIP) {
                        s.setCloneIn("Yes");
                     } else {
                        s.setCloneIn("No");
                  ...
                  if ("Input Control.Multi Remote Control".equalsIgnoreCase(s.getItem())) {
                     if ("Yes".equalsIgnoreCase(multiRemoteControlChecked)) {
                        s.setCloneIn("Yes");
                     } else {
                        s.setCloneIn("No");
   ...
   private void copySettingPackage(HttpServletRequest request, HttpServletResponse response) {
      ...
      try {
         ...
         List<String> currentSettingPackageNames = settingPackageManager.loadAll()
            .stream()
            .map(SettingPackage::getName)
            .map(String::trim)
   ...
   private void settingIndex(HttpServletRequest request, HttpServletResponse response) {
      try {
         ...
         if (null == settingPackage) {
            LOG.error("invalid setting package id:{}", id);
            throw new IOException("invalid setting package id");
         }

         SettingChannelBean scb = new Gson().fromJson(settingPackage.getValue(), SettingChannelBean.class);
         if (StringUtils.isNotBlank(channelId)) {
            ChannelPackage channelPackage = JpaManager.getChannelPackageManager().loadByKey(Integer.parseInt(channelId));
         ...
         request.setAttribute("settingPackageId", id);
   ...
   private void getAssignSettingPackageList(HttpServletRequest request, HttpServletResponse response) {
      ...
      if (platform.isEmpty()) {
         settingPackages = JpaManager.getSettingPackageManager().loadAll();
      } else {
         settingPackages = JpaManager.getSettingPackageManager().findSettingPackagesByPlatforms(platform);
      }

      try {
         IOUtils.write(new Gson().toJson(settingPackages).getBytes(), response.getOutputStream());
      } catch (IOException e) {
         LOG.error(e.getMessage(), e);
      }
   }
   ...
   private void assignSettingPackage(HttpServletRequest request, HttpServletResponse response) {
      ...
      if ("None".equalsIgnoreCase(settingPackageId)) {
         ...
         SettingPackageManager settingPackageManager = JpaManager.getSettingPackageManager();
         SettingPackage settingPackage = settingPackageManager.loadByKey(Integer.parseInt(settingPackageId));
         if (settingPackage == null) {
            LOG.error("settingpackageid:{} not exists", settingPackageId);
                  ...
                  if (!isEasySuite) {
                     ...
                     for (int i = 1; i < 3; i++) {
                        ...
                        for (int j = 0; j < 20; j++) {
                           ...
                           if (null != value && (null == value || !value.equals("0"))) {
                              try {
