...
@WebServlet("/siconfig")
public class SIConfigServlet extends BaseHttpServlet {
   ...
   private JSONArray checkLatestVersionShowInfo() {
      ...
      checkedReceptionClientList.stream()
         .forEach(
            receptionVersion -> {
               ...
               if (newVersion != null) {
                  ...
                  receptionVerson.put(
                     "tips",
                     "CMND Reception on PC <b>"
                        + receptionVersion.getClientId()
                        + "</b> is out-of-date,latest version is <b>"
   ...
   private String updateUserNameRoleConfig(HttpServletRequest request) {
      ...
      LOG.debug("[updateConfig]key={},value={}", key, value);
   ...
   private void showReceptionPage(HttpServletRequest request, HttpServletResponse response, SIConfig sic) throws ServletException, IOException {
      ...
      request.setAttribute(
         "compatibleVersions", ApiConstants.API_BACKWARD_SUPPORT_VERSIONS.length == 0 ? "-" : Arrays.toString(ApiConstants.API_BACKWARD_SUPPORT_VERSIONS)
      );
   ...
   private void saveTemplate(HttpServletRequest request, HttpServletResponse response) {
      try {
         LOG.info("abovePinCode is {}", request.getParameter("abovePinCode"));
         MyChoiceTemplate myChoiceTemplate = JpaManager.getMyChoiceTemplateManager().getMyChoiceTemplate();
   ...
   private Map<String, Object> formatApiLicenseData(ApiLicenseChecker apiLicenseChecker) {
      ...
      List<Map<String, String>> licenseDetail = Collections.emptyList();
