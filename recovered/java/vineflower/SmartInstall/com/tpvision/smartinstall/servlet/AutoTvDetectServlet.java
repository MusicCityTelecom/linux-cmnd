...
@WebServlet("/AutoTvDetectServlet")
public class AutoTvDetectServlet extends HttpServlet {
   ...
   private void searchTv(HttpServletRequest request, HttpServletResponse response) {
      ...
      TVDiscoveryManager.DetectTarget detectTarget = TVDiscoveryManager.DetectTarget.valueOf(
         TpvStringUtils.tryParseInt(request.getParameter("detectTarget"), -1)
      );
      LOG.info("firstIp:{},lastIp:{},detectTarget:{}", firstIp, lastIp, detectTarget);
      boolean searchAll = StringUtils.isNotBlank(lastIp) && !StringUtils.equals(firstIp, lastIp);
      List<OnlineDevices> detectResult = new ArrayList<>();
      if (searchAll) {
   ...
   private void saveDetectedTvToOnlineDb(OnlineDevices onlineTvInfo) {
      ...
      if (null == tv) {
         ...
         LOG.info("device table not exist this tv:{} save to onlinedevices", tvUniqueId);
         JpaManager.getOnlineDevicesManager().saveOnlineDevices(onlineTvInfo);
      } else {
         LOG.info("device table exists this tv:{} update device data", tvUniqueId);
   ...
   private void importTvData(HttpServletRequest request, HttpServletResponse response) {
      ...
      for (String networkIp : set) {
         ...
         LOG.info("import tv data ==> [key]={}, [value]={}", networkIp, tvids);
         String[] tvArray = tvids.split(",");
         List<OnlineDevices> onlineDevices = new ArrayList<>();
   ...
   private void correctWebserviceUrl(String tvUniqueId, HttpServletResponse response) {
      ...
      if (devices != null) {
         boolean sendResult = IPUpgradeManager.sendModifyWebserviceUrlTVSettingsRequestToTV(devices, false);
         if (sendResult) {
            Utils.writeToResponse("{\"status\":\"success\"}", "text/json;charset=UTF-8", response);
         }
      }
