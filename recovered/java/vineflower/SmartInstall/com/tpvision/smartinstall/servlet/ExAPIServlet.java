...
@WebServlet("/ExAPIServlet")
public class ExAPIServlet extends RunRFServlet {
   ...
   private void handleGetExApiStatus(HttpServletResponse response) {
      ...
      result.put(
         "compatibleVersions", ApiConstants.EXAPI_BACKWARD_SUPPORT_VERSIONS.length == 0 ? "-" : Arrays.toString(ApiConstants.EXAPI_BACKWARD_SUPPORT_VERSIONS)
      );
   ...
   private void handleTestRequest(HttpServletRequest request, HttpServletResponse response) {
      ...
      switch (command) {
         case "TransitCommunicationTo":
            ...
            } else if ("NonSecured".equalsIgnoreCase(value)) {
               Devices tv = this.getDevicesByIp(ip);
               SecuredCmdControlManager.sendSecuredCmdToHttp(tv);
            } else if ("SecuredDirect".equalsIgnoreCase(value)) {
   ...
   private Devices getDevicesByIp(String ip) {
      ...
      for (Devices tv : devices) {
         if (!"offline".equals(tv.getPowerstatus())) {
            return tv;
         }
      }
   ...
   private void handleGetJobs(HttpServletResponse response) {
      Utils.writeToResponse(
         new JSONObject(TpvBatchJobUtils.getExecuteAbleJobs(Job.ExecuteType.ALL, Job.ExecuteType.BYHAND)).toString(), "text/json;charset=UTF-8", response
      );
   }
