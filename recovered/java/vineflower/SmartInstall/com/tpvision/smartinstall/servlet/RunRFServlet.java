...
public class RunRFServlet extends BaseHttpServlet {
   ...
   protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      switch (mode) {
         ...
         case "AddToPlayoutList":
            ...
            if (!StringUtils.isEmpty(uid) || !StringUtils.isEmpty(request.getParameter("groupName"))) {
   ...
   private String getLocalInterfaces() {
      ...
      try {
         ...
         while (interfaces.hasMoreElements()) {
            ...
            if (!netInter.isLoopback() && netInter.supportsMulticast() && !netInter.isVirtual()) {
               for (InterfaceAddress ifAddr : netInter.getInterfaceAddresses()) {
                  ...
                  if (ifAddr.getAddress() instanceof Inet6Address) {
                     LOG.info("{},ipv6 address will ignore", ipStr);
                  } else {
   ...
   private String checkPlayoutList(HttpServletRequest request) {
      ...
      if (!StringUtils.isEmpty(request.getParameter("uid"))) {
         ...
         if (result.length() == 0) {
            ...
            status = this.failedStatus(result);
         }
      }
   ...
   private String playoutAVStream() {
      ...
      for (PlayoutInfo info : playoutInfoMgr.findPlayoutInfoByType("AV Stream")) {
         info.setConnectId(1);
   ...
   private String processIPUpgrade(HttpServletRequest request) {
      ...
      Map<String, String> parameters = this.requestParametersToMap(request);
   ...
   private String updateLastConfig(HttpServletRequest request) {
      ...
      String outputStr = request.getParameter("outputSetting");
      if (null != outputStr && !outputStr.isEmpty()) {
         LastRFConfig.OutputConfig outputConfig = new Gson().fromJson(outputStr, LastRFConfig.OutputConfig.class);
