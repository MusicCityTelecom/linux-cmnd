...
public class ApplicationControlHandler extends WebServiceCommandHandler {
   ...
   public String execute() {
      ...
      if ("Response".equalsIgnoreCase(this.cmdType)) {
         JSONArray applicationAttributesValue = this.commandDetails.optJSONArray("ApplicationAttributesValue");
         if (applicationAttributesValue != null) {
            for (int i = 0; i < applicationAttributesValue.length(); i++) {
               JSONObject appAttribute = applicationAttributesValue.getJSONObject(i);
               if (StringUtils.equalsIgnoreCase(appAttribute.optString("ApplicationName"), "Alarm")) {
   ...
   private String handleWeatherQueryRequest() {
      ...
      if ("Weather".equals(appName) && "Native".equals(appType) && !attrs.isEmpty() && "WeatherForecast".equals(attrs.get(0).toString())) {
         ...
         if (null != geonameId && StringUtils.isNotEmpty(geonameId.toString())) {
            ...
            String forecasts = "";
            if (result != null && result.getForecasts() != null) {
               forecasts = result.getForecasts();
               if (forecasts.startsWith("callback({") && forecasts.endsWith("})")) {
   ...
   private String responseApplicationControllWebService(String geonameId, String lang, String data) {
      return "{ \t\"Svc\" : \"WebServices\", \t\"SvcVer\": \"3.0\",\t\"Cookie\": 299,   \"CmdType\": \"Response\",\t\"Fun\" : \"ApplicationService\", \t\"CommandDetails\" : \t{\t\t\"ApplicationAttributesValue\" : [\t    {       \t\"ApplicationName\": \"Weather\",\t\t\t\"ApplicationType\": \"Native\",\t\t\t\"ApplicationAttributes\": {\t  \t\t\t\"GeonameLocationID\": "
         + geonameId
         + ",  \t\t\t\t\"GuestLanguage\": \""
         + lang
         + "\",  \t\t\t\t\"WeatherForecast\": {               \t\"ForecastProvider\" : \"MeteoWeather\",\t\t\t\t\t\"Forecast\" : { \t\t\t\t\t\t\"Blob\":"
         + data
         + "  \t\t\t\t\t}     \t\t\t}\t\t\t}\t\t}\t\t]\t}}";
   }
}
