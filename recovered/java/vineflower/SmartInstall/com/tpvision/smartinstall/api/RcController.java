...
public class RcController {
   ...
   public List<Integer> getAllRooms() {
      ...
      for (Devices device : JpaManager.getDevicesManager().loadAll()) {
         if (this.isValidRCDevices(device)) {
            String roomidString = device.getTvroomid();
            int intRoomid = TpvStringUtils.tryParseInt(roomidString, -1);
            if (intRoomid != -1) {
      ...
      List<Integer> list = new ArrayList<>(roomSet);
   ...
   public Set<String> getAllGroups() {
      ...
      for (int i = 0; i < data.length(); i++) {
         ...
         if (info.optBoolean("supportRemoteControl")) {
   ...
   @PostMapping("/getSettingsByGroupName")
   ...
   @PostMapping("/updateContentByGroupName")
   public ApiErrorCode updateContentByGroupName(String groupName, int contentId) {
      ...
      List<String> updateAbleDeviceIds = groups.stream()
         .map(g -> JpaManager.getDevicesManager().loadByKey(g.getTvid()))
         .filter(e -> this.isValidRCDevices(e) && !e.getProgress().equalsIgnoreCase("ST"))
         .map(Devices::getId)
         .distinct()
      ...
      String tvIds = StringUtils.join(updateAbleDeviceIds, ',');
   ...
   @PostMapping("/updateSettingByGroupName")
   ...
   private Object getDeviceByRoomNo(String roomNo) {
      ...
      List<Devices> devices = JpaManager.getDevicesManager().findDevicesByRoomId(roomNo).stream().filter(this::isValidRCDevices).collect(Collectors.toList());
      return devices.isEmpty() ? ApiErrorCode.RC_CANT_FIND_DEVICE_FOR_ROOMNO : devices.get(0);
   }
}
