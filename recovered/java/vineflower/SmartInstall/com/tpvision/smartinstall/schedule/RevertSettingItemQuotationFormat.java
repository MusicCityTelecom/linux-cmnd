...
public class RevertSettingItemQuotationFormat extends Job {
   ...
   public void execute() {
      ...
      JpaManager.getSettingPackageManager()
         .findSettingPackagesByPlatforms("2019 MS", "2019 PS", "2019 NAFTA", "TPM215HEA", "TPM215HKN")
         .stream()
         .forEach(
            settingPackage -> {
               String oldValue = settingPackage.getValue();
               String newValue = oldValue.replace(
                     "Input Control.Remote Control.SmartInfo RC Key Mapping", "Input Control.Remote Control.\\\"SmartInfo\\\" RC Key Mapping"
                  )
                  .replace("Input Control.Remote Control.TV Menu RC Key Mapping", "Input Control.Remote Control.\\\"TV Menu\\\" RC Key Mapping")
                  .replace("Input Control.Remote Control.WatchTV RC Key Mapping", "Input Control.Remote Control.\\\"WatchTV\\\" RC Key Mapping")
                  .replace("Input Control.Remote Control.Cast RC Key Mapping", "Input Control.Remote Control.\\\"Cast\\\" RC Key Mapping");
