...
public class ApiJwtAuthFilter implements Filter {
   private static final Logger logger = LoggerFactory.getLogger(ApiJwtAuthFilter.class);
   private static final List<String> SKIP_JWT_CHECK_URL_PREFIX = Arrays.asList(
         "/api/user", "/exapi/login", "/api/pms/showMessageIcon", "/api/mychoice/showTemplateIcon", "/api/reception/socket"
      )
      .stream()
   ...
   private ApiErrorCode checkApiVersion(HttpServletRequest request, String path) {
      ...
      if (apiVer > currentVersion) {
         ...
         return apiVer != currentVersion && !ArrayUtils.contains(backwardSupportVersions, apiVer)
            ? ApiErrorCode.SERVER_API_NOT_COMPATIBLE_WITH_CLINET_API_VERSION
            : null;
      }
   }
   ...
   private ApiErrorCode checkJwtToken(HttpServletRequest httpRequest, String path, ApiErrorCode failureResult) {
      ...
      if (StringUtils.isNotBlank(authorization) && authorization.startsWith("Bearer ")) {
         ...
         try {
            ...
            logger.error("token:{} expire", token);
            failureResult = ApiErrorCode.JWT_TOKEN_EXPIRE;
         } catch (MalformedJwtException ex) {
            logger.error("token:{} format error", token);
         ...
         logger.info("url: {} invalid token info: {}", path, authorization);
