...
import com.tpvision.smartcms.dao.PublishDAO;
...
@Repository
@Transactional
public class PublishDAOImpl
implements PublishDAO {
    @Autowired
    ...
    public Publish getEntityById(int id) {
        Publish website = this.getCurrentSession().get(Publish.class, (Serializable)Integer.valueOf(id));
        return website;
    }
    ...
    public List<Publish> getEntityList(String orientation) {
        ...
        if (Util.isNumeric(orientation)) {
            ...
            l = this.getCurrentSession().createQuery(queryString).setParameter(orientation, (Object)orientation).list();
        } else {
