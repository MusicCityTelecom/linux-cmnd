...
implements ResponseBodyAdvice<Object> {
    ...
    public ResponseEntity<ApiErrorCode> handleHttpRequestMethodNotSupportedException(Exception ex, WebRequest req) {
        return new ResponseEntity<ApiErrorCode>(ApiErrorCode.REQUEST_METHOD_ERROR, ApiErrorCode.REQUEST_METHOD_ERROR.getStatus());
    }

    @ExceptionHandler(value={Exception.class})
    public ResponseEntity<ApiErrorCode> handleException(Exception ex, WebRequest req) {
        logger.error(ex.getMessage(), ex);
        return new ResponseEntity<ApiErrorCode>(ApiErrorCode.SYSTEM_ERROR, ApiErrorCode.SYSTEM_ERROR.getStatus());
    }

    @Override
    public Object beforeBodyWrite(Object body, MethodParameter returnType, MediaType selectedContentType, Class<? extends HttpMessageConverter<?>> selectedConverterType, ServerHttpRequest request, ServerHttpResponse response) {
        if (body instanceof ApiErrorCode) {
            response.setStatusCode(((ApiErrorCode)((Object)body)).getStatus());
        }
