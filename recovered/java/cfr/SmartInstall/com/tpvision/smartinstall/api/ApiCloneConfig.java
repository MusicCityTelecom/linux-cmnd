...
public class ApiCloneConfig {
    ...
    public static final List<String> SUPPORT_PACKAGE_TYPE = Collections.unmodifiableList(Arrays.asList("Firmware", "Clone", "Settings", "Channels", "Welcome", "Apps", "Content", "Banners", "UI", "Schedules"));
    public static final Map<String, List<String>> UPGRADE_PART_CLONE_FILE_MAP = new HashMap<String, List<String>>();
    ...
    public static boolean isValidPackageTypeIgnoreCase(String packageType) {
        ...
        return SUPPORT_PACKAGE_TYPE.stream().map(String::toLowerCase).collect(Collectors.toList()).contains(StringUtils.lowerCase(packageType));
    }

    public static String fixPackageTypeCase(String packageType) {
        if (packageType.equalsIgnoreCase("UI")) {
            return "UI";
        }
        return StringUtils.capitalize(packageType.toLowerCase());
    }
