...
public class WakeupInfoManager {
    ...
    public WakeupInfo getNearestWakeupInfo(String roomId) {
        ...
        for (int i = 0; i < wakeupList.size(); ++i) {
            ...
            if (timeDiff <= 0L || timeDiff >= 86400000L || wakeupList.get(i).getStatus() != 0) continue;
    ...
    public WakeupInfo getNearestOtWakeupInfo(String roomId) {
        ...
        for (int i = wakeupList.size() - 1; i >= 0; --i) {
            if ((this.getTimeDifference(wakeupList.get(i).getWakeupTime()) >= 0L || wakeupList.get(i).getStatus() != 0) && wakeupList.get(i).getStatus() != 1) continue;
            return wakeupList.get(i);
        }
        return null;
    }
    ...
    public void deleteAllWakeups(String roomId) {
        ...
        if (!wakeupList.isEmpty()) {
            ...
            for (int i = 0; i < wakeupList.size(); ++i) {
                if (wakeupList.get(i).getStatus() != 1 || this.getTimeDifference(wakeupList.get(i).getWakeupTime()) <= 0L) continue;
