...
public class SIConfig {
    ...
    public boolean isWeatherServiceEnabled() {
        return null != this.weatherService && this.weatherService.equals(WEATHER_SERVICE_ENABLED);
    }

    public int getRealRefreshRate() {
        int targetRate = 30;
        if (null != this.refreshRate && (targetRate = Integer.valueOf(this.refreshRate).intValue()) < 15) {
            targetRate = 15;
        }
        return targetRate;
    }
