...
extends TmsUtils {
        ...
        try {
            this.socketClient = SocketClientHelper.createClient(this.address, this.port, new SocketClientCallback(){
                ...
                public void onConnected() {
                    LOG.info("Client ({}:{}) connected", (Object)OracleTmsUtils.this.address, (Object)OracleTmsUtils.this.port);
                ...
                public void onReceived(byte[] data) {
                    ...
                    LOG.info("Client({}:{}) Received:{}", OracleTmsUtils.this.address, OracleTmsUtils.this.port, res);
                    try {
                ...
                public void onSent(byte[] data) {
                    LOG.info("Client({}:{}) Sent:{}", OracleTmsUtils.this.address, OracleTmsUtils.this.port, new String(data).trim());
                }

                @Override
                public void onDisconnected() {
                    LOG.info("Client({}:{}) disconnected", (Object)OracleTmsUtils.this.address, (Object)OracleTmsUtils.this.port);
                    SocketClientHelper.removeClient(OracleTmsUtils.this.address, OracleTmsUtils.this.port);
                    OracleTmsUtils.this.setConnected(false);
                }
            });
    ...
    public void requestExpressCheckout(String roomid) {
        ...
        String date = TpvDateUtils.formatLocalDate(new Date(), DATE_YYMMDD);
        String time = TpvDateUtils.formatLocalDate(new Date(), TIME_HHMMSS);
        GuestInfo gi = PmsUtils.getGuestInfoByRoomId(roomid);
    ...
    public String isConnectedTms() {
        try {
            LOG.debug("Tms ip/domain: {}", (Object)this.address);
            if (!this.isSocketAlive()) {
    ...
    private String getDate() {
        return TpvDateUtils.formatLocalDate(new Date(), DATE_YYMMDD);
    }

    private String getTime() {
        return TpvDateUtils.formatLocalDate(new Date(), TIME_HHMMSS);
    }

    private String sendLinkActiveCommand() {
        if (this.sendCommandToTMS(String.format(Locale.ENGLISH, "LA|DA%s|TI%s|", this.getDate(), this.getTime()))) {
            return "Ok";
        }
        return ERROR_TCP_RETURN_ERROR;
    }

    private String sendLinkStartCommand() {
        if (this.sendCommandToTMS(String.format(Locale.ENGLISH, "LS|DA%s|TI%s|", this.getDate(), this.getTime()))) {
            return "Ok";
        }
        return ERROR_TCP_RETURN_ERROR;
    }
    ...
    private String sendLinkEndCommand() {
        if (this.sendCommandToTMS(String.format(Locale.ENGLISH, "LE|DA%s|TI%s|", this.getDate(), this.getTime()))) {
            return "Ok";
        }
        return ERROR_TCP_RETURN_ERROR;
    }
    ...
    public void processReceivedData(String data) throws IOException {
        LOG.info("FIAS >>>>> SI:{}", (Object)data);
        Map<String, String> record = OracleTmsUtils.parseNotification(data);
        if (record.isEmpty()) {
            LOG.error("parsed data:{} is null,exit", (Object)data);
            return;
        }
        switch (record.get("ID")) {
            ...
            case "LA": {
                ...
                if (!this.isOnline || this.isDatabaseSync) break;
                this.refresh();
                break;
            }
            case "LE": {
            ...
            default: {
                LOG.info("Unkown Oracle TMS Command:{}", (Object)record.get("ID"));
            }
        }
    }
    ...
    private void processWakeup(Map<String, String> record, boolean clearFlag) {
        ...
        String wakeupTime = OracleTmsUtils.formatDateTime(record.get("DA"), record.get("TI"), "yyyy-MM-dd HH:mm:ss");
        WakeupInfoManager wakeupInfoManager = JpaManager.getWakeupInfoManager();
        if (clearFlag) {
            LOG.info("Oracle TMS clear wakeup:{},{}", (Object)roomNo, (Object)wakeupTime);
            ...
            LOG.info("Oracle TMS set wakeup:{},{}", (Object)roomNo, (Object)wakeupTime);
    ...
    private void processMessageText(Map<String, String> record) {
        ...
        if (record.get("MI") != null) {
            String msgtime = OracleTmsUtils.formatDateTime(record.get("DA"), record.get("TI"), "yyyy-MM-dd HH:mm:ss");
    ...
    private void processDeleteMessage(Map<String, String> record) {
        ...
        LOG.info("Oracle TMS delete message: {}", (Object)record.get("MT"));
        if (msgId != null) {
    ...
    private void processExpressCheckout(Map<String, String> record) {
        ...
        String billDateTime = OracleTmsUtils.formatDateTime(record.getOrDefault("DA", ""), record.getOrDefault("TI", ""), BILLDATE_TIME_FORMAT);
    ...
    private void processBalance(Map<String, String> record) {
        ...
        String billDateTime = OracleTmsUtils.formatDateTime(record.getOrDefault("DA", TpvDateUtils.formatLocalDate(new Date(), DATE_YYMMDD)), record.getOrDefault("TI", TpvDateUtils.formatLocalDate(new Date(), TIME_HHMMSS)), BILLDATE_TIME_FORMAT);
    ...
    private void processGuestChanged(Map<String, String> record) throws IOException {
        ...
        String arrival = OracleTmsUtils.formatDate(record.getOrDefault("GA", gi.getArrivalDate()), "dd/MM/yyyy");
        String departure = OracleTmsUtils.formatDate(record.getOrDefault("GD", gi.getDepartureDate()), "dd/MM/yyyy");
    ...
    private void processCheckedin(Map<String, String> record) throws IOException {
        ...
        String arrival = OracleTmsUtils.formatDate(record.getOrDefault("GA", ""), "dd/MM/yyyy");
        String departure = OracleTmsUtils.formatDate(record.getOrDefault("GD", ""), "dd/MM/yyyy");
        rootElt.addElement("arrival").setText(arrival);
        rootElt.addElement("departure").setText(departure);
        rootElt.addElement("tv").setText("Standard");
        switch (vrValue = record.getOrDefault("VR", "")) {
            case "VA": {
                rootElt.addElement("viewbill").setText("True");
    ...
    private void processLinkStart(Map<String, String> record) {
        ...
        String[] INIT_CMDS = new String[]{"LR|RIGI|FLRNG#GSSFGNGLGGGTGFGAGD|", "LR|RIGC|FLRNG#GSROGNGLGGGTGFGAGDDATI|", "LR|RIGO|FLRNG#GSSFDATI|", "LR|RIXL|FLG#MIMTRNDATI|", "LR|RIXT|FLG#MIMTRNDATI|", "LR|RIXR|FLG#RN|", "LR|RIXI|FLG#RNF#FDBDBIDCDATI|", "LR|RIXB|FLG#RNBADATI|", "LR|RIXC|FLG#RNASDATI|", "LR|RIWR|FLRNDATI|", "LR|RIWA|FLRNDATIAS|", "LR|RIWC|FLRNDATI|", "LR|RIXD|FLG#MIMTRNDATI|", "LR|RIXM|FLG#RNMIRTDATI|"};
    ...
    public static String formatDateTime(String date, String time, String format) {
        ...
        try {
            String pattern = (date.length() == 6 ? DATE_YYMMDD : DATE_YYYYMMDD) + TIME_HHMMSS;
    ...
    protected void doLoadConfigs() {
        LOG.info("FIAS configs:{}", (Object)this.config);
    ...
    public void updateMessageStatus(String msgId, PmsUtils.MessageStatus newStatus) {
        ...
        switch (newStatus) {
            ...
            default: {
                LOG.info("Message status {} not need update to PMS", (Object)newStatus);
            }
        }
    ...
    public void responseWakeup(String roomId, String wakeupTime, String status) {
        String wakeupAs = String.format(Locale.ENGLISH, "WA|RN%s|DA%s|TI%s|AS%s|", roomId, OracleTmsUtils.parseWakeupDate(wakeupTime), OracleTmsUtils.parseWakeupTime(wakeupTime), status);
