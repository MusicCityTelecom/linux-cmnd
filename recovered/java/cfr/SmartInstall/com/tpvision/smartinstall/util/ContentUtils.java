...
public class ContentUtils {
    ...
    public static String getContentList() {
        String contentList = ContentUtils.getDataFromUrl(GET_LIST_URL);
    ...
    public static List<Integer> findContentIdByTitle(String title) {
        ...
        String data = ContentUtils.getContentList();
        if (null != data) {
            JSONArray jsonArr = new JSONArray(data);
            for (int i = 0; i < jsonArr.length(); ++i) {
                JSONObject obj = jsonArr.optJSONObject(i);
                if (!StringUtils.equalsIgnoreCase(title, obj.optString("title"))) continue;
    ...
    public static Content getContent(String id) {
        ...
        if (null != data) {
            ...
            for (int i = 0; i < jsonArr.length(); ++i) {
                JSONObject obj = jsonArr.optJSONObject(i);
                if (!id.equals(obj.optString("id"))) continue;
    ...
    public static String getContentTitle(int websiteId) {
        Content content = ContentUtils.getContent(String.valueOf(websiteId));
    ...
    private static String getDataFromUrl(String urlStr) {
        ...
        if (null != conn) {
            try (InputStreamReader input = new InputStreamReader(conn.getInputStream(), StandardCharsets.UTF_8);
                 BufferedReader r = new BufferedReader(input);){
                String str = "";
                do {
                    if (null == (str = r.readLine()) || "null".equals(str)) continue;
                    result.append(str);
    ...
    public static String downloadContentZip(String websiteId) throws IOException {
        LOG.info("download content:{}", (Object)websiteId);
        ...
        ContentUtils.getZipFromUrl(getzipUrl, storeZipFile.getAbsolutePath());
    ...
    public static String copyWebsiteToAssembly(String websiteId, String platform, String outputPath) throws IOException {
        LOG.info("copyWebsiteToAssembly,websiteId={}", (Object)websiteId);
        String tempZipFile = ContentUtils.downloadContentZip(websiteId);
        ...
        if (StringUtils.isNotBlank(txtName)) {
            String changedDate = ContentUtils.getSmartInfoChangedTime(String.valueOf(websiteId));
        ...
        LOG.info("AssginContentServlet:delete temp file {} successfully!", (Object)tempZipFile);
        return unZipSmartInfoBrowserLocationStr;
    }

    public static String getSmartInfoChangedTime(String id) {
        Content content = ContentUtils.getContent(id);
    ...
    public static void getZipFromUrl(String url, String destZipFileStr) throws IOException {
        LOG.info("download content from:{}", (Object)url);
