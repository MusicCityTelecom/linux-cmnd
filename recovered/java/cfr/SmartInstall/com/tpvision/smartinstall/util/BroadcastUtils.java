...
public class BroadcastUtils {
    ...
    public static void broadcastWakeOnLanMagicPackageForAddress(String macStr, InetAddress address) {
        LOG.info("broadcast wakeup package to interface {} for mac address {}", (Object)address, (Object)macStr);
        try {
            byte[] wolBytes = BroadcastUtils.assembleWOLCommand(macStr);
            BroadcastUtils.broadcast(wolBytes, address, 7);
        }
        catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    public static void broadcastWakeOnLanMagicPackageForAllNetworkInterfaces(String macStr) {
        LOG.info("broadcast wakeup package to all interface for mac address {}", (Object)macStr);
        try {
            byte[] wolBytes = BroadcastUtils.assembleWOLCommand(macStr);
            for (InetAddress address : BroadcastUtils.listAllBroadcastAddresses()) {
                BroadcastUtils.broadcast(wolBytes, address, 7);
            }
        }
        catch (Exception ex) {
    ...
    private static void broadcast(byte[] sendBytes, InetAddress address, int port) throws IOException {
        LOG.info("broadcast wakeup package to interface {}", (Object)address);
    ...
    private static byte[] assembleWOLCommand(String macStr) {
        int i;
        byte[] macBytes = BroadcastUtils.getMacBytes(macStr);
    ...
    public static void main(String[] args) throws IOException {
        BroadcastUtils.broadcastWakeOnLanMagicPackageForAllNetworkInterfaces("1C:5A:6B:CC:FA:A9");
        BroadcastUtils.broadcastWakeOnLanMagicPackageForAddress("1C:5A:6B:CC:FA:A9", InetAddress.getByName("255.255.255.255"));
    }
}
