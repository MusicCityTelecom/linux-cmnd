...
public class CastServerUtils {
    ...
    private static AnalyticalDataSyncState currentSyncState = AnalyticalDataSyncState.IDLE;
    ...
    public static String registerActivationCodeToPpdsCastServerr(String activationCode, String serialNumber, String macAddress, String hotelId) {
        ...
        return CastServerUtils.sendRequestToPpdsCastServer(HttpMethod.POST, "/v1/register ", null, regJson);
    }

    public static String unregisterCodeToPpdsCastServer(CastServerSetting castServerSetting) {
        return CastServerUtils.sendRequestToPpdsCastServer(HttpMethod.POST, "/v1/unregister/" + castServerSetting.getPropertyId(), castServerSetting, null);
    }

    public static String getSetupInfoFromPpdsCastServer(CastServerSetting castServerSetting) {
        return CastServerUtils.sendRequestToPpdsCastServer(HttpMethod.GET, "/v1/setup/" + castServerSetting.getPropertyId(), castServerSetting, null);
    }

    public static String submitSetupInfoToPpdsCastServer(CastServerSetting castServerSetting, JSONObject submitData) {
        return CastServerUtils.sendRequestToPpdsCastServer(HttpMethod.POST, "/v1/setup/" + castServerSetting.getPropertyId(), castServerSetting, submitData);
    }

    public static String checkStatusFromPpdsCastServer(CastServerSetting castServerSetting) {
        return CastServerUtils.sendRequestToPpdsCastServer(HttpMethod.GET, "/v1/checkstatus/" + castServerSetting.getPropertyId(), castServerSetting, null);
    }
    ...
    public static String getCastAnalyticsDataFromPpdsCastServer(CastServerSetting castServerSetting, JSONObject queryParameter) {
        return CastServerUtils.sendRequestToPpdsCastServer(HttpMethod.GET, "/v1/analytics/" + castServerSetting.getPropertyId(), castServerSetting, queryParameter);
    }

    public static String customizeLandingPageToPpdsCastServer(CastServerSetting castServerSetting, String encodeBase64Background, String ssidName, String guestDnsName, boolean isShowUrl) {
        ...
        return CastServerUtils.sendRequestToPpdsCastServer(HttpMethod.POST, "/v1/landingpage/" + castServerSetting.getPropertyId(), castServerSetting, dataJson);
    }
    ...
    public static void forwardDeviceRoomListToPpdsCastServer() {
        ...
        CastServerUtils.sendAsyncRequestToPpdsCastServer(HttpMethod.POST, "/v1/devicelist/" + castServerSetting.getPropertyId(), castServerSetting, result);
    }
    ...
    public static void startSyncAnalyticalData() {
        if (!CastServerUtils.tryToUpdateSyncStatus()) {
            return;
        }
        try {
            JSONObject resJson;
            ...
            if (TpvStringUtils.isJSONString(responseData = CastServerUtils.getCastAnalyticsDataFromPpdsCastServer(castServerSetting, queryParameter)) && (dataList = (resJson = new JSONObject(responseData)).optJSONArray("list")) != null) {
                int j = dataList.length();
                for (int i = 0; i < j; ++i) {
                    CastServerUtils.doAnalyticalDataSave(castAnalyticalDataManager, dataList, i);
                }
            }
        }
        catch (Exception e) {
            ...
            CastServerUtils.revertUpdateSyncStatus();
        }
    }
    ...
    public static void sendCheckInNoticeToOpenApiServer(GuestInfo guestInfo) {
        ...
        CastServerUtils.sendAsyncRequestToOpenApiServer(HttpMethod.POST, "/pms/checkin/" + CastServerUtils.getRequstPropertyId(castServerSetting), castServerSetting, sendData);
    }
    ...
    public static void sendCheckoutNoticeToOpenApiServer(GuestInfo guestInfo) {
        ...
        sendData.put("RoomNumber", guestInfo.getRoomid());
        sendData.put("GuestNumber", guestInfo.getGuestId());
        CastServerUtils.sendAsyncRequestToOpenApiServer(HttpMethod.POST, "/pms/checkout/" + CastServerUtils.getRequstPropertyId(castServerSetting), castServerSetting, sendData);
    }
    ...
    public static void sendChangeRoomNoticeToOpenApiServer(GuestInfo guestInfo, String oldRoom, String newRoom) {
        ...
        sendData.put("GuestNumber", guestInfo.getGuestId());
        CastServerUtils.sendAsyncRequestToOpenApiServer(HttpMethod.POST, "/pms/roomchange/" + CastServerUtils.getRequstPropertyId(castServerSetting), castServerSetting, sendData);
    }

    private static synchronized boolean tryToUpdateSyncStatus() {
        if (currentSyncState != AnalyticalDataSyncState.IDLE) {
            LOG.info("start sync analytical data failure as the current status is in : <{}>", (Object)currentSyncState);
            return false;
        }
        currentSyncState = AnalyticalDataSyncState.SYNCING;
        return true;
    }

    private static synchronized void revertUpdateSyncStatus() {
        currentSyncState = AnalyticalDataSyncState.IDLE;
    }
    ...
    private static void sendAsyncRequestToOpenApiServer(HttpMethod method, String url, CastServerSetting castServerSetting, JSONObject jsonObject) {
        new Thread(() -> CastServerUtils.sendRequestToWebServer(method, OPENAPI_URL_ADDR + url, castServerSetting, jsonObject)).start();
    }
    ...
    private static void sendAsyncRequestToPpdsCastServer(HttpMethod method, String url, CastServerSetting castServerSetting, JSONObject jsonObject) {
        new Thread(() -> CastServerUtils.sendRequestToWebServer(method, ENDPOINT_URL_ADDR + url, castServerSetting, jsonObject)).start();
    }
    ...
    private static String sendRequestToPpdsCastServer(HttpMethod method, String url, CastServerSetting castServerSetting, JSONObject jsonObject) {
        return CastServerUtils.sendRequestToWebServer(method, ENDPOINT_URL_ADDR + url, castServerSetting, jsonObject);
    }
    ...
    private static String sendRequestToWebServer(HttpMethod method, String url, CastServerSetting castServerSetting, JSONObject jsonObject) {
        ...
        if (castServerSetting != null) {
            headerMap = CastServerUtils.getRequestAuthHeader(castServerSetting);
        }
        LOG.info("request online server:method:{}, url:{}, headerMap:{}, content:{}", new Object[]{method, url, headerMap, jsonObject});
        ...
        requestEntity = jsonObject != null ? new HttpEntity<String>(jsonObject.toString(), requestHeaders) : new HttpEntity<String>("", requestHeaders);
        try {
            ResponseEntity<String> responseEntity = restTemplate.exchange(url, method, requestEntity, String.class, new Object[0]);
            String responseBody = (String)responseEntity.getBody();
            LOG.info("request sucess,code=<{}> response=<{}>", (Object)responseEntity.getStatusCode(), (Object)responseBody);
            return responseBody;
        }
    ...
    private static Map<String, String> getRequestAuthHeader(CastServerSetting castServerSetting) {
        ...
        authMap.put("Authorization", "Basic " + Base64.getEncoder().encodeToString((castServerSetting.getApiUser() + ":" + castServerSetting.getApiPassword()).getBytes()));
        return authMap;
    }

    public static enum AnalyticalDataSyncState {
        IDLE,
        SYNCING,
        INTERRUPPED;
