...
public class PmsMessageUtils {
    ...
    public static JSONArray loadPmsIconArray() {
        ...
        String iconPath = PmsMessageUtils.getMessageIconPath();
    ...
    private static String getMessageIconPath() {
        return String.format(Locale.ENGLISH, CommonConstants.MESSAGE_ICON_PATH_FORMAT, new Object[0]);
    }

    public static void deleteMessageIcon(String iconName) {
        String iconPath = PmsMessageUtils.getMessageIconPath() + iconName;
        LOG.info("delete icon:", (Object)iconName);
        TpvFileUtils.deleteFile(iconPath);
    }

    public static String doMessageIconUpload(HttpServletRequest request) throws Exception {
        String logoPath = PmsMessageUtils.getMessageIconPath();
        ...
        for (FileItem item : items) {
            ...
            String shortName = filename.substring(0, filename.length() > 4 ? 4 : filename.length());
        ...
        PmsMessageUtils.tidyMessageIconFiles();
        return filename;
    }
    ...
    public static String doDefaultMessageIconUpload(HttpServletRequest request) throws Exception {
        ...
        for (FileItem item : items) {
            if (item.isFormField()) continue;
            filename = PmsMessageUtils.saveIconFileToServer("message", item);
        }
        return filename;
    }
    ...
    public static String doDefaultBillIconUpload(HttpServletRequest request) throws Exception {
        ...
        for (FileItem item : items) {
            if (item.isFormField()) continue;
            filename = PmsMessageUtils.saveIconFileToServer("bill", item);
        }
        return filename;
    }
    ...
    public static String saveIconFileToServer(String iconType, FileItem fileItem) throws Exception {
        ...
        if (StringUtils.isNoneBlank(oldIconName) && !StringUtils.equalsIgnoreCase(imageName, oldIconName) && (oldFile = new File(CommonConstants.RECEPTION_LOGO_IMG_LOCATION + oldIconName)).isFile()) {
    ...
    public static List<Message> createMessage(String roomIds, String title, String content, String icon, String sendTime) {
        ...
        for (int i = 0; i < gis.size(); ++i) {
            ...
            if ("N".equalsIgnoreCase(guest.getCheckin())) {
                LOG.warn("guest <{}> not checked in skip to send message", (Object)guest.getGuestId());
                continue;
            }
            List<Devices> tvs = devicesManager.findDevicesByRoomId(guest.getRoomid());
            boolean isSupportMessage = false;
            for (Devices tv : tvs) {
                if (!PlatformUtils.isSupportMessage(tv.getType())) continue;
                isSupportMessage = true;
                break;
            }
            if (!isSupportMessage) {
                LOG.warn("guest <{}> in room <{}> related device not support message", (Object)guest.getGuestId(), (Object)guest.getRoomid());
                continue;
            }
            ...
            if ("Now".equals(sendTime)) {
                ...
                if (PmsUtils.sendMsg2TV(message.getGuestIds())) {
                    LOG.info("send new message to TV:{}", (Object)message.getGuestIds());
                }
            } else {
                try {
    ...
    private static void tidyMessageIconFiles() throws IOException {
        ...
        if (files.length > 10) {
            for (int i = 0; i < files.length - 10; ++i) {
                LOG.info("remove old icon files:{}", (Object)files[i].getName());
    ...
    public static JSONArray getGuestMessagesArray(Devices tv, List<Message> messages) {
        ...
        for (Message m : messages) {
            if ("Delete".equalsIgnoreCase(m.getStatus())) continue;
            try {
                date = dateFormat1.parse(m.getTimeSend());
            }
            catch (ParseException e) {
                LOG.error("parse date failed:id={},timesend={}", (Object)m.getId(), (Object)m.getTimeSend());
            }
