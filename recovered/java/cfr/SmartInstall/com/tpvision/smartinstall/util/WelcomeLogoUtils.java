...
public class WelcomeLogoUtils {
    ...
    public void addWelcomeLogo(File newWelcomeLogo) throws IOException {
        ...
        if (!(file = new File(CommonConstants.WELCOME_LOGO_THUMB_IMG_LOCATION + imageName)).exists()) {
    ...
    public void updateWelcomeThumbCache() {
        ...
        List<Welcome> welcomes = JpaManager.getWelcomeManager().loadAll();
    ...
    public static JSONArray getWelcomeLogoGallery(String platform) {
        ...
        if (logos != null) {
            for (File logo : logos) {
                if (logo.isDirectory()) continue;
                if (!WelcomeLogoUtils.checkWelcomeLogoCompatible(logo, platform)) {
                    LOG.debug("logo {} is not compatible with platform:{}", (Object)logo.getName(), (Object)platform);
                    continue;
                }
    ...
    public static String getWelcomeLogoImageMd5(int welcomeId) {
        File welcomeLogoFile = WelcomeLogoUtils.getWelcomeLogoFile(welcomeId);
    ...
    public String getWelcomeLogoListByJson(String platform) {
        ...
        List<Welcome> welcomes = JpaManager.getWelcomeManager().findWelcomesByType(type);
        for (Welcome welcome : welcomes) {
            JSONObject obj = new JSONObject();
    ...
    public String getThumbnailUrl(int id) {
        File cached = new File(this.getWebCachePath() + id + THUMBNAIL_EXT);
        return cached.exists() ? "/SmartInstall//static/images/welcomethumb/" + id + THUMBNAIL_EXT : "";
    }

    public static JSONArray getMediaInfos(String welcomeId) {
        JSONArray arr = new JSONArray();
        WelcomeAppData appData = WelcomeLogoUtils.getAppData(welcomeId);
        String welcomePath = WelcomeLogoUtils.getWelcomePath(welcomeId);
    ...
    private static File getWelcomeThumbFile(int welcomeId) {
        return new File(CommonConstants.WELCOME_LOGO_THUMB_IMG_LOCATION + welcomeId + THUMBNAIL_EXT);
    }
    ...
    public void copyWelcome(int id) throws IOException {
        ...
        List<String> currentWelcomeNames = JpaManager.getWelcomeManager().loadAll().stream().map(Welcome::getName).map(String::trim).collect(Collectors.toList());
        ...
        String origPath = WelcomeLogoUtils.getWelcomePath(String.valueOf(id));
        String newPath = WelcomeLogoUtils.getWelcomePath(String.valueOf(newId));
    ...
    public static void deleteWelcome(int id) throws IOException {
        ...
        String welcomepath = WelcomeLogoUtils.getWelcomePath(String.valueOf(id));
        File welcomePathFile = new File(welcomepath);
        if (welcomePathFile.exists()) {
            FileUtils.deleteQuietly(welcomePathFile);
        }
        if ((thumb = WelcomeLogoUtils.getWelcomeThumbFile(id)).exists()) {
    ...
    private static File getWelcomeAppPath(File clonePath) {
        ...
        if (professionalApps != null && professionalApps.exists()) {
            return TpvFileUtils.getFolderByName(professionalApps, WELCOME_APP_PATHNAME);
        }
        return null;
    }
    ...
    public static void updateWelcomeLogo(int welcomeId, File welcomeLogo) throws IOException {
        ...
        if (welcomeType == 1) {
            LOG.error("not support update welcome app logo,id={}", (Object)welcomeId);
            return;
        }
        ...
        if ((oldWelcomelogo = WelcomeLogoUtils.getWelcomeLogoFile(welcomeId)) != null && oldWelcomelogo.exists()) {
            FileUtils.deleteQuietly(oldWelcomelogo);
        }
        WelcomeLogoUtils.getInstance().removeThumbnailFile(welcomeId);
        String welcomePath = WelcomeLogoUtils.getWelcomePath(String.valueOf(welcomeId));
        FileUtils.copyFile(welcomeLogo, new File(welcomePath + "/" + targetFileName));
        WelcomeLogoUtils.refreshWelcomeLastEditTime(welcomeId);
        WelcomeLogoUtils.getInstance().generateWelcomeThumb(welcomeId);
            ...
            switch (welcometype) {
                case 0: {
                    ...
                    if (null != welcomeLogo) {
                        ...
                        welcomeLogoFile = TpvFileUtils.getFileByName(clonePathFile, "WelcomeLogo", "jpg,png,jpeg".split(","));
                    ...
                    String idendifierPath = welcomeLogo.getAbsolutePath() + "/WelcomeLogo_Identifier.txt";
                    File fileIdentifier = new File(idendifierPath);
                    if (!fileIdentifier.exists()) break;
                    String idendifier = FileUtils.readFileToString(fileIdentifier, StandardCharsets.UTF_8);
                    welcome.setLastEdit(idendifier);
                    break;
                }
                case 1: {
                    File welcomeApp = WelcomeLogoUtils.getWelcomeAppPath(clonePathFile);
                    if (null != welcomeApp) {
    ...
    private void generateWelcomeThumb(int welcomeId) {
        ...
        if (null != welcomeLogoFile) {
            WelcomeLogoUtils.generateThumb(welcomeLogoFile, welcomeId);
            try {
                this.addWelcomeLogo(welcomeLogoFile);
            }
            catch (IOException e) {
            ...
            LOG.info("no image found for thumbnail:{}", (Object)welcomeId);
    ...
    private void removeThumbnailFile(int welcomeId) {
        File cached;
        File file = WelcomeLogoUtils.getWelcomeThumbFile(welcomeId);
        if (file.exists()) {
            FileUtils.deleteQuietly(file);
        }
        if ((cached = new File(this.getWebCachePath() + welcomeId + THUMBNAIL_EXT)).exists()) {
    ...
    public void generateWelcomeAppImageThumb(String welcomeId) {
        ...
        if (imagesFile.exists() && imagesFile.isDirectory()) {
            File[] images;
            for (File image : images = imagesFile.listFiles()) {
                File thumbFile;
                String ext = FilenameUtils.getExtension(image.getName());
                ext = ext.toLowerCase();
                String thumbPath = this.getWebCachePath() + welcomeId + "/" + image.getName();
                if (!Arrays.asList(extFilters).contains(ext) || (thumbFile = new File(thumbPath)).exists()) continue;
    ...
    public static void saveAppConfig(String welcomeId, WelcomeAppSettings appSettings) {
        ...
        WelcomeLogoUtils.refreshWelcomeLastEditTime(Integer.parseInt(welcomeId));
    }
    ...
    public static void addContentItem(File uploaded, String welcomeId) throws IOException {
        ...
        contentItem.Content = WELCOME_APP_CONTENT_PREFIX + contentName;
    ...
    public static void saveAppData(String welcomeId, WelcomeAppData appData) {
        ...
        try {
            FileUtils.write(dataFile, (CharSequence)dataContent, StandardCharsets.UTF_8);
        }
        catch (IOException e) {
            LOG.error(e.getMessage(), e);
        }
        ...
        if (contentsFile != null) {
            for (File contentFile : contentsFile) {
                ...
                if (appData.getContentItemByName(contentName) != null) continue;
                FileUtils.deleteQuietly(contentFile);
            }
        }
        WelcomeAppSettings appSettings = WelcomeLogoUtils.getAppConfig(welcomeId);
        String contentValue = appData.CommandDetails.Content == null || appData.CommandDetails.Content.isEmpty() ? "Philips slideshow" : "Custom";
        WelcomeLogoUtils.updateAppConfigItem("Edit Welcome Screen.Set Background.Background Content", contentValue, appSettings);
        WelcomeLogoUtils.saveAppConfig(welcomeId, appSettings);
        WelcomeLogoUtils.getInstance().generateWelcomeAppImageThumb(welcomeId);
        WelcomeLogoUtils.getInstance().generateWelcomeThumb(Integer.parseInt(welcomeId));
        WelcomeLogoUtils.getInstance().updateWelcomeThumbCache();
        WelcomeLogoUtils.refreshWelcomeLastEditTime(Integer.parseInt(welcomeId));
    }
    ...
    public static WelcomeAppData getAppData(String welcomeId) {
        ...
        while (iterator.hasNext()) {
            ...
            if (!StringUtils.isEmpty(item.Content)) continue;
            iterator.remove();
        }
