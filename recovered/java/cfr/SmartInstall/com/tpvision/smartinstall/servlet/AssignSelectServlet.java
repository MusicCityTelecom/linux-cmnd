...
import com.tpvision.smartinstall.servlet.AssignContentServlet;
...
@WebServlet(value={"/assignSelect"})
public class AssignSelectServlet
extends BaseHttpServlet {
    ...
    private String getFirmwareList(String platforms, String versions, String tvGroups) {
        List<UpgSetting> upgSettings;
        UpgSettingManager upgMgr = JpaManager.getUpgSettingManager();
        List<UpgSetting> filterUpgSettings = upgSettings = StringUtils.isEmpty(platforms) ? upgMgr.loadAll() : upgMgr.findUpgSettingsByPlatforms(platforms.split(","));
    ...
    private List<UpgSetting> filterLowerVersionByTvGroups(List<UpgSetting> upgSettings, String tvGroups) {
        ArrayList<UpgSetting> list = new ArrayList<UpgSetting>();
        ...
        ArrayList<String> tvGroupNames = new ArrayList<String>();
        for (int i = 0; i < tvGroupsArray.size(); ++i) {
            ...
            if (!PlatformUtils.isNeedCheckLowerVersion(platform)) continue;
            tvGroupNames.add(tvGroupName);
        }
        JsonArray jsonArray = JpaManager.getDevicesManager().getDeviceJsonDataByTvGroup(String.join((CharSequence)",", tvGroupNames));
        ...
        for (UpgSetting upgSetting : upgSettings) {
            ...
            if (PlatformUtils.compareLowerVersion(settingVersion, maxVersion)) continue;
            list.add(upgSetting);
        }
        return list;
    }
    ...
    private String getContentList() {
        ...
        data = "{\"status\":\"success\",\"data\":" + contentData + "}";
        return data;
    }
    ...
    private String getBannersList(String platforms) {
        ...
        if (StringUtils.isBlank(platforms)) {
            ...
            for (String platformName : platformArray = platforms.split(",")) {
                LOG.info("platform Name : {}", (Object)platformName);
                if (!StringUtils.isNotBlank(platformName) || !PlatformUtils.getSupportCloneItems(platformName).contains((Object)CommonConstants.CloneItemType.Banner)) continue;
