...
@WebServlet(value={"/appPackage"})
public class AppPackageServlet
extends HttpServlet {
    ...
    private void appIndex(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (null == appPackage) {
            LOG.error("appPackage is null,id is:{}", (Object)id);
        } else {
            String appsRef = null;
            String name = appPackage.getName();
            String embed = request.getParameter("embeded");
            appsRef = embed != null && embed.equalsIgnoreCase("true") ? "/jsp/setting/subsettings/apps.jsp?name=" + name + "&id=" + id : "/appContent.jsp?name=" + name + "&id=" + id;
    ...
    private void getAssignAppPackageList(HttpServletRequest request, HttpServletResponse response) {
        ...
        appPackageList = StringUtils.isNotBlank(platform) ? JpaManager.getAppPackageManager().findAppPackageByPlatforms(platform) : JpaManager.getAppPackageManager().loadAll();
        String jsonStr = new Gson().toJson(appPackageList);
        try {
            IOUtils.write(jsonStr.getBytes(), (OutputStream)response.getOutputStream());
        }
        catch (IOException e) {
            LOG.error(e.getMessage(), e);
        }
    ...
    private void assignAppPackage(HttpServletRequest request, HttpServletResponse response) {
        ...
        if ("None".equalsIgnoreCase(appPackageId)) {
            ...
            if (appPackage == null) {
                LOG.error("selected AppPackage:{} not exists", (Object)appPackageId);
