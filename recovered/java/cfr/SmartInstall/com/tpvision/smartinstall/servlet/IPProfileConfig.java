...
public class IPProfileConfig {
    ...
    public boolean isValidConfigName(String configName) {
        try {
            IPProfileConfigName.valueOf(configName);
            return true;
        }
        catch (Exception e) {
            return false;
    ...
    public void updateConfig(IPProfileConfigName configName, String value) {
        this.configMap.put(configName.name(), value);
    }

    public void updateConfigs(Map<String, String> params) {
        for (Map.Entry<String, String> entry : params.entrySet()) {
            if (!this.isValidConfigName(entry.getKey())) continue;
            this.configMap.put(entry.getKey(), entry.getValue());
        }
    }

    public String getConfig(IPProfileConfigName configName) {
    ...
    public static IPProfileConfig fromJson(String json) {
        IPProfileConfig ipProfileConfig = IPProfileConfig.getDefaultIPProfileConfig();
        ...
        try {
            ...
            if (jsObj.has("configMap")) {
                ...
                Map retMap = (Map)new Gson().fromJson(configs, new TypeToken<Map<String, String>>(){}.getType());
    ...
    private static IPProfileConfig getDefaultIPProfileConfig() {
        ...
        for (IPProfileConfigName configName : IPProfileConfigName.values()) {
            if (configName.name().endsWith("_dropdownText")) {
                ipProfileConfig.updateConfig(configName, "10");
                continue;
            }
            if (configName.name().endsWith("_page")) {
    ...
    public static enum IPProfileConfigName {
        TabIndex,
        PMS_tabsIndex,
        PMS_tabsRooms_sort,
