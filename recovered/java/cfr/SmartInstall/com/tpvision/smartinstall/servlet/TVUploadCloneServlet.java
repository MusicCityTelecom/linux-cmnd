...
@WebServlet(value={"/CloneToServer"})
public class TVUploadCloneServlet
extends HttpServlet {
    ...
    private void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if ((tvInfo = this.getTvInfoByIp(tvIpaddress)) == null) {
            LOG.error("get uploaded tv by clientip {} failure", (Object)tvIpaddress);
            return;
        }
        try {
            StopWatch stopWatch = new StopWatch();
            stopWatch.start();
            LOG.info("real.chen: start to receive zip file from ip={}", (Object)tvIpaddress);
            ...
            stopWatch.stop();
            LOG.info("upload costtime:{}ms", (Object)stopWatch.getTime());
        }
        catch (Exception e) {
    ...
    private void processUploadedFile(Devices tvInfo, FileItem item, PrintWriter outNet) throws Exception {
        ...
        LOG.info("saved file:{},fileSize:{}b", (Object)uploadedFile.getAbsolutePath(), (Object)fileSize);
        outNet.flush();
        ...
        LOG.info("tvUniqueID:{},requireUploadSize:{},currentUploadCount:{}", tvUniqueID, UploadCloneUtils.uploadItemsCount_tv.get(tvUniqueID), UploadCloneUtils.uploadItemsRecvCount_cmnd.get(tvUniqueID));
    }

    private Devices getTvInfoByIp(String clientip) {
        DevicesManager iptvmanager = JpaManager.getDevicesManager();
        List<Devices> list = iptvmanager.findDevicesByTvipaddressAndCloneMode(clientip, "Upload");
        for (Devices tv : list) {
            if (!UploadCloneUtils.uploadItemsCount_tv.containsKey(tv.getTvuniqueid())) continue;
            return tv;
        }
        return null;
    }
