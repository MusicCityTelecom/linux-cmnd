...
public class UpgCreator {
    ...
    public String getParams(String pathname, UpgCreationCommandArgument uca) {
        ...
        if (uca.playout.toUpperCase().contains("F")) {
            for (File f : files) {
                if (!f.getName().equalsIgnoreCase("Autorun.upg") || f.length() <= 0L) continue;
                sbuild.append("s* ");
            }
        }
        if (uca.playout.toUpperCase().contains("C")) {
            if (uca.hasAll) {
                ...
                for (File f : files) {
                    ...
                    if (!f.getName().equalsIgnoreCase("ThemeTV") || !f.isDirectory()) continue;
                    sbuild.append("t* ");
                }
            } else {
                if (uca.hasHotelInfoPages) {
                    for (File f : files) {
                        if (!f.getName().equals("SmartUI") || !f.isDirectory()) continue;
                        sbuild.append("u* ci* ");
                    }
                }
                if (uca.hasHotelInfo) {
                    for (File f : files) {
                        if (!f.getName().equalsIgnoreCase("hotelinfo.jpg") || f.length() <= 0L) continue;
                        sbuild.append("h* ci* ");
                    }
                }
                if (uca.hasWelcomeLogo) {
                    for (File f : files) {
                        if (!f.getName().equalsIgnoreCase("welcomelogo.jpg") || f.length() <= 0L) continue;
                        sbuild.append("w* ci* ");
                    }
                }
                if (uca.hasThemeTV) {
    ...
    public CommandLine build2K14MSCmd(String pathname, UpgCreationCommandArgument uca, CommandLine cmdLine, String platformId) {
        File[] files = new File(pathname).listFiles();
        ...
        LOG.info(" cmdLine >> {}", (Object)cmdLine);
        return cmdLine;
    }
    ...
    public static UpgCreationCommandArgument getCommandArument(Map<String, String> parameters) {
        boolean rfall = false;
        rfall = true;
        String output = parameters.get("output");
        return new UpgCreationCommandArgument.Builder().all(rfall).rfPlayOut(output).build();
    }
    ...
    public static class UpgCreationCommandArgument {
        ...
        private UpgCreationCommandArgument(Builder build) {
        ...
        public static class Builder {
            ...
            public Builder themeTv(boolean theme) {
            ...
            public Builder welcomeLogo(boolean wl) {
                if (this.hasAll && wl) {
                    this.toggleAll();
                }
            ...
            public Builder hotelInfo(boolean hi) {
                if (this.hasAll && hi) {
                    this.toggleAll();
                }
            ...
            public Builder hotelInfoPages(boolean hip) {
                if (this.hasAll && hip) {
                    this.toggleAll();
                }
            ...
            public Builder setting(boolean se) {
                if (this.hasAll && se) {
                    this.toggleAll();
                }
                this.hasSetting = se;
            ...
            public Builder tvChannelList(boolean tvChannelList) {
            ...
            public Builder rfPlayOut(String put) {
                this.output = put;
                return this;
            }

            public UpgCreationCommandArgument build() {
                return new UpgCreationCommandArgument(this);
            }
        }
    }
}
