...
public class ChannelHelper {
    ...
    public void sortChannels() throws BaseHttpServlet.MessageException {
        this.checkValid();
        ArrayList<JSONObject> localchannelList = new ArrayList<JSONObject>();
    ...
    public void swapChannelsNo(String swapIds) throws BaseHttpServlet.MessageException {
        ...
        if (swapIdArray.length != 2) {
            LOG.error("swap ids <{}> format error", (Object)swapIds);
        ...
        if (id1 < 0 || id1 >= count || id2 < 0 || id2 >= count || id1 == id2) {
            LOG.error("invalid swap ids <{}>", (Object)swapIds);
        ...
        if (channelSetup1 == null || channelSetup2 == null) {
            LOG.error("missing channel setup data <{}> and <{}>", (Object)channelSetup1, (Object)channelSetup2);
    ...
    public void updateChannel(int id, String jsChannel) throws BaseHttpServlet.MessageException, SQLException {
        ...
        for (String item : items) {
            ...
            for (String keyStr : itemObj.keySet()) {
                ...
                if (!"presetnumber".equalsIgnoreCase(key)) continue;
                this.handleConflictChannelNo(id, keyvalue);
                if (isPresetnumberUpdated) continue;
    ...
    private void deleteMediaOriginalUrl(JSONObject object) {
        try {
            ...
            String srcRootPath = CommonConstants.CLONE_PROCESS_LOCATION + "ChannelPackages" + File.separator + this.channelPackageId + File.separator + "MediaChannels";
            ...
            String oldFileName = originalUrl.replace(LOGO_PREFIX, "");
            String oldFilePath = processMediaChannel.toString() + File.separator + originalUrl.replace(LOGO_PREFIX, "");
    ...
    public void updateApplication(int id, String jsApplication) throws BaseHttpServlet.MessageException, SQLException {
        ...
        for (String item : items = new String[]{"setup"}) {
            JSONObject itemObj = obj.optJSONObject(item);
    ...
    private void handleConflictChannelNo(int channelId, String channelNo) {
        for (int i = 0; i < this.jaChannels.length(); ++i) {
            JSONObject setupJson;
            if (i == channelId || (setupJson = this.jaChannels.getJSONObject(i).optJSONObject("setup")) == null) continue;
            String presetnumber = setupJson.optString("presetnumber");
            if (!String.valueOf(channelNo).equals(presetnumber)) continue;
    ...
    private void fixChannelLogoUrl() {
        ...
        ArrayList<Integer> requireLogoUrlList = new ArrayList<Integer>();
        HashMap<String, Integer> currentUsedLogoUrl = new HashMap<String, Integer>();
        for (int i = 0; i < this.jaChannels.length(); ++i) {
            Integer linkCount;
            String fileNameLowerCase;
            File[] obj = this.jaChannels.getJSONObject(i);
            JSONObject setupObj = obj.optJSONObject("setup");
            if (setupObj == null) {
                LOG.warn("invalid channel obj,{}", (Object)obj);
                continue;
            }
            String presetNum = setupObj.optString("presetnumber", "0");
            String logoPath = setupObj.optString("logo", "");
            if (!logoPath.contains("custom/")) continue;
            fileName = logoPath.replace("file://custom/", "");
            if (!fileName.equals(fileNameLowerCase = fileName.toLowerCase())) {
                ...
                if (logoFile.exists()) {
                    if (logoFile.renameTo(new File(channelLogoPath + fileNameLowerCase))) {
                        LOG.info("update to lower case name of file <{}>", (Object)logoFile.getAbsolutePath());
                    } else {
                        LOG.warn("rename file name failure <{}>", (Object)logoFile);
                    }
                }
            }
            if ((linkCount = (Integer)currentUsedLogoUrl.get(fileNameLowerCase)) == null) {
                linkCount = 0;
            }
            Integer n = linkCount;
            Integer n2 = linkCount = Integer.valueOf(linkCount + 1);
            currentUsedLogoUrl.put(fileNameLowerCase, linkCount);
            if (logoPath.contains("custom/" + presetNum + ".")) continue;
        ...
        if (listFiles != null) {
            for (File file : listFiles) {
                if (!file.isFile() || currentUsedLogoUrl.containsKey(fileName = file.getName().toLowerCase())) continue;
        ...
        for (Integer index : requireLogoUrlList) {
            ...
            int leftLinkCount = (Integer)currentUsedLogoUrl.get(oriFileName) - 1;
            if (leftLinkCount <= 0) {
                if (oriFile.renameTo(tempFile)) {
                    LOG.info("rename logo name from <{}> to <{}>", (Object)oriFileName, (Object)tempFileName);
                }
            } else {
                try {
                    FileUtils.copyFile(oriFile, tempFile);
                    LOG.info("copy logo name from <{}> to <{}>", (Object)oriFileName, (Object)tempFileName);
                }
                catch (IOException e) {
                    LOG.error(e.getMessage(), e);
                }
    ...
    public void deleteLogoFile(String logo) {
        ...
        String newFileName = null == logo ? "" : logo.replace(LOGO_PREFIX, "/");
    ...
    public void updateThemeTvIconMapping(Map<String, String> iconMap) throws BaseHttpServlet.MessageException {
        ...
        for (Map.Entry<String, String> ttv : iconMap.entrySet()) {
            String oldIconPath;
            ...
            if ((oldIconPath = themTv.optString("icon", "").replace(LOGO_PREFIX, "")).equalsIgnoreCase(updatedPath)) continue;
    ...
    private void removeExpireThemeTvIcon(String oldIconPath) {
        ...
        String fullLogoPath;
        File oldLogoFile;
        if (oldIconPath.contains("custom") && (oldLogoFile = new File(fullLogoPath = (themeIconPath = CloneItemUtils.getChannelThemeTvIconPath(this.channelPackageId)) + File.separator + oldIconPath)).exists()) {
            try {
    ...
    public void updateChannelLogoMapping(Map<String, String> logsMap) throws BaseHttpServlet.MessageException {
        this.checkValid();
        for (Map.Entry<String, String> logo : logsMap.entrySet()) {
    ...
    private void updateSingChannelLog(String presetNum, String newPicName) {
        ...
        for (int i = 0; i < ja1.length(); ++i) {
            ...
            if (!loopPresetNum.equalsIgnoreCase(presetNum)) continue;
            ...
            if (StringUtils.equalsIgnoreCase(oldLogoPath, newlogoPath) || !oldLogoPath.contains("custom")) continue;
            String oldLogoFileName = oldLogoPath.replace(LOGO_PREFIX, "/");
    ...
    public boolean isPresetNumberExist(String presetNumber) throws BaseHttpServlet.MessageException {
        this.checkValid();
        ArrayList<String> presetNumberList = new ArrayList<String>();
        for (int i = 0; i < this.jaChannels.length(); ++i) {
            JSONObject obj = this.jaChannels.getJSONObject(i);
            JSONObject setupObj = obj.optJSONObject("setup");
            if (setupObj == null) {
                LOG.error("invalid channel obj,{}", (Object)obj);
                continue;
            }
            String presetNum = setupObj.optString("presetnumber", "0");
    ...
    public JSONArray getChannelLogoMap() throws BaseHttpServlet.MessageException {
        ...
        for (int i = 0; i < this.jaChannels.length(); ++i) {
            ...
            String string = logoPath = setupObj.has("logo") ? setupObj.getString("logo") : "";
    ...
    private boolean channelLogoExists(String logoPath) {
        String path = null;
        if (null != logoPath && (path = logoPath.replaceAll(LOGO_PREFIX, "")).startsWith("default/")) {
            path = "";
        }
    ...
    public void updateThemeTvNameList(List<String> nameList) throws BaseHttpServlet.MessageException {
        ...
        for (int i = 1; i <= 10; ++i) {
            ...
            obj.put("name", (themetvname = nameList.get(i - 1)) == null ? "" : themetvname);
            String oldIconPath = obj.optString("icon");
            if (!StringUtils.isBlank(themetvname) || !StringUtils.isNotBlank(oldIconPath)) continue;
            this.removeExpireThemeTvIcon(oldIconPath);
            obj.put("icon", "");
        }
    ...
    public List<String> getChannelNamesByTypeAndMediums(String type, String ... mediums) throws BaseHttpServlet.MessageException {
        this.checkValid();
        ArrayList<String> nameList = new ArrayList<String>();
        List mediumList = Arrays.asList(mediums).stream().map(String::toLowerCase).collect(Collectors.toList());
        for (int i = 0; i < this.jaChannels.length(); ++i) {
            JSONObject channel = this.jaChannels.getJSONObject(i);
            ...
            if (!mediumList.isEmpty() && (medium == null || !mediumList.contains(medium.toLowerCase()))) continue;
            JSONObject setup = channel.optJSONObject("setup");
            nameList.add(setup.optString("presetnumber") + " - " + setup.optString("name"));
        }
        return nameList;
    ...
    public void newChannel(String data) throws BaseHttpServlet.MessageException {
        ...
        if ("v5".equalsIgnoreCase(this.scb.getChannelVersion())) {
            ...
            LOG.error("not support channel add except v4 & v5, current version is :{}", (Object)this.scb.getChannelVersion());
        }
        this.sortChannels();
    }
    ...
    public void addV4ChannelObj(JSONObject obj) {
        ...
        for (Channel channel : channels) {
            ...
            if (!presetNumberOp.isPresent()) continue;
    ...
    public void addV5ChannelObj(JSONObject obj) {
        ArrayList<com.tpvision.smartinstall.xml.channel.v5.Channel> channels = new ArrayList<com.tpvision.smartinstall.xml.channel.v5.Channel>();
    ...
    public List<com.tpvision.smartinstall.xml.channel.v5.Channel> CMNDStreamChannelFromRequest(JSONObject obj) {
        ...
        for (ResponsePlayInfo.AVService avService : avServices) {
            ...
            if (avService.getFreq() > 0L) {
                ...
                String encoding = "VBR";
                String multicasturl = String.format("%s://%s:%d/%d/%d/%d/%s", casttype, avService.getUrl(), avService.getPort(), avService.getSID(), avService.getONID(), avService.getTSID(), encoding);
                Multicast multicast = new Multicast();
                multicast.setUrl(multicasturl);
                channel.setMulticast(multicast);
            }
