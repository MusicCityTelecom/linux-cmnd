...
public class SettingChannelLoader {
    ...
    public static com.tpvision.smartinstall.xml.channel.v4.TvContents loadChannelV4(String processCloneFilePath, String platformId) throws IOException, JAXBException {
        ...
        if ("TPN161HE_CloneData".equalsIgnoreCase(platformId)) {
            ...
            Channel channel = SettingChannelLoader.findChanelXmlFile(new File(processCloneFilePath));
            path = processCloneFilePath + "/" + channel.getFileName();
        }
        File chanelXml = new File(path);
        if (!chanelXml.exists()) {
            return new com.tpvision.smartinstall.xml.channel.v4.TvContents();
        }
        try (BOMInputStream stream = new BOMInputStream(new FileInputStream(chanelXml));
             InputStreamReader freader = new InputStreamReader((InputStream)stream, StandardCharsets.UTF_8);){
            if (chanelXml.exists()) {
                JAXBContext context = JAXBContext.newInstance("com.tpvision.smartinstall.xml.channel.v4");
                Unmarshaller unmarshaller = context.createUnmarshaller();
                ret = (com.tpvision.smartinstall.xml.channel.v4.TvContents)unmarshaller.unmarshal(freader);
            } else {
                ret = new com.tpvision.smartinstall.xml.channel.v4.TvContents();
            }
        }
        catch (Exception e) {
    ...
    public static TvContents loadChannelV6(String processCloneFilePath) throws IOException, JAXBException {
    ...
    private static final Channel findChanelXmlFile(File file) {
        ...
        return SettingChannelLoader.loadChannel(xmlFiles);
    }

    private static Channel loadChannel(List<File> xmlFiles) {
        Channel ret = new Channel();
        for (File f : xmlFiles) {
            if (f.getName().indexOf("CH") <= -1) continue;
