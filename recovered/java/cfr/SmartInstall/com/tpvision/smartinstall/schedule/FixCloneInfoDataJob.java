...
public class FixCloneInfoDataJob
extends Job {
    ...
    public void execute() {
        ...
        LOG.info("get setting list count => {}", (Object)settings.size());
        settings.forEach(this::hanldleSetting);
        LOG.info("end handle clone info data");
    }

    private void hanldleSetting(Setting setting) {
        LOG.info("start to handle setting[{}]", (Object)setting.getId());
        if (!"N".equalsIgnoreCase(setting.getIsdelete())) {
            LOG.warn("is deltelteStatus is not N ,current is {}", (Object)setting.getIsdelete());
            return;
        }
    ...
    private void handleSchedule(Setting setting, String platformId) {
        ...
        if (scheduleId != 0) {
            LOG.info("schedule id is {}, skip handle", (Object)scheduleId);
            return;
        }
        List<Schedule> schedules = JpaManager.getScheduleManager().findByName(setting.getName());
        if (!schedules.isEmpty()) {
            LOG.info("schedule with name:{} already exists", (Object)setting.getName());
            return;
        }
    ...
    private void handleUiCustomization(Setting setting, String platformId) {
        ...
        if (uiCustomizationsId != 0) {
            LOG.info("UiCustomizations id is {}, skip handle", (Object)uiCustomizationsId);
            return;
        }
        ...
        if (!uiCustomizationList.isEmpty()) {
            LOG.info("uiCustomization with name:{} already exists", (Object)setting.getName());
            return;
        }
        UiCustomizations uiCustomizations = UploadCloneUtils.loadConfToUiCustomizationsDb(this.getSettingProcessPath(setting), setting.getName(), platformId);
    ...
    private void handleWelcomeLog(Setting setting, String platformId) {
        ...
        if (welcomeId != 0) {
            LOG.info("welcomeId is {}, skip handle", (Object)welcomeId);
            return;
        }
        List<Welcome> welcomes = JpaManager.getWelcomeManager().findWelcomesByName(setting.getName());
        if (!welcomes.isEmpty()) {
            LOG.info("welcome with name:{} already exists", (Object)setting.getName());
            return;
        }
        String userName = setting.getCreatedBy();
        String configName = setting.getName();
        try {
            welcomeId = WelcomeLogoUtils.getInstance().saveWelcomeToDb(platformId, CommonConstants.CLONE_PROCESS_LOCATION + configName, configName, userName);
            if (welcomeId > 0) {
    ...
    private void handleSettingPackage(Setting setting, String platformId) {
        ...
        if (settingPackageId != 0) {
            LOG.info("setting package id is {}, skip handle", (Object)settingPackageId);
            return;
        }
        ...
        if (!settingPackages.isEmpty()) {
            LOG.info("settingPackage with name:{} already exists", (Object)setting.getName());
            return;
        }
        ...
        SettingPackage settingPackage = UploadCloneUtils.loadConfToSettingPackageDb(this.getSettingProcessPath(setting), setting.getName(), settingChannelBean, platformId);
    ...
    private File getCloneProcessDirectory(Setting setting) {
        ...
        LOG.warn("{} clone file is missing", (Object)file.getAbsolutePath());
        return null;
    }
}
