...
extends TpvTimerTask {
    ...
    public void tryRun() {
        ...
        CmndMetricsTask.writeMetricsLog("sys", sysJson);
        List<Devices> devicesList = JpaManager.getDevicesManager().loadAll();
        for (Devices tv : devicesList) {
            CmndMetricsTask.writeIptvInfoToMetricsLog(tv);
        }
    ...
    public static void writeIptvInfoToMetricsLog(Devices tv) {
        ...
        CmndMetricsTask.writeMetricsLog("ip", tvJson);
    }
    ...
    public static void writeCastAnalyticMetrics(CastAnalyticalData castAnalyticalData) {
        ...
        CmndMetricsTask.writeMetricsLog("cast", castAnalyticJson);
    }
    ...
    private static String getMetricsPmsType() {
        ...
        for (String key : mapPmsType.keySet()) {
            if (!pmsType.equalsIgnoreCase(key)) continue;
    ...
    public static void writeIPTVMetricsLog(Devices tv, JSONObject jsonData) {
        ...
        if ("PMSService".equalsIgnoreCase(jsonData.optString("Fun")) && (jsonBuffer = jsonData.optJSONObject("CommandDetails")) != null && (jsonBuffer = jsonBuffer.optJSONObject("PMSParameters")) != null) {
            String action = jsonBuffer.optString("Action");
            if ("".equalsIgnoreCase(action)) {
                if (!jsonBuffer.isNull("GuestBill")) {
                    CmndMetricsTask.writePMSInfoToMetricsLog(tv, "billrequest");
                }
            } else if ("NewGuestMessage".equalsIgnoreCase(action)) {
                CmndMetricsTask.writePMSInfoToMetricsLog(tv, "message");
            } else if (Arrays.asList("CheckIn", "CheckOut").contains(action)) {
                CmndMetricsTask.writePMSInfoToMetricsLog(tv, action.toLowerCase());
            } else if ("UpdateGuestPreferences".equalsIgnoreCase(action)) {
                CmndMetricsTask.writePMSInfoToMetricsLog(tv, "languagechange");
            }
        }
    }
    ...
    public static void writeRFTVMetricsLog(PmsUtils.PmsAction pmsAction, String roomId) {
        ...
        for (Devices tv : tvs) {
            ...
            CmndMetricsTask.writePMSInfoToMetricsLog(tv, pmsAction.toString().toLowerCase());
        }
    }

    public static void writePMSInfoToMetricsLog(Devices tv, String action) {
        String pmsType = "".equalsIgnoreCase(CmndMetricsTask.getPmsTypeName()) ? CmndMetricsTask.getMetricsPmsType() : CmndMetricsTask.getPmsTypeName();
        ...
        if ("checkout".equalsIgnoreCase(action) && isExpressCheckOut.get().booleanValue()) {
        ...
        CmndMetricsTask.writeMetricsLog("pms", pmsJson);
    }
    ...
    public static void loadRFData() {
        ...
        for (PlayoutInfo info : playoutInfos) {
            ...
            CmndMetricsTask.writeMetricsLog("rf", playoutJson);
        }
    }
    ...
    private void loadServerHardWareInfo(JSONObject sysJson) {
        ...
        String ramInfo = totalMemByte / 0x40000000L + ".0 GB";
        String resolution = UNKNOWN;
        List<Display> displays = hal.getDisplays();
        for (Display display : displays) {
            String[] infoArr;
            for (String line : infoArr = display.toString().split("\n")) {
                int idx = line.indexOf("Active Pixels");
                if (idx == -1) continue;
                resolution = line.substring(idx).replace("Active Pixels", "").trim();
                break;
            }
            if (resolution.equalsIgnoreCase(UNKNOWN)) continue;
            break;
        }
    ...
    private void loadOSInfo(JSONObject sysJson) {
        ...
        if (ProcessUtils.execCommondWithReturn("C:/", "systeminfo", processOutput)) {
            ...
            for (String str : dataArr = value.split("\r\n")) {
                ...
                if (!UNKNOWN.equalsIgnoreCase(osEdition) && !UNKNOWN.equalsIgnoreCase(osVersion) && !UNKNOWN.equalsIgnoreCase(systemModel)) break;
            }
        }
