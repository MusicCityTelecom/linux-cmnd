...
public class VersionChecker {
    ...
    private static VersionDetail latestCmndVersion = null;
    private static VersionDetail latestReceptionVersion = null;
    private static Map<String, VersionDetail> latestFirmwareVersionMap = new HashMap<String, VersionDetail>();
    ...
    public static synchronized void loadLatestVersionInfoFromRemoteJson() {
        ...
        try {
            ...
            ResponseEntity<String> response = template.exchange(LATEST_VERSION_JSON_LOACTION, HttpMethod.GET, null, String.class, new Object[0]);
            jsonBody = (String)response.getBody();
            LOG.info("latest json data==>{}", (Object)jsonBody);
        }
        catch (Exception ex) {
        ...
        VersionDetail cmndVersionDetail = VersionChecker.extractCmndVersion(versionJson);
        if (cmndVersionDetail != null) {
            latestCmndVersion = cmndVersionDetail;
        }
        VersionDetail receptionVersionDetail = VersionChecker.extractReceptionVersion(versionJson);
        if (cmndVersionDetail != null) {
            latestReceptionVersion = receptionVersionDetail;
        }
        VersionChecker.extractLatestFirmwareVersionMap(versionJson);
        LOG.info("extrace version data from json complete");
    }

    public static VersionDetail getCmndNewVersion(String currentVersion) {
        if (latestCmndVersion != null && VersionChecker.isHaveNewerVersion(currentVersion, latestCmndVersion.getVersion())) {
            return latestCmndVersion;
        }
        return null;
    }

    public static VersionDetail getReceptionNewVersion(String currentVersion) {
        if (latestReceptionVersion != null && VersionChecker.isHaveNewerVersion(currentVersion, latestReceptionVersion.getVersion())) {
            return latestReceptionVersion;
        }
        return null;
    }

    public static VersionDetail getFirmwareNewVersion(String platformName, String modelName, String currentVersion) {
        String platformWithoutSpace = TpvStringUtils.removeSpaceFromTvPlatform(platformName);
        VersionDetail versionDetail = latestFirmwareVersionMap.get((platformWithoutSpace + modelName).toLowerCase());
        if (versionDetail == null) {
            versionDetail = latestFirmwareVersionMap.get(platformWithoutSpace.toLowerCase());
        }
        if (versionDetail != null && VersionChecker.isHaveNewerVersion(currentVersion, versionDetail.getVersion())) {
            return versionDetail;
        }
        return null;
    }

    public static VersionDetail getLatestReceptionVersionDirectly() {
        return latestReceptionVersion;
    }
        ...
        try {
            ...
            for (int i = 0; i < latestFirmwareArray.length(); ++i) {
                ...
                if (StringUtils.isBlank(platformName)) {
                    LOG.warn("invalid firmware object: {}", (Object)firmwareObject);
                    continue;
                }
                ...
                if (filterModels != null) {
                    filterModels.forEach(model -> latestFirmwareVersionMap.put((platformName + model).toLowerCase(), versionDetail));
                    continue;
                }
    ...
    private static VersionDetail extractReceptionVersion(JSONObject jsonObject) {
        try {
            ...
            for (int i = 0; i < latestSoftwareArray.length(); ++i) {
                ...
                if (!StringUtils.equalsIgnoreCase("CMND Reception", softwareObject.optString("name"))) continue;
    ...
    private static VersionDetail extractCmndVersion(JSONObject jsonObject) {
        try {
            JSONArray latestSoftwareArray = jsonObject.getJSONArray("latest_softwares");
            for (int i = 0; i < latestSoftwareArray.length(); ++i) {
                JSONObject softwareObject = latestSoftwareArray.getJSONObject(i);
                if (!StringUtils.equalsIgnoreCase("CMND", softwareObject.optString("name"))) continue;
                JSONObject versionDetailObject = softwareObject.getJSONObject("version_details");
                VersionDetail versionDetail = new VersionDetail();
                versionDetail.setVersion(versionDetailObject.getString("version"));
                versionDetail.setUrl(versionDetailObject.optString("url"));
