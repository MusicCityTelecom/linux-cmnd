...
public class IPCloneServiceHandler
extends WebServiceCommandHandler {
    ...
    private static final Map<String, Integer> reSendCounter = new HashMap<String, Integer>();
    ...
    public String execute() {
        ...
        if ("Upgrade".equals(this.device.getCloneMode())) {
            LOG.info("IPCloneService for upgrade,tvPowerStatus={}, tvProgress={}", (Object)this.device.getPowerstatus(), (Object)this.device.getProgress());
        } else if ("Upload".equals(this.device.getCloneMode())) {
            LOG.info("IPCloneService for upload, tvPowerStatus={}, tvProgress={}", (Object)this.device.getPowerstatus(), (Object)this.device.getUploadProgress());
        }
    ...
    private String handleNotInUpgradeModeStatus(Devices tv, JSONObject ipCloneParameters) {
        ...
        if (null != cloneSessionStatus) {
            ...
            LOG.info("response SessionStatus={}", (Object)sessionStatus);
            if ("Upgrade".equalsIgnoreCase(tv.getCloneMode()) && "INP".equalsIgnoreCase(tvProgress)) {
                ...
                if ("Failed".equalsIgnoreCase(sessionStatus) || "Failure".equalsIgnoreCase(sessionStatus) || "".equalsIgnoreCase(sessionStatus)) {
                    ...
                    if (isReSendCloneUrl) {
                        ...
                        this.upgradeFailure(tv, sessionStatus, "Upgrade failed due to insufficient storage space, will not retry! TV ID: " + tv.getId() + ", IP: " + tv.getTvipaddress());
                    } else {
                        ...
                        if (!retryTvUpgradeItem.isEmpty()) return this.reSendClone(tv, sessionStatus);
                        LOG.info("after check IP upgrade version, not need resend");
                        this.upgradeFailure(tv, sessionStatus, "Upgrade failed due to tv response <" + sessionStatus + ">! TV ID: " + tv.getId() + ", IP: " + tv.getTvipaddress());
                    }
                } else if ("PartiallySuccessful".equalsIgnoreCase(sessionStatus) || "Successful".equalsIgnoreCase(sessionStatus) || "Sucessful".equalsIgnoreCase(sessionStatus) || "Not Available".equalsIgnoreCase(sessionStatus)) {
                    ...
                    if (tv.getFirmwareid() != 0) {
                        ...
                        if (tv.getCloneid() == 0) {
                            ...
                            if (retryTvUpgradeItem.isEmpty()) {
                                LOG.info("assign fw + cl tv response shows clone {} is upgrade successfully, finish this upgrade", (Object)tv.getCloneid());
                                this.upgradeSuccess(tv, sessionStatus);
                            } else {
                                LOG.info("assign fw + cl, clone {} not finished, waiting further response...", (Object)tv.getCloneid());
                                boolean isReSendCloneUrl = this.checkReSendCloneUrl(tv.getCloneType(), tv.getCloneid());
                                if (isReSendCloneUrl) {
                                    LOG.info("clone id:{} enter iic mode ,resend clone japit", (Object)tv.getCloneid());
                    ...
                    LOG.info("sessionStatus={},skip handle it", (Object)sessionStatus);
                }
            } else {
    ...
    private String generateReadyForUpgradeJapit(Devices tv) {
        ...
        if ("Upgrade".equalsIgnoreCase(tv.getCloneMode()) && "U".equals(tv.getProgress()) || "INP".equalsIgnoreCase(tv.getProgress())) {
            IPCloneService iPCloneService;
            LOG.info("generate ReadyForUpgrade japit, current tv progress={}  fwId={},cloneId={},type={}", tv.getProgress(), tv.getFirmwareid(), tv.getCloneid(), tv.getType());
            DevicesManager iptvmanager = JpaManager.getDevicesManager();
            ...
            if (!(iPCloneService = IPCloneServiceManager.generateIPCloneService(tv, JAPITUtils.WebServiceType.WebServices, new String[0])).isDownloadUrlEmpty()) {
    ...
    private String handleReadyForUpgrade(Devices tv, String cloneToServerStatus, JSONObject ipCloneParameters, JSONObject cloneToServerParameters) {
        ...
        LOG.info("ReadyForUpgrade cloneMode={} Progress={} PowerState={} cloneToServerStatus={} isReSendCloneUrl={}", tv.getCloneMode(), previousProgress, tv.getPowerstatus(), cloneToServerStatus, isReSendCloneUrl);
        if ("Upgrade".equals(tv.getCloneMode())) {
            ...
            if ("ST".equals(tv.getProgress())) {
                ...
                if (downloadLimiter.isDownloadAllowed(tv)) {
                    downloadLimiter.startUpgrade(tv);
                    retData = this.generateReadyForUpgradeJapit(tv);
                    if (isReSendCloneUrl && "INP".equals(tv.getProgress())) {
                        LOG.info("ReadyForUpgrade=>isReSendCloneUrl={}", (Object)isReSendCloneUrl);
                        retData = this.getReSendUpgradeData(tv, retData);
                    }
                }
            } else if (tv.getFirmwareid() == 0 && "INP".equalsIgnoreCase(previousProgress)) {
    ...
    private boolean checkReSendCloneUrl(String cloneType, Integer cloneId) {
        ...
        if (cloneId != null && cloneId > 0) {
            isReSendCloneUrl = IPCloneServiceHandler.checkScriptReinstallTvCommand(cloneType, cloneId);
        }
    ...
    private static boolean checkScriptReinstallTvCommand(String selectCloneType, int cloneId) {
        ...
        String srcRootPath = CommonConstants.CLONE_PROCESS_LOCATION + set.getName() + File.separator + PlatformUtils.getPlatformId(set.getPlatform()) + File.separator + "MasterCloneData" + File.separator + "Script";
        ...
        if (scriptDire.exists() && scriptDire.isDirectory()) {
            ...
            try {
                ...
                for (Activity activity : activities) {
                    if (!"ReinstallTV".equalsIgnoreCase(activity.getAction())) continue;
        ...
        LOG.info("hasReinstallTvCommand={}", (Object)hasReinstallTvCommand);
        return hasReinstallTvCommand;
    }
    ...
    private Map<String, String> getTvUpgradeResponseVersion(JSONArray tvItemStatus) {
        ...
        if (null != tvItemStatus) {
            ...
            for (int i = 0; i < tvItemCount; ++i) {
                ...
                if ("NotAvailable".equals(cloneStatus)) continue;
                ...
                String cloneItemVersionNo = "Cancelled".equals(cloneStatus) ? "" : cloneItemDetails.optString("CloneItemVersionNo");
    ...
    private List<String> checkIpUpgradeVersion(String type, String tvserialnumber, String siIdentifiers, JSONArray tvItemStatus) {
        ...
        ArrayList<String> retryTvUpgradeItem = new ArrayList<String>();
        ...
        for (String itemName : trCloneUpgradeVersionMap.keySet()) {
            String cloneUpgradeVersionNo;
            if (ignoreCheckList.contains(itemName)) {
                LOG.warn("{} ignore version check", (Object)itemName);
                continue;
            }
            if (!tvUpgradeResponseVersion.containsKey(itemName)) {
                LOG.warn("tv response clone items does not contain valid clone: {}, ignore retry", (Object)itemName);
                continue;
            }
            if ("RoomSpecificSettings".equalsIgnoreCase(itemName)) {
            ...
            if (StringUtils.equalsIgnoreCase(tvItemVersionNo, cloneUpgradeVersionNo = trCloneUpgradeVersionMap.get(itemName))) {
                LOG.info("{} upgrade success ,new version <{}> ", (Object)itemName, (Object)tvItemVersionNo);
                continue;
            }
            LOG.warn("{} upgrade failure assign version:{}, response version:{}", itemName, cloneUpgradeVersionNo, tvItemVersionNo);
            retryTvUpgradeItem.add(itemName);
        }
        LOG.info("[iptv]retryTvUpgradeItem={}", (Object)retryTvUpgradeItem);
    ...
    private String reSendClone(Devices tv, String sessionStatus) {
        ...
        if (reSendCounter.get(tv.getId()) == 0) {
            this.upgradeFailure(tv, sessionStatus, "Display <" + tv.getTvserialnumber() + "> upgrade failed! Reason-(retry " + MAX_RESEND_COUNTER + " times upgrade," + tv.getTvroomid() + " tv response failed)!");
        } else {
            reSendCounter.put(tv.getId(), reSendCounter.get(tv.getId()) - 1);
            LOG.info("NotInUpgradeMode DeviceId={} reSendCounter={} ", (Object)tv.getId(), (Object)reSendCounter.get(tv.getId()));
    ...
    private String autoStandbyUpgradeHandler(Devices tv, JSONObject ipCloneParameters) {
        LOG.info("standby clone upgrade handler-> tv={}  ip:={} ", (Object)this.tvUniqueId, (Object)this.clientIp);
        JSONObject cloneSessionStatus = ipCloneParameters.optJSONObject("CloneSessionStatus");
        if (null != cloneSessionStatus) {
    ...
    private String androidFWUpgradeReadyForUpgrade(JSONObject ipCloneParameters, Devices device) {
        ...
        if ("Failed".equalsIgnoreCase(sessionStatus)) return this.generateReadyForUpgradeJapit(device);
        if ("Failure".equalsIgnoreCase(sessionStatus)) return this.generateReadyForUpgradeJapit(device);
        if ("InProgress".equalsIgnoreCase(sessionStatus)) {
        ...
        if (positionFwId <= -1) {
            LOG.info("fw version check failed,siVersion:<{}>,tv's version:<{}>", (Object)device.getSiFirmwareIdentifier(), (Object)device.getTvFirmwareIdentifier());
            return this.generateReadyForUpgradeJapit(device);
        }
        LOG.info("fw version check success,same version:<{}>", (Object)device.getTvFirmwareIdentifier());
        ...
        if (device.getCloneid() != 0) {
            JSONArray cloneItemStatus = cloneSessionStatus.optJSONArray("CloneItemStatus");
            List<String> retryTvUpgradeItem = this.checkIpUpgradeVersion(device.getType(), device.getTvserialnumber(), device.getSiIdentifiers(), cloneItemStatus);
            if (!retryTvUpgradeItem.isEmpty()) return this.generateReadyForUpgradeJapit(device);
    ...
    private boolean checkRoomSpecialSettingVersion(String type, String tvserialnumber) {
        ...
        block31: {
            ...
            String roomSpecificSetting = CommonConstants.CLONE_ASSEMBLY_LOCATION + platform + File.separator + "RoomSpecificSettings" + File.separator + "RoomSpecificSettings.xml";
            try {
                ...
                if (!roomSettingXml.exists()) break block31;
                try (FileInputStream stream = new FileInputStream(roomSettingXml);
                    ...
                    if (null != roomSettings) {
                        ...
                        if (null != tvserialnumber && TpvStringUtils.limitStringLength(tvserialnumber, 32).equalsIgnoreCase(TpvStringUtils.limitStringLength(roomSpecialSerialNumber, 32))) {
                            isSame = false;
                        } else {
