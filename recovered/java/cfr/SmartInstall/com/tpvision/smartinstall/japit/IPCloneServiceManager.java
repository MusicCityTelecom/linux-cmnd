...
public class IPCloneServiceManager {
    ...
    public static void sendRoomIdToTV(Devices tv) {
        ...
        LOG.info("send roomid <{}> to <{}>", (Object)roomid, (Object)ip);
        ...
        if (PlatformUtils.isSupportTVSettingUpdateRoomId(tv.getType())) {
            ...
            japitJson = IPCloneServiceManager.generateSimpleTvSettingJson(tv, NetworkUtils.getServerIpFromSameRoute(ip), identifier);
        } else {
            SettingCreator.createRoomSpecificSettings(roomid, tv.getTvserialnumber(), ip, tv.getType());
            japitJson = IPCloneServiceManager.generateRoomSpecificSettingsJson(tv, ip);
        }
    ...
    public static void updateFirmWareUpgradeColor(Devices device, String colorChange) {
        ...
        LOG.info("ChangeColor to {} in updateFirmWareUpgradeColor", (Object)colorChange);
    ...
    public static void updateCloneUpgradeColor(Devices device, String colorChange) {
        ...
        LOG.info("ChangeColor to {} in updateCloneUpgradeColor", (Object)colorChange);
        boolean isFinish = "#01DF01".equalsIgnoreCase(colorChange) || "red".equalsIgnoreCase(colorChange) || "black".equalsIgnoreCase(colorChange);
        ...
        if (isFinish) {
            if ("#01DF01".equalsIgnoreCase(colorChange)) {
                ...
                } else if ("Clone".equals(device.getCloneType()) && (setting = JpaManager.getSettingManager().loadByKey(device.getCloneid())) != null && setting.getSettingPackageId() > 0) {
    ...
    public static void setColorForFWClone(Devices device, String colorChange) {
        IPCloneServiceManager.updateFirmWareUpgradeColor(device, colorChange);
        IPCloneServiceManager.updateCloneUpgradeColor(device, colorChange);
    }

    public static void sendIPCloneDataToTV(Devices device) {
        IPCloneService ipCloneService = IPCloneServiceManager.generateIPCloneService(device, JAPITUtils.WebServiceType.WebListeningServices, new String[0]);
        if (ipCloneService.isDownloadUrlEmpty()) {
            device.setProgress("ST");
            device.setStatus("Successful");
            IPCloneServiceManager.setColorForFWClone(device, "#01DF01");
            LOG.warn("download url is empty, skip this upgrade ,update the upgrade status to success directly");
        } else {
            IPCloneServiceManager.setColorForFWClone(device, "#FFBF00");
            ...
            JAPITUtils.sendAsyncCommand(device, ipCloneJson, (bResult, resultJson) -> {
                JSONObject iPCloneParameters;
                boolean isUpdateDeviceProcess = false;
                isUpdateDeviceProcess = bResult ? (iPCloneParameters = resultJson.optJSONObject("IPCloneParameters")) != null && UPRADE_STUATUS_UPGRADE_IN_PROGRESS.equalsIgnoreCase(iPCloneParameters.optString("CurrentUpgradeStatus")) : "1.0".equalsIgnoreCase(ipCloneService.SvcVer) && "japit return error,not a valid json string".equalsIgnoreCase(resultJson.getString("errMsg"));
                LOG.info("japit send result is <{}>, and update device process is <{}>", (Object)bResult, (Object)isUpdateDeviceProcess);
    ...
    public static void sendDLCommandToTV(Devices tv, String progress) throws Exception {
        String data = IPCloneServiceManager.generateBaseRequestIPCloneService(tv);
        ...
        IPCloneServiceManager.sendDownloadCloneJson(cloneToServerParameters, tv, progress);
    }

    private static String generateBaseRequestIPCloneService(Devices tv) {
        return "{  \"Svc\" : \"WebListeningServices\",  \"SvcVer\": \"" + PlatformUtils.getIpCloneServiceVer(tv) + "\",  \"Cookie\": " + JAPITUtils.getJapitRandomCookieValue() + ",  \"CmdType\" : \"Request\",  \"Fun\" : \"IPCloneService\"}";
    }

    private static String generateCancelCloneJapit(Devices tv) {
        return "{\t\"Svc\": \"WebListeningServices\",\t\"SvcVer\": \"" + PlatformUtils.getIpCloneServiceVer(tv) + "\",\t\"Cookie\": " + JAPITUtils.getJapitRandomCookieValue() + ",\t\"CmdType\": \"Change\",\t\"Fun\": \"IPCloneService\",\t\"CommandDetails\": {\t\t\"CloneAction\": \"Cancel\",\t\t\"WebListeningServiceParameters\": {\t\t\t\"TVUniqueID\": \"" + tv.getTvuniqueid() + "\"\t\t}\t}}";
    }
    ...
    public static String changeIPCloneService(String tvUniqueId) {
        ...
        return "{  \"Svc\": \"WebServices\",  \"SvcVer\": \"1.0\",  \"Cookie\": 293,  \"CmdType\": \"Change\",  \"Fun\": \"IPCloneService\",  \"CommandDetails\": {    \"WebServiceParameters\": {      \"PollingFrequency\": " + pollingFrequency + ",      \"PollingFrequencyGreen\": " + pollingFrequencyGreen + ",      \"TVUniqueID\": \"" + tvUniqueId + "\"    },    \"IPCloneParameters\": {},    \"CloneToServerParameters\": {}  }}";
    }
    ...
    private static void sendDownloadCloneJson(JSONObject cloneToServerParameters, Devices tv, String progress) {
        ...
        LOG.info("[sendDLCommandToTV]type={},tvUniqueID:{}", (Object)tvType, (Object)tvUniqueId);
        String downloadJson = IPCloneServiceManager.generateDownloadClonePackageJson(tv, "WebListeningServices", cloneItemsAvailableToServer);
        ...
        if ((jsonObject = new JSONObject(downloadResultJSON)).get("Fun").toString().equalsIgnoreCase("Error")) {
    ...
    public static void refreshTVCloneItemVersionToDb(JSONObject commandDetails, Devices tv) {
        ...
        if (null != ipCloneParameters) {
            ...
            if (cloneSessionStatus != null) {
                ...
                JSONObject cloneToServerSessionStatus;
                JSONObject cloneToServerParameters = commandDetails.optJSONObject("CloneToServerParameters");
                if (null != cloneToServerParameters && null != (cloneToServerSessionStatus = (JSONObject)cloneToServerParameters.get("CloneToServerSessionStatus"))) {
    ...
    public static IPCloneService generateIPCloneService(Devices tv, JAPITUtils.WebServiceType webServiceType, String ... ignoreUpgradeItems) {
        ...
        List supportCloneItems = PlatformUtils.getIpUpgradeItems(tv.getType()).stream().filter(e -> !ignoreList.contains(e)).collect(Collectors.toList());
        String serverIp = NetworkUtils.getServerIpFromSameRoute(tv.getTvipaddress());
        String baseServerPath = "";
        baseServerPath = HttpUtils.isSupportHttpsByUniqueId(tv.getTvuniqueid()) ? String.format(Locale.ENGLISH, "https://%s:%d/SmartInstall", serverIp, CommonConstants.CMND_HTTPS_PORT) : String.format(Locale.ENGLISH, "http://%s:%d/SmartInstall", serverIp, CommonConstants.CMND_HTTP_PORT);
        if (tv.getCloneid() > 0) {
            ...
            for (File cloneItem : cloneItems) {
                ...
                if (!validCloneItemExt.contains(cloneFileExt)) {
                    LOG.warn("cloneItemName:{} has a invalid ext {} ,skip upgrade", (Object)cloneFileName, (Object)cloneFileExt);
                    continue;
                }
                ...
                if (!supportCloneItems.contains(cloneItemName)) {
                    LOG.warn("cloneItemName:{} not in supported list ,skip upgrade", (Object)cloneFileName);
                    continue;
                }
                if (isCheckSiAssignItem && !assignCloneItems.containsKey(cloneItemName)) {
                    LOG.warn("cloneItemName:{} not in assign clone list, skip upgrade", (Object)cloneFileName);
                    continue;
                }
                String siCloneIdentifier = is2K14MS ? TpvDateUtils.getCurrentIndentifierFormatTime() : IPCloneServiceManager.getCloneItemIdentifier(identifiers, cloneItemName);
                ...
                if (!"Script".equalsIgnoreCase(itemBaseName) && siCloneIdentifier.equalsIgnoreCase(tvCloneIdentifer)) {
                    LOG.warn("cloneItemName:{}, tv identifier is same as CMND identifier <{}>,skip upgrade", (Object)cloneItemName, (Object)siCloneIdentifier);
                    continue;
                }
        ...
        if (tv.getFirmwareid() > 0 && supportCloneItems.contains("MainFirmware")) {
            ...
            if (firmwareFile.exists()) {
                if (isNeedCheckLowerVersion && PlatformUtils.compareLowerVersion(tv.getSiFirmwareIdentifier(), tv.getTvFirmwareIdentifier())) {
                    LOG.warn("platformId:{}, tv identifier<{}> is higer than CMND identifier <{}>,skip upgrade", platformId, tv.getTvFirmwareIdentifier(), tv.getSiFirmwareIdentifier());
                } else {
                    IPCloneService.CloneItemDownloadDetail cloneItemDownloadDetail = new IPCloneService.CloneItemDownloadDetail();
                    cloneItemDownloadDetail.CloneItemDetails.CloneItemName = "MainFirmware";
                    cloneItemDownloadDetail.CloneItemDetails.CloneItemVersionNo = IPCloneServiceManager.getFirmwareIdentifier(firmwareId);
    ...
    private static String getCloneItemIdentifier(SiIdentifiers identifiers, String cloneItem) {
        for (SiIdentifiers.CloneItem cloneItem1 : identifiers.SiAssignItem) {
            if (!cloneItem1.CloneItemName.equalsIgnoreCase(cloneItem)) continue;
    ...
    private static String generateDownloadClonePackageJson(Devices tv, String svc, JSONArray cloneItemsAvailableToServer) {
        JSONObject webServiceParameters;
        ...
        if ("WebServices".equals(svc)) {
            webServiceParameters = new JSONObject();
        ...
        cloneToServerParameters.put("CloneToServerDetails", IPCloneServiceManager.getCloneToServerDetails(uri, cloneItemsAvailableToServer));
    ...
    private static JSONArray getCloneToServerDetails(String uri, JSONArray cloneItemsAvailable) {
        HashSet<String> cloneItems = new HashSet<String>();
        for (int i = 0; i < cloneItemsAvailable.length(); ++i) {
            String cloneItemName;
            JSONObject item = (JSONObject)cloneItemsAvailable.get(i);
            if (!item.has("CloneItemName") || !StringUtils.isNotBlank(cloneItemName = item.optString("CloneItemName"))) continue;
            cloneItems.add(cloneItemName);
        }
        JSONArray cloneToServerDetails = new JSONArray();
        String[] allCloneItems = new String[]{"TVSettings", "TVChannelList", "WelcomeLogo", "SmartInfoImages", "SmartInfoPages", "AndroidApps", "RoomSpecificSettings", "DataDump", "CustomDashboardFallback", "Script", "MediaChannels", "WeatherForecast", "HTVCfg.xml", "Banner", "PMS", "AndroidAppsData", "ProfessionalApps", "ProfessionalAppsData", "Schedules", "MyChoice", "Vsecure"};
        int allCloneItemsLen = allCloneItems.length;
        for (int i = 0; i < allCloneItemsLen; ++i) {
            if (!cloneItems.contains(allCloneItems[i])) continue;
    ...
    private static String generateRoomSpecificSettingsJson(Devices tv, String clientip) {
        ...
        String clidentifier = IPCloneServiceManager.getCloneIdentifier(CommonConstants.servletContextPath + clPath);
        ...
        return "{ \"Svc\" : \"WebListeningServices\", \"SvcVer\": \"" + PlatformUtils.getIpCloneServiceVer(tv) + "\",\"Cookie\": " + JAPITUtils.getJapitRandomCookieValue() + ", \"CmdType\": \"Change\",\"Fun\":\"IPCloneService\", \"CommandDetails\" : { \"WebListeningServiceParameters\" : { \"TVUniqueID\" : \"" + tv.getTvuniqueid() + "\"},\"IPCloneParameters\" :{\"CloneItemDownloadDetails\": [{\"CloneItemDetails\" : { \"CloneItemName\" : \"RoomSpecificSettings\",\"CloneItemVersionNo\" : \"" + clidentifier + "\"},\"URL\" : \"" + strHttp + serverip + ":" + serverPort + "/SmartInstall/Profile/Clone/" + clientip.replace(".", "") + "/RoomSpecificSettings.zip\"}]}}}";
    }
    ...
    public static String generateSimpleTvSettingJson(Devices tv, String cmndIP, String identifier) {
        ...
        String strHttp = HttpUtils.getHttpString(isSupportHttps);
        int serverPort = HttpUtils.getCMNDPort(isSupportHttps);
        return "{ \"Svc\" : \"WebListeningServices\", \"SvcVer\": \"" + PlatformUtils.getIpCloneServiceVer(tv) + "\",\"Cookie\": " + JAPITUtils.getJapitRandomCookieValue() + ", \"CmdType\": \"Change\",\"Fun\":\"IPCloneService\", \"CommandDetails\" : { \"WebListeningServiceParameters\" : { \"TVUniqueID\" : \"" + tv.getTvuniqueid() + "\"},\"IPCloneParameters\" :{\"CloneItemDownloadDetails\": [{\"CloneItemDetails\" : { \"CloneItemName\" : \"TVSettings\",\"CloneItemVersionNo\" : \"" + identifier + "\"},\"URL\" : \"" + strHttp + cmndIP + ":" + serverPort + "/SmartInstall/Profile/Clone/" + tv.getTvipaddress().replace(".", "") + "/TVSettings.zip\"}]}}}";
    }

    public static void cancelIpCloneServiceForTV(Devices tv) {
        try {
            String data = IPCloneServiceManager.generateCancelCloneJapit(tv);
    ...
    public static boolean refreshTVCloneStatusAndCheckIsForceUpgradeReady(Devices tv) {
        ...
        try {
            ...
            if (jsonObject.has("Fun") && jsonObject.get("Fun").toString().equalsIgnoreCase("IPCloneService")) {
                ...
                IPCloneServiceManager.refreshTVCloneItemVersionToDb(commandDetails, tv);
                if (PlatformUtils.isSupportUpgradeStatusCheckBeforeForceUpgrade(tv.getType())) {
                    ...
                    result = UPRADE_STUATUS_READY_FOR_UPGRADE.equalsIgnoreCase(currentUpgradeStatus);
                }
            }
        }
        catch (Exception e) {
