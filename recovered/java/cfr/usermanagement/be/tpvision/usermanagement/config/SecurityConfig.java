...
import be.tpvision.usermanagement.config.UserManagementSavedRequestAwareAuthenticationSuccessHandler;
...
extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity httpSecurity) throws Exception {
        ((HttpSecurity)((HttpSecurity)((ExpressionUrlAuthorizationConfigurer.AuthorizedUrl)((HttpSecurity)((HttpSecurity)((HttpSecurity)httpSecurity.exceptionHandling().authenticationEntryPoint(this.casAuthenticationEntryPoint()).and()).addFilter(this.casAuthenticationFilter()).addFilterBefore((Filter)this.singleSignOutFilter(), CasAuthenticationFilter.class)).addFilterBefore((Filter)this.requestCasGlobalLogoutFilter(), LogoutFilter.class)).authorizeRequests().anyRequest()).authenticated().and()).logout().logoutUrl("/logout").permitAll().and()).csrf().disable();
    }
