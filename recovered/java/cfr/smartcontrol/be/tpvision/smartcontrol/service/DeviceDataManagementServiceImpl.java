...
@Service
public class DeviceDataManagementServiceImpl
implements DeviceDataManagementService {
    ...
    @Autowired
    public DeviceDataManagementServiceImpl(DeviceService deviceService, MiscellaneousService miscellaneousService, GeneralService generalService, InputSourcesService inputSourcesService, SystemService systemService) {
        Assert.notNull((Object)deviceService, ConstructorMessages.DEVICE_SERVICE_CAN_NOT_BE_NULL);
        this.deviceService = deviceService;
        Assert.notNull((Object)miscellaneousService, ConstructorMessages.MISCELLANEOUS_SERVICE_CAN_NOT_BE_NULL);
    ...
    private boolean shouldUpdate(Device device) {
        ...
        try {
            ...
            if (newSerialCode != null) {
                ...
                boolean bl = hasSerialCodeChanged = !Objects.equals(oldSerialCode, newSerialCode);
                if (isOldSerialCodeNull || !hasSerialCodeChanged) {
                    return true;
                }
            }
        }
        catch (CommandNotAcknowledgedException | CommandNotAvailableException e) {
    ...
    private void fetchInfoData(Set<DeviceListItem> deviceListItemSet) {
        for (DeviceListItem deviceListItem : deviceListItemSet) {
            if (!(deviceListItem instanceof Device)) continue;
    ...
    private void fetchFirmwareVersionAndroid(Device device) {
        ...
        catch (CommandNotAcknowledgedException | CommandNotAvailableException | UnsupportedOperationException e) {
            String message = e.getMessage();
            logger.info(message);
        }
    }
    ...
    private void clearOverviewData(Device device) {
        Assert.notNull((Object)device, "Device can not be null.");
