...
import be.tpvision.smartcontrol.messages.codecs.sicp188.input_sources.failovers.ToDomainMessages;
import be.tpvision.smartcontrol.messages.codecs.sicp188.input_sources.failovers.ToProtocolMessages;
...
public class FailoversCodec
extends Codec<Failovers> {
    ...
    private static final Map<Failover, Byte> domainFailovers = new EnumMap<Failover, Byte>(Failover.class);
    private static final Map<Byte, Failover> protocolFailovers;
    ...
    @Override
    public byte[] toProtocol(Failovers failovers) {
        Assert.notNull((Object)failovers, ToProtocolMessages.FAILOVERS_CAN_NOT_BE_NULL);
        ArrayList<Failover> domainFailoverList = failovers.getFailoverList();
        ...
        Byte[] protocolFailoverArray = (Byte[])domainFailoverList.stream().map(domainFailovers::get).toArray(Byte[]::new);
    ...
    @Override
    public Failovers toDomain(byte[] bytes) {
        ...
        ArrayList domainFailoverList = Arrays.stream(protocolFailoverArray).map(protocolFailovers::get).collect(Collectors.toCollection(ArrayList::new));
        boolean isAnyDomainFailoverNull = domainFailoverList.stream().anyMatch(Objects::isNull);
    ...
    static {
        ...
        protocolFailovers = MapUtilities.inverse(domainFailovers);
    }
}
