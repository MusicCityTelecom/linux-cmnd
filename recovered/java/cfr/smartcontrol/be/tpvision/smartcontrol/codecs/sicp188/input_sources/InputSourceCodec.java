...
import be.tpvision.smartcontrol.messages.codecs.sicp188.input_sources.input_source.ToDomainMessages;
import be.tpvision.smartcontrol.messages.codecs.sicp188.input_sources.input_source.ToProtocolMessages;
...
extends Codec<InputSource> {
    ...
    public byte[] toProtocol(InputSource inputSource) {
        Assert.notNull((Object)inputSource, ToProtocolMessages.INPUT_SOURCE_CAN_NOT_BE_NULL);
        InputSource.SourceType domainSourceType = inputSource.getSourceType();
        Assert.state(domainSourceType != null, ToProtocolMessages.DOMAIN_SOURCE_TYPE_CAN_NOT_BE_NULL);
        Byte protocolSourceType = domainSourceTypes.get((Object)domainSourceType);
        Assert.state(protocolSourceType != null, ToProtocolMessages.PROTOCOL_SOURCE_TYPE_CAN_NOT_BE_NULL);
        InputSource.SourceLabel domainSourceLabel = inputSource.getSourceLabel();
        Assert.state(domainSourceLabel != null, ToProtocolMessages.DOMAIN_SOURCE_LABEL_CAN_NOT_BE_NULL);
        Byte protocolSourceLabel = domainSourceLabels.get((Object)domainSourceLabel);
        Assert.state(protocolSourceLabel != null, ToProtocolMessages.PROTOCOL_SOURCE_LABEL_CAN_NOT_BE_NULL);
        byte reservedByte = 0;
        return new byte[]{protocolSourceType, protocolSourceType, protocolSourceLabel, reservedByte};
    }
    ...
    public InputSource toDomain(byte[] bytes) {
        ...
        Assert.state(protocolSourceType != null, ToDomainMessages.PROTOCOL_SOURCE_TYPE_CAN_NOT_BE_NULL);
        InputSource.SourceType domainSourceType = protocolSourceTypes.get(protocolSourceType);
        Assert.state(protocolSourceType != null, ToDomainMessages.DOMAIN_SOURCE_TYPE_CAN_NOT_BE_NULL);
        Byte protocolSourceLabel = bytes[2];
        Assert.state(protocolSourceType != null, ToDomainMessages.PROTOCOL_SOURCE_LABEL_CAN_NOT_BE_NULL);
        InputSource.SourceLabel domainSourceLabel = protocolSourceLabels.get(protocolSourceLabel);
        Assert.state(protocolSourceType != null, ToDomainMessages.DOMAIN_SOURCE_LABEL_CAN_NOT_BE_NULL);
