...
public class LedStripsCodec
extends Codec<LedStrips> {
    static final byte STATUS_OFF_BYTE = 0;
    static final byte STATUS_ON_BYTE = 1;
    private static final Map<LedStrips.Status, Byte> domainStatusses = new EnumMap<LedStrips.Status, Byte>(LedStrips.Status.class);
    private static final Map<Byte, LedStrips.Status> protocolStatusses;
    ...
    @Override
    public byte[] toProtocol(LedStrips ledStrips) {
        Assert.notNull((Object)ledStrips, ToProtocolMessages.LED_STRIPS_CAN_NOT_BE_NULL_MESSAGE);
        ...
        Byte protocolStatus = domainStatusses.get((Object)domainStatus);
    ...
    @Override
    public LedStrips toDomain(byte[] bytes) {
        if (bytes == null || bytes.length < 4) {
            return null;
        }
        byte protocolStatus = bytes[0];
        LedStrips.Status domainStatus = protocolStatusses.get(protocolStatus);
        Assert.notNull((Object)domainStatus, ToDomainMessages.STATUS_CAN_NOT_BE_NULL_MESSAGE);
    ...
    static {
        ...
        protocolStatusses = MapUtilities.inverse(domainStatusses);
    }
}
