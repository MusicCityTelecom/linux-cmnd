...
public abstract class NettyCommandSender
implements CommandSender<IpDestination> {
    private static final Logger logger = LoggerFactory.getLogger(NettyCommandSender.class);
    private final EventLoopGroup eventLoopGroup = new NioEventLoopGroup();
    private final Bootstrap bootstrap;

    public NettyCommandSender(SicpInitializer sicpInitializer) {
        this.bootstrap = (Bootstrap)((Bootstrap)((Bootstrap)new Bootstrap().group(this.eventLoopGroup)).channel(NioSocketChannel.class)).handler(sicpInitializer);
    }
    ...
    @Override
    public Response send(Request command, IpDestination destination) {
        ...
        if (command != null && destination != null) {
            ...
            try {
                ...
                logger.info("sending command {} to {}", (Object)command, (Object)destination);
                response = sicpHandler.send(command);
                channel.closeFuture().sync();
                logger.info("response object {}", (Object)response);
                return response;
            }
    ...
    @Override
    public <C> C send(Request command, IpDestination destination, Class<C> responseClass) {
