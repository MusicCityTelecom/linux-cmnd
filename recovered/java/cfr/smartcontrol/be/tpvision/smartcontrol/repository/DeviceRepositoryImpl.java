...
extends AbstractRepository<Long, Device>
implements DeviceRepository {
    private final DeviceUtilities deviceUtilities;

    @Autowired
    public DeviceRepositoryImpl(DeviceUtilities deviceUtilities) {
        super(Device.class);
        this.deviceUtilities = deviceUtilities;
    }

    @Override
    public SingularAttribute<? super Device, ?> getSingularAttribute(be.tpvision.smartcontrol.repository.order_fields.OrderField<? super Device> orderField) {
        Assert.notNull(orderField, GetSingularAttributeMessages.ORDER_FIELD_CAN_NOT_BE_NULL);
        OrderField deviceOrderField = (OrderField)orderField;
    ...
    @Override
    public List<Device> getAllOrderedBy(be.tpvision.smartcontrol.repository.order_fields.OrderField<? super Device> orderField, OrderDirection orderDirection) {
        List<Device> deviceList = super.getAll();
        return this.deviceUtilities.orderDevices(deviceList, orderField, orderDirection);
    }

    @Override
    public Device getByIpDestination(IpDestination ipDestination) {
        Assert.notNull((Object)ipDestination, GetByIpDestinationMessages.IP_DESTINATION_CAN_NOT_BE_NULL);
    ...
    @Override
    public Device getBySerialCode(String serialCode) {
        Assert.notNull((Object)serialCode, GetBySerialCodeMessages.SERIAL_CODE_CAN_NOT_BE_NULL);
