...
@Repository
public class DeviceRepositoryJdbcImpl
implements DeviceRepositoryJdbc {
    private final JdbcTemplate jdbcTemplate;
    private final RowMapper<DeviceDTO> deviceRowMapper;

    @Autowired
    public DeviceRepositoryJdbcImpl(JdbcTemplate jdbcTemplate, RowMapper<DeviceDTO> deviceRowMapper) {
    ...
    public DeviceDTO getDeviceDTO(long id) {
        ...
        try {
            return this.jdbcTemplate.queryForObject("SELECT id, address, control_id, group_id, serial_code, ftp_settings_use_default, ftp_settings_port, ftp_settings_username, ftp_settings_password FROM devices WHERE id = ?", this.deviceRowMapper, id);
        }
    ...
    public boolean getContentRotatedBySerialCode(String serialCode) {
        ...
        try {
            return this.jdbcTemplate.queryForObject("SELECT content_rotated FROM devices WHERE serial_code = ?", Boolean.TYPE, serialCode);
        }
        catch (EmptyResultDataAccessException emptyResultDataAccessException) {
