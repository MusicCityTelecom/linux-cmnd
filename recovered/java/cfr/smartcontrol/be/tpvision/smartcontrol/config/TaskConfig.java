...
implements SchedulingConfigurer {
    ...
    @Value(value="${smartcms.content.temp.clean-up.enabled}")
    private boolean tempContentCleanUpEnabled;
    @Value(value="${smartcms.content.temp.clean-up.interval.minutes}")
    private int tempContentCleanUpIntervalMinutes;
    @Value(value="${smartcontrol.synchronization.enabled}")
    private boolean dataSynchronisationEnabled;
    @Value(value="${smartcontrol.synchronization.interval.minutes}")
    private int dataSynchronisationIntervalMinutes;
    @Value(value="${smartcontrol.content.update.enabled}")
    private boolean contentUpdateEnabled;
    @Value(value="${smartcontrol.content.update.interval.minutes}")
    ...
    private static <T> T executeTask(FutureTask<T> futureTask, long timeout, TimeUnit timeUnit) {
        ...
        Assert.notNull((Object)timeUnit, ExecuteTaskMessages.TIME_UNIT_CAN_NOT_BE_NULL);
    ...
    public void configureTasks(ScheduledTaskRegistrar taskRegistrar) {
        ...
        if (this.tempContentCleanUpEnabled) {
            intervalMilliseconds = TimeUnit.MINUTES.toMillis(this.tempContentCleanUpIntervalMinutes);
        ...
        if (this.dataSynchronisationEnabled) {
            intervalMilliseconds = TimeUnit.MINUTES.toMillis(this.dataSynchronisationIntervalMinutes);
            ...
            Runnable overviewRunnable = () -> {
                ...
                TaskConfig.executeTask(futureTask, timeout, TimeUnit.MILLISECONDS);
            };
        ...
        if (this.contentUpdateEnabled) {
            intervalMilliseconds = TimeUnit.MINUTES.toMillis(this.contentUpdateIntervalMinutes);
