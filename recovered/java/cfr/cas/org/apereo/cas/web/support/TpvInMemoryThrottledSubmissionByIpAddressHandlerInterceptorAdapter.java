...
implements AsyncHandlerInterceptor {
    ...
    public final boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object o) throws Exception {
        if (this.isLoginSubmitRequest(request) && this.exceedsThreshold(request)) {
            ...
            String username = StringUtils.isNotBlank(this.usernameParameter) ? StringUtils.defaultString(request.getParameter(this.usernameParameter), "N/A") : "N/A";
    ...
    private void recordThrottle(HttpServletRequest request) {
        this.logger.warn("Throttling submission from {}. Authentication attempt exceeds the failure threshold {}, the lock interval is {} minutes", request.getRemoteAddr(), this.errorMaxCount, this.lockMinutes);
    }
