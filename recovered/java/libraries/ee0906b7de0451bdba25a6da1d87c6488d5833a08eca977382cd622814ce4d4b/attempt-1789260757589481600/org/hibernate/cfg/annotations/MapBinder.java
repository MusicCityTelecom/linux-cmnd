...
extends CollectionBinder {
    ...
    protected Value createFormulatedValue(Value value, Collection collection, String targetPropertyName, PersistentClass associatedClass, PersistentClass targetPropertyPersistentClass, MetadataBuildingContext buildingContext) {
        ...
        if (!(element instanceof OneToMany)) {
            ...
            fromAndWhere = this.getFromAndWhereFormula(associatedClass.getTable().getQualifiedTableName().toString(), element.getColumnIterator(), referencedEntityColumns);
        } else if (!associatedClass.equals(targetPropertyPersistentClass)) {
            fromAndWhere = this.getFromAndWhereFormula(targetPropertyPersistentClass.getTable().getQualifiedTableName().toString(), element.getColumnIterator(), associatedClass.getIdentifier().getColumnIterator());
        }
