...
implements ResourceConnector {
    private static final ClassLoader CL_STUB = GroovyScriptEngine.doPrivileged(() -> new ClassLoader(){});
    ...
    private GroovyClassLoader initGroovyLoader() {
        GroovyClassLoader groovyClassLoader = GroovyScriptEngine.doPrivileged(() -> {
            if (this.parentLoader instanceof GroovyClassLoader) {
                return new ScriptClassLoader((GroovyClassLoader)this.parentLoader);
            }
            return new ScriptClassLoader(this.parentLoader, this.config);
        });
    ...
    protected boolean isSourceNewer(ScriptCacheEntry entry) {
        ...
        for (String scriptName : entry.dependencies) {
            ...
            try {
                lastMod = this.getLastModified(scriptName);
            }
    ...
    extends GroovyClassLoader {
        ...
        private void setResLoader() {
            GroovyResourceLoader rl = this.getResourceLoader();
            this.setResourceLoader(className -> {
        ...
        protected CompilationUnit createCompilationUnit(CompilerConfiguration configuration, CodeSource source) {
            ...
            local.cu = cu;
            StringSetMap cache = local.dependencyCache;
            ...
            cache.clear();
            cu.addPhaseOperation((sourceUnit, context, classNode) -> {
        ...
        public Class parseClass(GroovyCodeSource codeSource, boolean shouldCacheSource) throws CompilationFailedException {
            StampedCommonCache stampedCommonCache = this.sourceCache;
            synchronized (stampedCommonCache) {
