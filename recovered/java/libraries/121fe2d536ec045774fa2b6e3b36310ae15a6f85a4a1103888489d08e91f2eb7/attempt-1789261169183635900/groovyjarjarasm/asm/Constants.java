...
final class Constants {
    ...
    static boolean isWhitelisted(String internalName) {
        if (!internalName.startsWith("groovyjarjarasm/asm/")) {
            return false;
        }
        String member = "(Annotation|Class|Field|Method|Module|RecordComponent|Signature)";
        return internalName.contains("Test$") || Pattern.matches("groovyjarjarasm/asm/util/Trace" + member + "Visitor(\\$.*)?", internalName) || Pattern.matches("groovyjarjarasm/asm/util/Check" + member + "Adapter(\\$.*)?", internalName);
    }
