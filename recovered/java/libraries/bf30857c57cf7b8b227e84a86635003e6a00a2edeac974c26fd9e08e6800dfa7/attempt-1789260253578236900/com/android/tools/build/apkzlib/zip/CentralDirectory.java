...
class CentralDirectory {
    ...
    CentralDirectory(ZFile file) {
        this.file = file;
        this.bytesSupplier = new CachedSupplier<byte[]>(this::computeByteRepresentation);
        this.verifyLog = file.getVerifyLog();
    }

    static CentralDirectory makeFromData(ByteBuffer bytes, int count, ZFile file, ByteStorage storage) throws IOException {
        ...
        for (int i2 = 0; i2 < count; ++i2) {
            try {
                ...
                throw new IOException("Failed to read directory entry index " + i2 + " (total directory bytes read: " + bytes.position() + ").", e2);
            }
        }
        return directory;
    }
    ...
    static CentralDirectory makeFromEntries(Set<StoredEntry> entries, ZFile file) {
        ...
        for (StoredEntry entry : entries) {
            ...
            Preconditions.checkArgument(!directory.entries.containsKey(cdr.getName()), "Duplicate filename");
    ...
    private void readEntry(ByteBuffer bytes, ByteStorage storage) throws IOException {
        ...
        this.verifyLog.verify(versionNeededToExtract <= 20L, "Ignored unknown version needed to extract in zip directory entry: %s.", versionNeededToExtract);
        ...
        this.verifyLog.verify(method != null, "Unknown method in zip directory entry: %s.", methodCode);
        ...
        F_DISK_NUMBER_START.verify(bytes, this.verifyLog);
        long internalAttributes = F_INTERNAL_ATTRIBUTES.read(bytes);
        this.verifyLog.verify((internalAttributes & 0xFFFFFFFFFFFFFFFEL) == 0L, "Ignored invalid internal attributes: %s.", internalAttributes);
        long externalAttributes = F_EXTERNAL_ATTRIBUTES.read(bytes);
        long entryOffset = F_OFFSET.read(bytes);
        long remainingSize = (long)fileNameLength + (long)extraFieldLength + (long)fileCommentLength;
        ...
        ListenableFuture<CentralDirectoryHeaderCompressInfo> compressInfo = Futures.immediateFuture(new CentralDirectoryHeaderCompressInfo(method, compressedSize, versionNeededToExtract));
        CentralDirectoryHeader centralDirectoryHeader = new CentralDirectoryHeader(fileName, encodedFileName, uncompressedSize, compressInfo, flags, this.file);
        ...
        try {
            entry = new StoredEntry(centralDirectoryHeader, this.file, null, storage);
        }
        catch (IOException e2) {
            throw new IOException("Failed to read stored entry '" + fileName + "'.", e2);
        }
        if (this.entries.containsKey(fileName)) {
            this.verifyLog.log("File file contains duplicate file '" + fileName + "'.");
        }
    ...
    private byte[] computeByteRepresentation() {
        ArrayList<StoredEntry> sorted2 = Lists.newArrayList(this.entries.values());
        Collections.sort(sorted2, StoredEntry.COMPARE_BY_NAME);
        CentralDirectoryHeader[] cdhs = new CentralDirectoryHeader[this.entries.size()];
        CentralDirectoryHeaderCompressInfo[] compressInfos = new CentralDirectoryHeaderCompressInfo[this.entries.size()];
        byte[][] encodedFileNames = new byte[this.entries.size()][];
        byte[][] extraFields = new byte[this.entries.size()][];
        byte[][] comments = new byte[this.entries.size()][];
        try {
            int idx = 0;
            int total = 0;
            for (StoredEntry entry : sorted2) {
