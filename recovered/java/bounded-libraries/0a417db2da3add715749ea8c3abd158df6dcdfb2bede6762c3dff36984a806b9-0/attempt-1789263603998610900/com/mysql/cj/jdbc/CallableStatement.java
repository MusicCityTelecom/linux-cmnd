...
implements java.sql.CallableStatement {
                ...
                if (this.paramInfo.numParameters > 0) {
                    ...
                    while (paramIter.hasNext()) {
                        ...
                        try (StatementImpl setPstmt = null;){
                            ...
                            ((ClientPreparedStatement)setPstmt).getQueryBindings().setFromBindValue(0, ((PreparedQuery)this.query).getQueryBindings().getBindValues()[inParamInfo.index]);
    ...
    protected boolean checkReadOnlySafeStatement() throws SQLException {
        if (QueryInfo.isReadOnlySafeQuery(((PreparedQuery)this.query).getOriginalSql(), this.session.getServerSession().isNoBackslashEscapesSet())) {
            ...
            if (StringUtils.startsWithIgnoreCaseAndWs(sql, "CALL", statementKeywordPos = QueryInfo.indexOfStatementKeyword(sql, this.session.getServerSession().isNoBackslashEscapesSet())) || StringUtils.startsWithIgnoreCaseAndWs(sql, "SELECT", statementKeywordPos)) {
