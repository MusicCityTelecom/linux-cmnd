    ...
    public byte[] getBytes(BindValue binding) {
        switch (binding.getMysqlType()) {
            ...
            case LONGBLOB: {
                ...
                if (this.isEscapeNeededForString(x, stringLength)) {
                    String escString = StringUtils.escapeString(new StringBuilder((int)((double)x.length() * 1.1)), x, this.serverSession.useAnsiQuotedIdentifiers(), this.charsetEncoder).toString();
        ...
        switch (binding.getMysqlType()) {
            ...
            case BIT: {
                ...
                if ("true".equalsIgnoreCase(x) || "Y".equalsIgnoreCase(x)) {
                    ...
                    throw ExceptionFactory.createException(WrongArgumentException.class, Messages.getString("PreparedStatement.66", new Object[]{x}), this.exceptionInterceptor);
                }
                return String.valueOf(b != false ? 1 : 0);
            }
            case TINYINT: 
            ...
            case FLOAT: 
            case FLOAT_UNSIGNED: {
                return StringUtils.fixDecimalExponent(Float.toString(Float.valueOf(x).floatValue()));
            }
            case DOUBLE: 
    ...
    public void encodeAsBinary(Message msg, BindValue binding) {
        ...
        switch (binding.getMysqlType()) {
            ...
            case BIT: {
                ...
                intoPacket.writeInteger(NativeConstants.IntegerDataType.INT1, b != false ? 1L : 0L);
                return;
            }
            case TINYINT: 
            case TINYINT_UNSIGNED: {
            ...
            case FLOAT: 
            case FLOAT_UNSIGNED: {
                intoPacket.writeInteger(NativeConstants.IntegerDataType.INT4, Float.floatToIntBits(Float.valueOf(x).floatValue()));
                return;
            }
            case DOUBLE: 
            case DOUBLE_UNSIGNED: {
