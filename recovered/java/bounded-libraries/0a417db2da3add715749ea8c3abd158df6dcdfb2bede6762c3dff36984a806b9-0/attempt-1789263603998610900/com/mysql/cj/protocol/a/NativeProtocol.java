...
import com.mysql.cj.protocol.a.ClobValueEncoder;
...
import com.mysql.cj.protocol.a.InstantValueEncoder;
...
import com.mysql.cj.protocol.a.OffsetTimeValueEncoder;
...
RuntimeProperty.RuntimePropertyListener {
    ...
    public Supplier<ValueEncoder> getValueEncoderSupplier(Object obj) {
        Optional<Supplier> mysqlType;
        if (obj == null) {
            return ByteArrayValueEncoder::new;
        }
        Supplier res = DEFAULT_ENCODERS.get(obj.getClass());
        if (res == null && (mysqlType = DEFAULT_ENCODERS.entrySet().stream().filter(m -> ((Class)m.getKey()).isAssignableFrom(obj.getClass())).map(m -> (Supplier)m.getValue()).findFirst()).isPresent()) {
            res = mysqlType.get();
        }
        return res;
    }
    ...
    static {
        ...
        DEFAULT_ENCODERS.put(java.sql.Date.class, SqlDateValueEncoder::new);
