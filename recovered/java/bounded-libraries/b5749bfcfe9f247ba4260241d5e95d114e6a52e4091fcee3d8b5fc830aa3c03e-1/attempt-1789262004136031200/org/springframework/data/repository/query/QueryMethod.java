...
public class QueryMethod {
    ...
    public QueryMethod(Method method, RepositoryMetadata metadata, ProjectionFactory factory) {
        ...
        Parameters.TYPES.stream().filter(type -> org.springframework.data.repository.util.ClassUtils.getNumberOfOccurrences(method, type) > 1).findFirst().ifPresent(type -> {
        ...
        if (this.isPageQuery()) {
            Assert.isTrue((boolean)this.parameters.hasPageableParameter(), (String)String.format("Paging query needs to have a Pageable parameter; Offending method: %s", method));
        }
