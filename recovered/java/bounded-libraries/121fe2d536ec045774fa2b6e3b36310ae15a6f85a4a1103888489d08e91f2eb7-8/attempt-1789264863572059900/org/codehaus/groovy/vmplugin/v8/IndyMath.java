...
public class IndyMath {
    ...
    private static final Map<String, Map<MethodType, MethodHandle>> METHODS = new HashMap<String, Map<MethodType, MethodHandle>>();
    ...
    private static MethodType widenOperators(MethodType mt) {
        ...
        if (parameterCount == 2) {
            ...
            if (TypeHelper.isIntCategory(leftType) && TypeHelper.isIntCategory(rightType)) {
                return IIV;
            }
            if (TypeHelper.isLongCategory(leftType) && TypeHelper.isLongCategory(rightType)) {
                return LLV;
            }
            if (TypeHelper.isBigDecCategory(leftType) && TypeHelper.isBigDecCategory(rightType)) {
                return GGV;
            }
            if (TypeHelper.isDoubleCategory(leftType) && TypeHelper.isDoubleCategory(rightType)) {
                return DDV;
            }
            return OOV;
        }
        if (parameterCount == 1) {
            TypeDescriptor.OfField leftType = mt.parameterType(0);
            if (TypeHelper.isIntCategory(leftType)) {
                return IV;
            }
            if (TypeHelper.isLongCategory(leftType)) {
                return LV;
            }
            if (TypeHelper.isBigDecCategory(leftType)) {
                return GV;
            }
            if (TypeHelper.isDoubleCategory(leftType)) {
                return DV;
            }
        }
        return mt;
