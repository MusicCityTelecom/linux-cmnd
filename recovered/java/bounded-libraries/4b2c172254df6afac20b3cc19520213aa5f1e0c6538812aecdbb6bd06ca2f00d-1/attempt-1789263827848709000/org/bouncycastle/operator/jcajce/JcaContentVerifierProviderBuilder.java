            ...
            public ContentVerifier get(AlgorithmIdentifier algorithmIdentifier) throws OperatorCreationException {
                ...
                if (publicKey instanceof CompositePublicKey) {
                    ...
                    for (int i = 0; i != list.size(); ++i) {
                        try {
                            Signature signature = JcaContentVerifierProviderBuilder.this.createSignature(algorithmIdentifier, (PublicKey)list.get(i));
                            Signature signature2 = JcaContentVerifierProviderBuilder.this.createRawSig(algorithmIdentifier, (PublicKey)list.get(i));
                            if (signature2 != null) {
                                return new RawSigVerifier(algorithmIdentifier, signature, signature2);
                            }
                            return new SigVerifier(algorithmIdentifier, signature);
    ...
    private ContentVerifier createCompositeVerifier(AlgorithmIdentifier algorithmIdentifier, PublicKey publicKey) throws OperatorCreationException {
        if (publicKey instanceof CompositePublicKey) {
            ...
            for (int i = 0; i != aSN1Sequence.size(); ++i) {
                AlgorithmIdentifier algorithmIdentifier2 = AlgorithmIdentifier.getInstance((Object)aSN1Sequence.getObjectAt(i));
                signatureArray[i] = list.get(i) != null ? this.createSignature(algorithmIdentifier2, (PublicKey)list.get(i)) : null;
            }
        ...
        Signature[] signatureArray = new Signature[aSN1Sequence.size()];
        for (int i = 0; i != aSN1Sequence.size(); ++i) {
            AlgorithmIdentifier algorithmIdentifier3 = AlgorithmIdentifier.getInstance((Object)aSN1Sequence.getObjectAt(i));
            try {
    ...
    implements ContentVerifier {
        ...
        public CompositeVerifier(Signature[] signatureArray) throws OperatorCreationException {
            ...
            for (int i = n + 1; i != signatureArray.length; ++i) {
                ...
                this.stream = new TeeOutputStream(this.stream, OutputStreamFactory.createStream((Signature)signatureArray[i]));
            }
        }
        ...
        public boolean verify(byte[] byArray) {
            try {
                ...
                boolean bl = false;
                for (int i = 0; i != aSN1Sequence.size(); ++i) {
                    if (this.sigs[i] == null || this.sigs[i].verify(DERBitString.getInstance((Object)aSN1Sequence.getObjectAt(i)).getBytes())) continue;
                    bl = true;
                }
                return !bl;
            }
