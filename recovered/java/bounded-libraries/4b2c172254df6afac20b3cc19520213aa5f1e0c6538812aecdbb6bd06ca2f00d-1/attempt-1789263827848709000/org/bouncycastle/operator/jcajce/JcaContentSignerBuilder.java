...
public class JcaContentSignerBuilder {
    ...
    public JcaContentSignerBuilder(String string, AlgorithmParameterSpec algorithmParameterSpec) {
        ...
        if (algorithmParameterSpec instanceof PSSParameterSpec) {
            ...
            this.sigAlgId = new AlgorithmIdentifier(MiscObjectIdentifiers.id_alg_composite, (ASN1Encodable)JcaContentSignerBuilder.createCompParams(compositeAlgorithmSpec));
        } else {
    ...
    private static ASN1Sequence createCompParams(CompositeAlgorithmSpec compositeAlgorithmSpec) {
        ...
        ASN1EncodableVector aSN1EncodableVector = new ASN1EncodableVector();
        List list = compositeAlgorithmSpec.getAlgorithmNames();
        List list2 = compositeAlgorithmSpec.getParameterSpecs();
        for (int i = 0; i != list.size(); ++i) {
            AlgorithmParameterSpec algorithmParameterSpec = (AlgorithmParameterSpec)list2.get(i);
            if (algorithmParameterSpec == null) {
                aSN1EncodableVector.add((ASN1Encodable)defaultSignatureAlgorithmIdentifierFinder.find((String)list.get(i)));
                continue;
            }
            if (algorithmParameterSpec instanceof PSSParameterSpec) {
                aSN1EncodableVector.add((ASN1Encodable)JcaContentSignerBuilder.createPSSParams((PSSParameterSpec)algorithmParameterSpec));
                continue;
            }
