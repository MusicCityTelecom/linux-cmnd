    ...
    public OutputEncryptor build(final char[] cArray) throws OperatorCreationException {
        ...
        try {
            ...
            if (this.isPKCS12(this.algorithm)) {
                ...
                cipher.init(1, (Key)new PKCS12KeyWithParameters(cArray, byArray, this.iterationCount));
                algorithmIdentifier = new AlgorithmIdentifier(this.algorithm, (ASN1Encodable)new PKCS12PBEParams(byArray, this.iterationCount));
            } else if (this.algorithm.equals((ASN1Primitive)PKCSObjectIdentifiers.id_PBES2)) {
                PBKDFConfig pBKDFConfig;
                Object object = pBKDFConfig = this.pbkdf == null ? this.pbkdfBuilder.build() : this.pbkdf;
                if (MiscObjectIdentifiers.id_scrypt.equals((ASN1Primitive)pBKDFConfig.getAlgorithm())) {
                    ...
                    SecretKey secretKey = secretKeyFactory.generateSecret((KeySpec)new ScryptKeySpec(cArray, byArray, scryptConfig.getCostParameter(), scryptConfig.getBlockSize(), scryptConfig.getParallelizationParameter(), this.keySizeProvider.getKeySize(new AlgorithmIdentifier(this.keyEncAlgorithm))));
                    ...
                    PBES2Parameters pBES2Parameters = algorithmParameters != null ? new PBES2Parameters(new KeyDerivationFunc(MiscObjectIdentifiers.id_scrypt, (ASN1Encodable)scryptParams), new EncryptionScheme(this.keyEncAlgorithm, (ASN1Encodable)ASN1Primitive.fromByteArray((byte[])cipher.getParameters().getEncoded()))) : new PBES2Parameters(new KeyDerivationFunc(MiscObjectIdentifiers.id_scrypt, (ASN1Encodable)scryptParams), new EncryptionScheme(this.keyEncAlgorithm));
                    ...
                    cipher.init(1, (Key)this.simplifyPbeKey(secretKey), this.random);
                    AlgorithmParameters algorithmParameters = cipher.getParameters();
                    PBES2Parameters pBES2Parameters = algorithmParameters != null ? new PBES2Parameters(new KeyDerivationFunc(PKCSObjectIdentifiers.id_PBKDF2, (ASN1Encodable)new PBKDF2Params(byArray, pBKDF2Config.getIterationCount(), pBKDF2Config.getPRF())), new EncryptionScheme(this.keyEncAlgorithm, (ASN1Encodable)ASN1Primitive.fromByteArray((byte[])cipher.getParameters().getEncoded()))) : new PBES2Parameters(new KeyDerivationFunc(PKCSObjectIdentifiers.id_PBKDF2, (ASN1Encodable)new PBKDF2Params(byArray, pBKDF2Config.getIterationCount(), pBKDF2Config.getPRF())), new EncryptionScheme(this.keyEncAlgorithm));
                    algorithmIdentifier = new AlgorithmIdentifier(this.algorithm, (ASN1Encodable)pBES2Parameters);
                }
            } else {
