...
extends AbstractSimpleMessageHandlerFactoryBean<AggregatingMessageHandler> {
    ...
    protected AggregatingMessageHandler createHandler() {
        MessageGroupProcessor outputProcessor = this.processorBean instanceof MessageGroupProcessor ? (MessageGroupProcessor)this.processorBean : (!StringUtils.hasText((String)this.methodName) ? new MethodInvokingMessageGroupProcessor(this.processorBean) : new MethodInvokingMessageGroupProcessor(this.processorBean, this.methodName));
