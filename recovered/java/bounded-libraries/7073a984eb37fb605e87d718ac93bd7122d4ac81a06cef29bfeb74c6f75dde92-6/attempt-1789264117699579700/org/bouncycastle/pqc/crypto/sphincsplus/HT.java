...
class HT {
    ...
    SIG_XMSS xmss_sign(byte[] byArray, byte[] byArray2, int n, byte[] byArray3, ADRS aDRS) {
        ...
        for (int i = 0; i < this.engine.H_PRIME; ++i) {
            int n2 = n / (1 << i) ^ 1;
            byArrayArray[i] = this.treehash(byArray2, n2 * (1 << i), i, byArray3, aDRS);
        }
        aDRS = new ADRS(aDRS);
        aDRS.setType(0);
        aDRS.setKeyPairAddress(n);
        byte[] byArray4 = this.wots.sign(byArray, byArray2, byArray3, aDRS);
    ...
    byte[] treehash(byte[] byArray, int n, int n2, byte[] byArray2, ADRS aDRS) {
        ...
        for (int i = 0; i < 1 << n2; ++i) {
            ...
            linkedList.add(0, new NodeEntry(byArray3, aDRS2.getTreeHeight()));
        }
