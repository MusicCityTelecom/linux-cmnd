...
implements CompilationUnitAware {
    ...
    implements BuilderStrategy {
        protected static List<PropertyInfo> getPropertyInfoFromClassNode(ClassNode cNode, List<String> includes, List<String> excludes) {
            return AbstractBuilderStrategy.getPropertyInfoFromClassNode(cNode, includes, excludes, false);
        }
        ...
        protected static List<PropertyInfo> getPropertyInfoFromClassNode(ClassNode cNode, List<String> includes, List<String> excludes, boolean allNames) {
            ...
            for (FieldNode fNode : GeneralUtils.getInstancePropertyFields(cNode)) {
                if (AbstractASTTransformation.shouldSkip(fNode.getName(), excludes, includes, allNames)) continue;
        ...
        protected static List<PropertyInfo> getPropertyInfoFromBeanInfo(ClassNode cNode, List<String> includes, List<String> excludes, boolean allNames) {
            ...
            try {
                ...
                for (PropertyDescriptor descriptor : beanInfo.getPropertyDescriptors()) {
                    if (AbstractASTTransformation.shouldSkipUndefinedAware(descriptor.getName(), excludes, includes, allNames) || descriptor.isHidden() || descriptor.getWriteMethod() == null) continue;
        ...
        protected boolean getIncludeExclude(BuilderASTTransformation transform, AnnotationNode anno, ClassNode cNode, List<String> excludes, List<String> includes) {
            ...
            List<String> directExcludes = BuilderASTTransformation.getMemberStringList(anno, "excludes");
            if (directExcludes != null) {
                excludes.addAll(directExcludes);
            }
            if ((directIncludes = BuilderASTTransformation.getMemberStringList(anno, "includes")) != null) {
                includes.clear();
                includes.addAll(directIncludes);
            }
            if (directIncludes == null && excludes.isEmpty() && transform.hasAnnotation(cNode, TupleConstructorASTTransformation.MY_TYPE)) {
                AnnotationNode tupleConstructor = cNode.getAnnotations(TupleConstructorASTTransformation.MY_TYPE).get(0);
                if (excludes.isEmpty()) {
                    List<String> tupleExcludes = BuilderASTTransformation.getMemberStringList(tupleConstructor, "excludes");
                    if (tupleExcludes != null) {
                        excludes.addAll(tupleExcludes);
                    }
                }
                if (includes.isEmpty()) {
                    List<String> tupleIncludes = BuilderASTTransformation.getMemberStringList(tupleConstructor, "includes");
            ...
            List<String> includesToCheck = includes.size() == 1 && Undefined.isUndefined(includes.get(0)) ? null : includes;
        ...
        protected List<FieldNode> getFields(BuilderASTTransformation transform, AnnotationNode anno, ClassNode buildee) {
            ...
            return includeSuperProperties ? GeneralUtils.getSuperPropertyFields(buildee) : GeneralUtils.getInstancePropertyFields(buildee);
        }
        ...
        protected List<PropertyInfo> getPropertyInfoFromClassNode(BuilderASTTransformation transform, AnnotationNode anno, ClassNode cNode, List<String> includes, List<String> excludes, boolean allNames, boolean allProperties) {
            ...
            for (PropertyNode pNode : BeanUtils.getAllProperties(cNode, false, false, allProperties)) {
                if (AbstractASTTransformation.shouldSkip(pNode.getName(), excludes, includes, allNames)) continue;
                props.add(new PropertyInfo(pNode.getName(), pNode.getType()));
                seen.add(pNode.getName());
            }
            for (FieldNode fNode : this.getFields(transform, anno, cNode)) {
                if (seen.contains(fNode.getName()) || AbstractASTTransformation.shouldSkip(fNode.getName(), excludes, includes, allNames)) continue;
                props.add(new PropertyInfo(fNode.getName(), fNode.getType()));
            }
            return props;
        }
        ...
        protected List<PropertyInfo> getPropertyInfos(BuilderASTTransformation transform, AnnotationNode anno, ClassNode buildee, List<String> excludes, List<String> includes, boolean allNames, boolean allProperties) {
            if (buildee.getModule() == null) {
                return AbstractBuilderStrategy.getPropertyInfoFromBeanInfo(buildee, includes, excludes, allNames);
            }
            return this.getPropertyInfoFromClassNode(transform, anno, buildee, includes, excludes, allNames, allProperties);
        }
