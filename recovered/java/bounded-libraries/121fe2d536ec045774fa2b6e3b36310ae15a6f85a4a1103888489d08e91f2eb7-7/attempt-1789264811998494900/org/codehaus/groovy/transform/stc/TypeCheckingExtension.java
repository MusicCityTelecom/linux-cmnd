...
public class TypeCheckingExtension {
    ...
    public ClassNode parameterizedType(ClassNode baseType, ClassNode ... genericsTypeArguments) {
        if (baseType.isArray()) {
            return this.parameterizedType(baseType.getComponentType(), genericsTypeArguments).makeArray();
        }
        ...
        if (generics != null) {
            ...
            result.setGenericsTypes((GenericsType[])Arrays.stream(genericsTypeArguments).map(StaticTypeCheckingVisitor::wrapTypeIfNecessary).map(GenericsType::new).toArray(GenericsType[]::new));
        }
        return result;
    }
