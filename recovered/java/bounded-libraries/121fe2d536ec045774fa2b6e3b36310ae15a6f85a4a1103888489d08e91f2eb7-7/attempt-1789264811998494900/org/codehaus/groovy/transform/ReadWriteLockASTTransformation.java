...
extends AbstractASTTransformation {
    ...
    public void visit(ASTNode[] nodes, SourceUnit source) {
        ...
        if (parent instanceof MethodNode) {
            ...
            FieldNode lockExpr = this.determineLock(value, cNode, mNode.isStatic(), myTypeName);
            if (lockExpr == null) {
                return;
            }
            MethodCallExpression lockType = GeneralUtils.callX(GeneralUtils.varX(lockExpr), isWriteLock ? "writeLock" : "readLock");
            ...
            mNode.setCode(GeneralUtils.block(GeneralUtils.stmt(acquireLock), GeneralUtils.tryCatchS(originalCode, GeneralUtils.stmt(releaseLock))));
        }
    }
    ...
    private FieldNode determineLock(String value, ClassNode targetClass, boolean isStatic, String myTypeName) {
        ...
        if (isStatic) {
            ...
            if (field == null) {
                int visibility = 26;
                field = targetClass.addField(DEFAULT_STATIC_LOCKNAME, visibility, LOCK_TYPE, ReadWriteLockASTTransformation.createLockObject());
        ...
        if (field == null) {
            int visibility = 18;
            field = targetClass.addField(DEFAULT_INSTANCE_LOCKNAME, visibility, LOCK_TYPE, ReadWriteLockASTTransformation.createLockObject());
        } else if (field.isStatic()) {
