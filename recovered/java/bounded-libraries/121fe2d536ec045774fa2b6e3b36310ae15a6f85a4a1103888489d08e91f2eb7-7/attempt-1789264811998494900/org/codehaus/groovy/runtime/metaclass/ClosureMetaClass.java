...
extends MetaClassImpl {
    ...
    private MetaMethod getDelegateMethod(Closure closure, Object delegate, final String methodName, Class[] argClasses) {
    ...
    public Object invokeMethod(Class sender, Object object, String methodName, Object[] originalArguments, boolean isCallToSuper, boolean fromInsideClass) {
        ...
        boolean bl = shouldDefer = resolveStrategy == 3 && ClosureMetaClass.isInternalMethod(methodName);
        ...
        Object value = null;
        try {
            value = this.getProperty(sender, object, methodName, isCallToSuper, fromInsideClass);
        }
        ...
        throw last != null ? last : new MissingMethodException(methodName, this.theClass, arguments, false);
    }
    ...
    private void assignMethodChooser() {
        if (this.closureMethods.size() == 1) {
            MetaMethod doCall = (MetaMethod)this.closureMethods.get(0);
            CachedClass[] c = doCall.getParameterTypes();
            int length = c.length;
            if (length == 0) {
                this.chooser = (arguments, coerce) -> {
    ...
    public MetaMethod pickMethod(String methodName, Class[] argumentTypes) {
        if (CLOSURE_CALL_METHOD.equals(methodName) || CLOSURE_DO_CALL_METHOD.equals(methodName)) {
            return this.pickClosureMethod(argumentTypes != null ? argumentTypes : MetaClassHelper.EMPTY_CLASS_ARRAY);
        }
