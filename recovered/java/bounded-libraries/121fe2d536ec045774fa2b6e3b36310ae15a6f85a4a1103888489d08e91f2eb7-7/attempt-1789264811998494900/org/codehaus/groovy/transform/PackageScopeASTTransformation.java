...
extends AbstractASTTransformation {
    ...
    public void visit(ASTNode[] nodes, SourceUnit source) {
        ...
        if (parent instanceof ClassNode) {
            List<PackageScopeTarget> targets = value == null ? Collections.singletonList(legacyMode ? PackageScopeTarget.FIELDS : PackageScopeTarget.CLASS) : PackageScopeASTTransformation.determineTargets(value);
    ...
    private void visitClassNode(ClassNode cNode, List<PackageScopeTarget> value) {
        ...
        if (value.contains((Object)PackageScopeTarget.CLASS)) {
            if (cNode.isSyntheticPublic()) {
                PackageScopeASTTransformation.revertVisibility(cNode);
            } else {
        ...
        if (value.contains((Object)PackageScopeTarget.FIELDS)) {
            ...
            for (FieldNode fNode : fList) {
                ...
                PackageScopeASTTransformation.revertVisibility(fNode);
            }
        }
    }
    ...
    private void visitFieldNode(FieldNode fNode) {
        ...
        for (PropertyNode pNode : pList) {
            if (!pNode.getName().equals(fName) || pNode.getAnnotations(MY_TYPE) == null) continue;
            foundProp = pNode;
            break;
        }
        if (foundProp != null) {
            ...
            foundProp = null;
            for (PropertyNode pNode : pList) {
                if (!pNode.getName().equals(fName)) continue;
                foundProp = pNode;
                break;
            }
            if (foundProp != null) {
                ...
                if (foundProp.hasInitialExpression()) {
                    if (fNode.hasInitialExpression()) {
                        this.addError("The split property definition named '" + fName + "' must not have an initial value for both the field and the property", fNode);
                    }
    ...
    private static List<PackageScopeTarget> determineTargets(Expression expr) {
        ...
        if (expr instanceof PropertyExpression) {
            list.add(PackageScopeASTTransformation.extractTarget((PropertyExpression)expr));
        } else if (expr instanceof ListExpression) {
            ListExpression expressionList = (ListExpression)expr;
            List<Expression> expressions = expressionList.getExpressions();
            for (Expression ex : expressions) {
                if (!(ex instanceof PropertyExpression)) continue;
                list.add(PackageScopeASTTransformation.extractTarget((PropertyExpression)ex));
            }
        }
        return list;
    }
