...
import org.apache.groovy.util.concurrent.concurrentlinkedhashmap.LinkedDeque;
...
Serializable {
    ...
    V compute(K key, Function<? super K, ? extends V> mappingFunction, boolean onlyIfAbsent) {
        WeightedValue oldWeightedValue;
        int weight;
        Node prior;
        ConcurrentLinkedHashMap.checkNotNull(key);
        ConcurrentLinkedHashMap.checkNotNull(mappingFunction);
        ObjectHolder objectHolder = new ObjectHolder();
        block0: while (true) {
            Function<Object, Node> f = k -> {
            ...
            prior = this.data.computeIfAbsent(key, f);
            Node node = (Node)objectHolder.getObject();
            if (null == node) {
                Object value = prior.getValue();
                weight = this.weigher.weightOf(key, value);
                WeightedValue weightedValue = new WeightedValue(value, weight);
