...
extends InvocationWriter {
    ...
    private static final Handle BSM = new Handle(6, INDY_INTERFACE_NAME, "bootstrap", BSM_METHOD_TYPE_DESCRIPTOR, false);
    ...
    protected void writeGetProperty(Expression receiver, String propertyName, boolean safe, boolean implicitThis, boolean groovyObject) {
        ...
        this.finishIndyCall(BSM, IndyInterface.CallType.GET.getCallSiteName(), sig, 1, propertyName, flags);
    }
    ...
    private void writeIndyCast(ClassNode sourceType, ClassNode targetType) {
        String sig = "(" + BytecodeHelper.getTypeDescription(sourceType) + ')' + BytecodeHelper.getTypeDescription(targetType);
        this.controller.getMethodVisitor().visitInvokeDynamicInsn(IndyInterface.CallType.CAST.getCallSiteName(), sig, BSM, "()", 0);
