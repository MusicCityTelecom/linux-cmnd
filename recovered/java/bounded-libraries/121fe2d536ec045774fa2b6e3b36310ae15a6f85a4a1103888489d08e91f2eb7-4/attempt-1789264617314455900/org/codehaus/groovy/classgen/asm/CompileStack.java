...
public class CompileStack {
    ...
    private Map<String, BytecodeVariable> stackVariables = new HashMap<String, BytecodeVariable>();
    private int currentVariableIndex = 1;
    private int nextVariableIndex = 1;
    private final Deque<BytecodeVariable> temporaryVariables = new LinkedList<BytecodeVariable>();
    private final Deque<BytecodeVariable> usedVariables = new LinkedList<BytecodeVariable>();
    ...
    private final List<BlockRecorder> visitedBlocks = new LinkedList<BlockRecorder>();
    private final Deque<StateStackElement> stateStack = new LinkedList<StateStackElement>();
    private final Deque<Boolean> implicitThisStack = new LinkedList<Boolean>();
    private final Deque<Boolean> lhsStack = new LinkedList<Boolean>();
    private String className;
    private int localVariableOffset;
    private Label thisStartLabel;
    private Label thisEndLabel;
    private final Map<String, Label> namedLoopBreakLabel;
    private final Map<String, Label> namedLoopContinueLabel;
    private final Deque<ExceptionTableEntry> typedExceptions;
    private final Deque<ExceptionTableEntry> untypedExceptions;
    private boolean lhs;
    ...
    public CompileStack(WriterController controller) {
        this.implicitThisStack.add(Boolean.FALSE);
        this.lhsStack.add(Boolean.FALSE);
        this.namedLoopBreakLabel = new HashMap<String, Label>();
        this.namedLoopContinueLabel = new HashMap<String, Label>();
    ...
    public void pushState() {
        ...
        this.stackVariables = new HashMap<String, BytecodeVariable>(this.stackVariables);
    ...
    private void popState() {
        ...
        StateStackElement element = this.stateStack.removeLast();
    ...
    public void removeVar(int tempIndex) {
        BytecodeVariable head = this.temporaryVariables.removeFirst();
    ...
    public BytecodeVariable getVariable(String variableName, boolean mustExist) {
        ...
        BytecodeVariable v = this.stackVariables.get(variableName);
        if (v == null && mustExist) {
    ...
    public int defineTemporaryVariable(String name, boolean store) {
        return this.defineTemporaryVariable(name, ClassHelper.dynamicType(), store);
    }
    ...
    public void init(VariableScope scope, Parameter[] parameters) {
        ...
        this.clear = false;
        this.pushVariableScope(scope);
        this.defineMethodVariables(parameters, scope.isInStaticContext());
    ...
    public void pushVariableScope(VariableScope scope) {
        this.pushState();
        this.scope = scope;
        this.superBlockNamedLabels = new HashMap<String, Label>(this.superBlockNamedLabels);
        this.superBlockNamedLabels.putAll(this.currentBlockNamedLabels);
        this.currentBlockNamedLabels = new HashMap<String, Label>();
    }
    ...
    private void defineMethodVariables(Parameter[] params, boolean isInStaticContext) {
        Label startLabel;
        this.thisStartLabel = startLabel = new Label();
        this.controller.getMethodVisitor().visitLabel(startLabel);
        this.makeLocalVariablesOffset(params, isInStaticContext);
        for (Parameter param : params) {
            ...
            if (param.isClosureSharedVariable()) {
                ...
                answer = this.defineVar(name, param.getOriginType(), true, useExistingReference);
                ...
                if (!useExistingReference) {
                    ...
                    BytecodeVariable var = new BytecodeVariable(this.currentVariableIndex, param.getOriginType(), name, this.currentVariableIndex);
    ...
    public BytecodeVariable defineVariable(Variable v, ClassNode variableType, boolean initFromStack) {
        ...
        BytecodeVariable before = this.stackVariables.put(name, answer);
        ...
        if (!initFromStack) {
            if (ClassHelper.isPrimitiveType(v.getOriginType()) && ClassHelper.getWrapper(v.getOriginType()).equals(variableType)) {
                ...
                CompileStack.pushInitValue(type.redirect(), mv);
            }
        }
    ...
    private void makeNextVariableID(ClassNode type, boolean useReferenceDirectly) {
        ...
        if ((ClassHelper.isPrimitiveLong(type) || ClassHelper.isPrimitiveDouble(type)) && !useReferenceDirectly) {
    ...
    public void applyFinallyBlocks(Label label, boolean isBreakLabel) {
        ...
        Iterator<StateStackElement> iter = this.stateStack.descendingIterator();
        while (iter.hasNext()) {
            StateStackElement element = iter.next();
            if (element.currentBlockNamedLabels.containsValue(label)) continue;
        ...
        LinkedList<BlockRecorder> blockRecorders = new LinkedList<BlockRecorder>(this.finallyBlocks);
        if (result != null) {
            blockRecorders.removeAll(result.finallyBlocks);
        }
        this.applyBlockRecorder(blockRecorders);
    }

    private void applyBlockRecorder(Collection<BlockRecorder> blockRecorders) {
        if (blockRecorders.isEmpty() || blockRecorders.size() == this.visitedBlocks.size()) {
            return;
        }
        MethodVisitor mv = this.controller.getMethodVisitor();
        Label start = new Label();
        for (BlockRecorder recorder : blockRecorders) {
            if (this.visitedBlocks.contains(recorder)) continue;
            Label end = new Label();
            mv.visitInsn(0);
            mv.visitLabel(end);
