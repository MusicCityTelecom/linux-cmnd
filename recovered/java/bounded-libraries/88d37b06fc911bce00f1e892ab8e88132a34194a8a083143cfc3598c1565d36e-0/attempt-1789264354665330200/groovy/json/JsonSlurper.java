...
public class JsonSlurper {
    ...
    public Object parse(Path path, String charset) throws IOException {
        return this.parse(Files.newInputStream(path, new OpenOption[0]), charset);
    }
    ...
    public Object parse(@NamedParams(value={@NamedParam(value="connectTimeout", type=Integer.class), @NamedParam(value="readTimeout", type=Integer.class), @NamedParam(value="useCaches", type=Boolean.class), @NamedParam(value="allowUserInteraction", type=Boolean.class), @NamedParam(value="requestProperties", type=Map.class)}) Map<String, ?> params, URL url) {
        return this.parseURL(url, params);
    }

    private Object parseURL(URL url, @NamedParams(value={@NamedParam(value="connectTimeout", type=Integer.class), @NamedParam(value="readTimeout", type=Integer.class), @NamedParam(value="useCaches", type=Boolean.class), @NamedParam(value="allowUserInteraction", type=Boolean.class), @NamedParam(value="requestProperties", type=Map.class)}) Map<String, ?> params) {
        BufferedReader reader = null;
        try {
            reader = params == null || params.isEmpty() ? ResourceGroovyMethods.newReader((URL)url) : ResourceGroovyMethods.newReader((URL)url, params);
            ...
            if (reader != null) {
                DefaultGroovyMethodsSupport.closeWithWarning((Closeable)reader);
            }
        }
    }
    ...
    private Object parseURL(URL url, Map params, String charset) {
        BufferedReader reader = null;
        try {
            reader = params == null || params.isEmpty() ? ResourceGroovyMethods.newReader((URL)url, (String)charset) : ResourceGroovyMethods.newReader((URL)url, (Map)params, (String)charset);
