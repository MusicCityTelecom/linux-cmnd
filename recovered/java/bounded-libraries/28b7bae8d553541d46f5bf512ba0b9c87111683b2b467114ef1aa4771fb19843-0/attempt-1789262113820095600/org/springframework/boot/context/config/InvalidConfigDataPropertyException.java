...
extends ConfigDataException {
    ...
    InvalidConfigDataPropertyException(ConfigurationProperty property, boolean profileSpecific, ConfigurationPropertyName replacement, ConfigDataResource location) {
        super(InvalidConfigDataPropertyException.getMessage(property, profileSpecific, replacement, location), null);
    ...
    static void throwOrWarn(Log logger, ConfigDataEnvironmentContributor contributor) {
        ...
        if (propertySource != null) {
            WARNINGS.forEach((name, replacement) -> {
                ConfigurationProperty property = propertySource.getConfigurationProperty((ConfigurationPropertyName)name);
                if (property != null) {
                    logger.warn((Object)InvalidConfigDataPropertyException.getMessage(property, false, replacement, contributor.getResource()));
                }
            });
    ...
    static {
        LinkedHashMap<ConfigurationPropertyName, ConfigurationPropertyName> warnings = new LinkedHashMap<ConfigurationPropertyName, ConfigurationPropertyName>();
        ...
        LinkedHashSet<ConfigurationPropertyName> errors = new LinkedHashSet<ConfigurationPropertyName>();
        ...
        errors.add(ConfigurationPropertyName.of("spring.profiles.default"));
