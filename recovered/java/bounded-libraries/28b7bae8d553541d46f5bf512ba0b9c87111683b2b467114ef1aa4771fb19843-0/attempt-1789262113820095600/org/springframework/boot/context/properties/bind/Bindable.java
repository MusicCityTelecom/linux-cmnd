...
public final class Bindable<T> {
    ...
    public Bindable<T> withAnnotations(Annotation ... annotations) {
        return new Bindable<T>(this.type, this.boxedType, this.value, annotations != null ? annotations : NO_ANNOTATIONS, NO_BIND_RESTRICTIONS);
    }

    public Bindable<T> withExistingValue(T existingValue) {
        Assert.isTrue((existingValue == null || this.type.isArray() || this.boxedType.resolve().isInstance(existingValue) ? 1 : 0) != 0, () -> "ExistingValue must be an instance of " + this.type);
        Supplier<Object> value = existingValue != null ? () -> existingValue : null;
        return new Bindable<Object>(this.type, this.boxedType, value, this.annotations, this.bindRestrictions);
    }
    ...
    public static <T> Bindable<T> of(ResolvableType type) {
        ...
        return new Bindable<T>(type, boxedType, null, NO_ANNOTATIONS, NO_BIND_RESTRICTIONS);
    }
