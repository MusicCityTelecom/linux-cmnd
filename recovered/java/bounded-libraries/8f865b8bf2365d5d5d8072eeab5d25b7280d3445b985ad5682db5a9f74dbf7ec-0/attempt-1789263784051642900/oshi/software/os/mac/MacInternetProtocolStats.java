    ...
    private static InternetProtocolStats.IPConnection queryIPConnection(int pid, int fd) {
        try (SystemB.SocketFdInfo si = new SystemB.SocketFdInfo();){
            ...
            if (si.size() == ret && si.psi.soi_family == 2 || si.psi.soi_family == 30) {
                ...
                InternetProtocolStats.IPConnection iPConnection = new InternetProtocolStats.IPConnection(type, laddr, lport, faddr, fport, state, si.psi.soi_qlen, si.psi.soi_incqlen, pid);
                return iPConnection;
            }
        }
        return null;
