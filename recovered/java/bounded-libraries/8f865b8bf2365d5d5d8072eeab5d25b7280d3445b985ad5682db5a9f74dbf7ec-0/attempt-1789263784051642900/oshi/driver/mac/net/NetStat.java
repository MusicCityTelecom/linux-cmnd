...
public final class NetStat {
    ...
    public static Map<Integer, IFdata> queryIFdata(int index) {
        ...
        try (ByRef.CloseableSizeTByReference len = new ByRef.CloseableSizeTByReference();){
            if (0 != SystemB.INSTANCE.sysctl(mib, 6, null, (LibCAPI.size_t.ByReference)len, null, LibCAPI.size_t.ZERO)) {
            ...
            try (Memory buf = new Memory(len.longValue());){
                block19: {
                    if (0 != SystemB.INSTANCE.sysctl(mib, 6, (Pointer)buf, (LibCAPI.size_t.ByReference)len, null, LibCAPI.size_t.ZERO)) {
