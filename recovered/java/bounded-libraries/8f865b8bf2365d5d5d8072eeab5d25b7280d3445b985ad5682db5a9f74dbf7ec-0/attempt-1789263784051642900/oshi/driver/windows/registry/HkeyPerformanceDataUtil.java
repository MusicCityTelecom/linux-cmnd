...
public final class HkeyPerformanceDataUtil {
    ...
    private static synchronized Memory readPerfDataBuffer(String objectName) {
        ...
        try (ByRef.CloseableIntByReference lpcbData = new ByRef.CloseableIntByReference(maxPerfBufferSize);){
            ...
            int ret = Advapi32.INSTANCE.RegQueryValueEx(WinReg.HKEY_PERFORMANCE_DATA, objectIndexStr, 0, null, (Pointer)pPerfData, (IntByReference)lpcbData);
            if (ret != 0 && ret != 234) {
            ...
            while (ret == 234) {
                ...
                ret = Advapi32.INSTANCE.RegQueryValueEx(WinReg.HKEY_PERFORMANCE_DATA, objectIndexStr, 0, null, (Pointer)pPerfData, (IntByReference)lpcbData);
            }
