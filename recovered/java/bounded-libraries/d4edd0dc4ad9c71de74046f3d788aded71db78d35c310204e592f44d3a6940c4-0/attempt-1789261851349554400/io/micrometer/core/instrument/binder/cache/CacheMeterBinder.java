...
implements MeterBinder {
    ...
    public final void bindTo(MeterRegistry registry) {
        ...
        if (this.missCount() != null) {
            ...
            }).tags(this.tags).tag("result", "miss").description("The number of times cache lookup methods have returned a cached (hit) or uncached (newly loaded or null) value (miss).").register(registry);
        }
        FunctionCounter.builder("cache.gets", cache, c -> this.hitCount()).tags(this.tags).tag("result", "hit").description("The number of times cache lookup methods have returned a cached (hit) or uncached (newly loaded or null) value (miss).").register(registry);
