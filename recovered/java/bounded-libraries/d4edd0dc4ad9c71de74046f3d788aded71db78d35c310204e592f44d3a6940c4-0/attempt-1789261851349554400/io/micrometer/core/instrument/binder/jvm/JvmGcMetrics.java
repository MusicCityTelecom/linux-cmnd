    ...
    implements NotificationListener {
        ...
        public void handleNotification(Notification notification, Object ref) {
            ...
            if (JvmMemory.isConcurrentPhase(gcCause, notificationInfo.getGcName())) {
                ((Timer.Builder)Timer.builder("jvm.gc.concurrent.phase.time").tags(JvmGcMetrics.this.tags)).tags("action", gcAction, "cause", gcCause).description("Time spent in concurrent phase").register(this.registry).record(duration, TimeUnit.MILLISECONDS);
            } else {
                ((Timer.Builder)Timer.builder("jvm.gc.pause").tags(JvmGcMetrics.this.tags)).tags("action", gcAction, "cause", gcCause).description("Time spent in GC pause").register(this.registry).record(duration, TimeUnit.MILLISECONDS);
            }
            ...
            if (longLivedAfter < longLivedBefore || this.shouldUpdateDataSizeMetrics(notificationInfo.getGcName())) {
