...
extends AbstractPersistentProperty<P> {
    ...
    private static final Class<? extends Annotation> IDENTITY_TYPE = AnnotationBasedPersistentProperty.loadIdentityType();
    @Nullable
    ...
    private final Lazy<Boolean> isId = Lazy.of(() -> this.isAnnotationPresent(Id.class) || IDENTITY_TYPE != null && this.isAnnotationPresent(IDENTITY_TYPE));
    ...
    private final Lazy<TypeInformation<?>> associationTargetType = Lazy.of(() -> {
        ...
        return Optional.of(Reference.class).map(this::findAnnotation).map(Reference::to).map(it -> !Class.class.equals(it) ? ClassTypeInformation.from(it) : this.getActualTypeInformation()).orElseGet(() -> super.getAssociationTargetTypeInformation());
    });
