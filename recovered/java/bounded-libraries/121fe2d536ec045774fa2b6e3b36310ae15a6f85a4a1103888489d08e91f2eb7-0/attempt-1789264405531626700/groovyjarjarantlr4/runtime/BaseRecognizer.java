...
import groovyjarjarantlr4.runtime.MismatchedNotSetException;
...
public abstract class BaseRecognizer {
    ...
    public String getErrorMessage(RecognitionException e, String[] tokenNames) {
        ...
        if (e instanceof UnwantedTokenException) {
            ...
            String tokenName = mtne.expecting == -1 ? "EOF" : tokenNames[mtne.expecting];
    ...
    protected BitSet combineFollows(boolean exact) {
        ...
        for (int i = top; i >= 0; --i) {
            ...
            if (!exact) continue;
            if (!localFollowSet.member(1)) break;
            if (i <= 0) continue;
            followSet.remove(1);
        }
        return followSet;
    ...
    public static List<String> getRuleInvocationStack(Throwable e, String recognizerClassName) {
        ...
        for (int i = stack.length - 1; i >= 0; --i) {
            ...
            if (t.getClassName().startsWith("groovyjarjarantlr4.runtime.") || t.getMethodName().equals(NEXT_TOKEN_RULE_NAME) || !t.getClassName().equals(recognizerClassName)) continue;
    ...
    public int getRuleMemoization(int ruleIndex, int ruleStartIndex) {
        ...
        if (this.state.ruleMemo[ruleIndex] == null) {
            this.state.ruleMemo[ruleIndex] = new HashMap<Integer, Integer>();
        }
        if ((stopIndexI = this.state.ruleMemo[ruleIndex].get(ruleStartIndex)) == null) {
            return -1;
        }
        return stopIndexI;
    }
    ...
    public void memoize(IntStream input, int ruleIndex, int ruleStartIndex) {
        int stopTokenIndex;
        int n = stopTokenIndex = this.state.failed ? -2 : input.index() - 1;
