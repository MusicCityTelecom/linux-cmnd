...
extends BuilderASTTransformation.AbstractBuilderStrategy {
    ...
    private void createBuilderForAnnotatedClass(BuilderASTTransformation transform, ClassNode buildee, AnnotationNode anno, boolean useSetters, boolean allNames, boolean force) {
        ...
        InitializerStrategy.createBuildeeConstructors(transform, buildee, builder, filteredFields, needsConstructor, useSetters);
    }
    ...
    private void createBuilderForAnnotatedMethod(BuilderASTTransformation transform, MethodNode mNode, AnnotationNode anno, boolean useSetters) {
        ...
        if (mNode instanceof ConstructorNode) {
            InitializerStrategy.createBuildeeConstructors(transform, buildee, builder, convertedFields, false, useSetters);
        } else {
    ...
    private void buildCommon(ClassNode buildee, AnnotationNode anno, List<FieldNode> fieldNodes, ClassNode builder) {
        ...
        for (int i = 0; i < fieldNodes.size(); ++i) {
            ClassNodeUtils.addGeneratedMethod(builder, this.createBuilderMethodForField(builder, fieldNodes, prefix, i));
        }
    ...
    private static void createBuilderConstructors(ClassNode builder, ClassNode buildee, List<FieldNode> fields) {
        ...
        BlockStatement body = GeneralUtils.block(GeneralUtils.ctorSuperS());
        InitializerStrategy.initializeFields(fields, body, false);
        ClassNodeUtils.addGeneratedConstructor(builder, 2, InitializerStrategy.getParams(fields, buildee), BuilderASTTransformation.NO_EXCEPTIONS, body);
    }
    ...
    private static void createBuildeeConstructors(BuilderASTTransformation transform, ClassNode buildee, ClassNode builder, List<FieldNode> fields, boolean needsConstructor, boolean useSetters) {
        ...
        if (needsConstructor) {
            BlockStatement body = GeneralUtils.block(GeneralUtils.ctorSuperS());
            InitializerStrategy.initializeFields(fields, body, useSetters);
            ClassNodeUtils.addGeneratedConstructor(buildee, 2, InitializerStrategy.getParams(fields, buildee), BuilderASTTransformation.NO_EXCEPTIONS, body);
        }
    }
    ...
    private static ConstructorNode createInitializerConstructor(ClassNode buildee, ClassNode builder, List<FieldNode> fields) {
        ...
        return ClassNodeUtils.addGeneratedConstructor(buildee, 1, GeneralUtils.params(GeneralUtils.param(paramType, "initializer")), BuilderASTTransformation.NO_EXCEPTIONS, GeneralUtils.block(GeneralUtils.ctorThisS(GeneralUtils.args(argsList))));
    }
    ...
    private static void initializeFields(List<FieldNode> fields, BlockStatement body, boolean useSetters) {
        for (FieldNode field : fields) {
            ...
            body.addStatement(GeneralUtils.stmt(useSetters && !field.isFinal() ? GeneralUtils.callThisX(GeneralUtils.getSetterName(name), GeneralUtils.varX(GeneralUtils.param(field.getType(), name))) : GeneralUtils.assignX(GeneralUtils.propX((Expression)GeneralUtils.varX("this"), field.getName()), GeneralUtils.varX(GeneralUtils.param(field.getType(), name)))));
        }
    }
