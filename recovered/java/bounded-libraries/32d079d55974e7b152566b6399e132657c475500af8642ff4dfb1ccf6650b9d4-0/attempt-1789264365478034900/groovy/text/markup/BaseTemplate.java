...
implements Writable {
    ...
    public BaseTemplate yield(Object obj) throws IOException {
        this.writeIndent();
        this.out.write(XmlUtil.escapeXml((String)obj.toString()));
        return this;
    }

    public String stringOf(Closure cl) throws IOException {
        Writer old = this.out;
        StringBuilderWriter stringWriter = new StringBuilderWriter(32);
    ...
    private String escapeQuotes(String str) {
        ...
        String escape = this.configuration.isUseDoubleQuotes() ? "&quot;" : "&apos;";
    ...
    public void includeEscaped(String templatePath) throws IOException {
        URL resource = this.engine.resolveTemplate(templatePath);
        this.yield(ResourceGroovyMethods.getText((URL)resource, (String)this.engine.getCompilerConfiguration().getSourceEncoding()));
    }

    public void includeUnescaped(String templatePath) throws IOException {
        URL resource = this.engine.resolveTemplate(templatePath);
        this.yieldUnescaped(ResourceGroovyMethods.getText((URL)resource, (String)this.engine.getCompilerConfiguration().getSourceEncoding()));
    }

    public Object tryEscape(Object contents) {
        if (contents instanceof CharSequence) {
            return XmlUtil.escapeXml((String)contents.toString());
        }
        return contents;
    }
    ...
    public String toString() {
        StringBuilderWriter wrt = new StringBuilderWriter(512);
        try {
            this.writeTo((Writer)wrt);
        }
        catch (IOException e) {
            UncheckedThrow.rethrow((Throwable)e);
        }
        return wrt.toString();
    }
