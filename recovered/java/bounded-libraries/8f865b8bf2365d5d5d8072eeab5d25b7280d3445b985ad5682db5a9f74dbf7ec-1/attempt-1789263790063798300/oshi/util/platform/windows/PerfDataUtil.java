...
public final class PerfDataUtil {
    ...
    public static long updateQueryTimestamp(WinNT.HANDLEByReference query) {
        try (ByRef.CloseableLONGLONGByReference pllTimeStamp = new ByRef.CloseableLONGLONGByReference();){
            int ret = IS_VISTA_OR_GREATER ? PDH.PdhCollectQueryDataWithTime(query.getValue(), (WinDef.LONGLONGByReference)pllTimeStamp) : PDH.PdhCollectQueryData(query.getValue());
            int retries = 0;
            while (ret == -2147481643 && retries++ < 3) {
                Util.sleep(1 << retries);
                ret = IS_VISTA_OR_GREATER ? PDH.PdhCollectQueryDataWithTime(query.getValue(), (WinDef.LONGLONGByReference)pllTimeStamp) : PDH.PdhCollectQueryData(query.getValue());
            }
            if (ret != 0) {
    ...
    public static long queryCounter(WinNT.HANDLEByReference counter) {
        try (Struct.CloseablePdhRawCounter counterValue = new Struct.CloseablePdhRawCounter();){
            int ret = PDH.PdhGetRawCounterValue(counter.getValue(), PDH_FMT_RAW, (Pdh.PDH_RAW_COUNTER)counterValue);
            if (ret != 0) {
                if (LOG.isWarnEnabled()) {
