...
extends FieldGenOrMethodGen {
    ...
    public MethodGen(Method m, String class_name, ConstantPool cp, boolean useTags) {
        ...
        for (Attribute attribute : attributes = m.getAttributes()) {
                ...
                for (Attribute codeAttr : codeAttrs = code.getAttributes()) {
                    ...
                    if (a instanceof LocalVariableTable) {
                        ...
                        if (useTags) {
                            ...
                            for (Node node : lv) {
                                Type t = Type.getType(((LocalVariable)node).getSignature());
                                LocalVariableTag lvt = new LocalVariableTag(t, ((LocalVariable)node).getSignature(), ((LocalVariable)node).getName(), ((LocalVariable)node).getIndex(), ((LocalVariable)node).getStartPC());
    ...
    public final String toString() {
        ...
        if (this.exceptionsThrown.size() > 0) {
            for (String s : this.exceptionsThrown) {
                buf.append("\n\t\tthrows " + s);
            }
        }
        return buf.toString();
