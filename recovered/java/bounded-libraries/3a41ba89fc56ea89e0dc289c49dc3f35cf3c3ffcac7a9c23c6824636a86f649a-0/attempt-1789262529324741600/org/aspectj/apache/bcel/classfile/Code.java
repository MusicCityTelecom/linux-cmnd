...
extends Attribute {
    ...
    public final String toString(boolean verbose) {
        StringBuilder buf = new StringBuilder("Code(max_stack = " + this.maxStack + ", max_locals = " + this.maxLocals + ", code_length = " + this.code.length + ")\n" + Utility.codeToString(this.code, this.cpool, 0, -1, verbose));
        if (this.exceptionTable.length > 0) {
            buf.append("\nException handler(s) = \nFrom\tTo\tHandler\tType\n");
            for (Cloneable cloneable : this.exceptionTable) {
                buf.append(((CodeException)cloneable).toString(this.cpool, verbose) + "\n");
            }
        }
        if (this.attributes.length > 0) {
            buf.append("\nAttribute(s) = \n");
            for (Cloneable cloneable : this.attributes) {
                buf.append(((Attribute)cloneable).toString() + "\n");
            }
        }
        return buf.toString();
    }
    ...
    public String getCodeString() {
        ...
        if (this.exceptionTable.length > 0) {
            ...
            for (CodeException exc : this.exceptionTable) {
