...
implements CMSSignatureAlgorithmNameGenerator {
    ...
    public String getSignatureName(AlgorithmIdentifier algorithmIdentifier, AlgorithmIdentifier algorithmIdentifier2) {
        ...
        if (EdECObjectIdentifiers.id_Ed25519.equals((ASN1Primitive)aSN1ObjectIdentifier)) {
            return "Ed25519";
        }
        if (EdECObjectIdentifiers.id_Ed448.equals((ASN1Primitive)aSN1ObjectIdentifier)) {
            return "Ed448";
        }
        if (PKCSObjectIdentifiers.id_alg_hss_lms_hashsig.equals((ASN1Primitive)aSN1ObjectIdentifier)) {
            return "LMS";
        }
        String string = this.getDigestAlgName(aSN1ObjectIdentifier);
        if (!string.equals(aSN1ObjectIdentifier.getId())) {
            return string + "with" + this.getEncryptionAlgName(aSN1ObjectIdentifier);
        }
