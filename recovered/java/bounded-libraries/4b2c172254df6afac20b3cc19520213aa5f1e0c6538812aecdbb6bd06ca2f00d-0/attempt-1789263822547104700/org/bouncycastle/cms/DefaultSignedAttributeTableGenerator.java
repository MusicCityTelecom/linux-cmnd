...
implements CMSAttributeTableGenerator {
    ...
    protected Hashtable createStandardAttributeTable(Map map) {
        ...
        if (!hashtable.containsKey(CMSAttributes.signingTime)) {
            object = new Date();
            attribute = new Attribute(CMSAttributes.signingTime, (ASN1Set)new DERSet((ASN1Encodable)new Time((Date)object)));
            hashtable.put(attribute.getAttrType(), attribute);
        }
        ...
        if (!hashtable.contains(CMSAttributes.cmsAlgorithmProtect)) {
            object = new Attribute(CMSAttributes.cmsAlgorithmProtect, (ASN1Set)new DERSet((ASN1Encodable)new CMSAlgorithmProtection((AlgorithmIdentifier)map.get("digestAlgID"), 1, (AlgorithmIdentifier)map.get("signatureAlgID"))));
