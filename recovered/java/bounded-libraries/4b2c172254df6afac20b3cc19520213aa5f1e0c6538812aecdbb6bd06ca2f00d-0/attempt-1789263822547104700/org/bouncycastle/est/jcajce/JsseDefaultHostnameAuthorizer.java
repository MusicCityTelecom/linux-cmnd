...
implements JsseHostnameAuthorizer {
    ...
    public boolean verify(String string, X509Certificate x509Certificate) throws IOException {
        ...
        try {
            ...
            if (rDNArray != null) {
                block6: for (List<?> list : rDNArray) {
                    ...
                    String string2 = list.get(1) instanceof byte[] ? Hex.toHexString((byte[])((byte[])list.get(1))) : list.get(1).toString();
        ...
        rDNArray = X500Name.getInstance((Object)x509Certificate.getSubjectX500Principal().getEncoded()).getRDNs();
        for (int i = rDNArray.length - 1; i >= 0; --i) {
            ...
            for (int j = 0; j != attributeTypeAndValueArray.length; ++j) {
                ...
                if (!attributeTypeAndValue.getType().equals((ASN1Primitive)BCStyle.CN)) continue;
    ...
    public static boolean isValidNameMatch(String string, String string2, Set<String> set) throws IOException {
        if (string2.contains("*")) {
            ...
            if (n == string2.lastIndexOf("*")) {
                ...
                if (set != null && set.contains(Strings.toLowerCase((String)string2.substring(n2)))) {
                    throw new IOException("Wildcard `" + string2 + "` matches known public suffix.");
                }
                String string3 = Strings.toLowerCase((String)string2.substring(n + 1));
                String string4 = Strings.toLowerCase((String)string);
