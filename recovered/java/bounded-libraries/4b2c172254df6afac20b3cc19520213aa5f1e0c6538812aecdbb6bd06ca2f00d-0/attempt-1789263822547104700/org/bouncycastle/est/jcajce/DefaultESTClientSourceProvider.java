...
implements ESTClientSourceProvider {
    ...
    public Source makeSource(String string, int n) throws IOException {
        ...
        object = Strings.toLowerCase((String)sSLSocket.getSession().getCipherSuite());
        ...
        if (Strings.toLowerCase((String)sSLSocket.getSession().getCipherSuite()).contains("null")) {
            throw new IOException("EST clients must not use NULL ciphers");
        }
        if (Strings.toLowerCase((String)sSLSocket.getSession().getCipherSuite()).contains("anon")) {
            throw new IOException("EST clients must not use anon ciphers");
        }
        if (Strings.toLowerCase((String)sSLSocket.getSession().getCipherSuite()).contains("export")) {
