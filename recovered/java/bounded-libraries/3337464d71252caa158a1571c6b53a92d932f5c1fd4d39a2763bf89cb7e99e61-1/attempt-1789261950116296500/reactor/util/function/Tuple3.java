...
extends Tuple2<T1, T2> {
    ...
    public <R> Tuple3<R, T2, T3> mapT1(Function<T1, R> mapper) {
        return new Tuple3<R, Object, T3>(mapper.apply(this.t1), this.t2, this.t3);
    }

    public <R> Tuple3<T1, R, T3> mapT2(Function<T2, R> mapper) {
        return new Tuple3<Object, R, T3>(this.t1, mapper.apply(this.t2), this.t3);
    }

    public <R> Tuple3<T1, T2, R> mapT3(Function<T3, R> mapper) {
        return new Tuple3<Object, Object, R>(this.t1, this.t2, mapper.apply(this.t3));
    }
