...
abstract class TransactionSynchronizationUtils {
    ...
    public static Mono<Void> triggerBeforeCompletion(Collection<TransactionSynchronization> synchronizations) {
        return Flux.fromIterable(synchronizations).concatMap(TransactionSynchronization::beforeCompletion).onErrorContinue((t, o) -> logger.debug((Object)"TransactionSynchronization.beforeCompletion threw exception", t)).then();
    }
    ...
    public static Mono<Void> invokeAfterCompletion(Collection<TransactionSynchronization> synchronizations, int completionStatus) {
        return Flux.fromIterable(synchronizations).concatMap(it -> it.afterCompletion(completionStatus)).onErrorContinue((t, o) -> logger.debug((Object)"TransactionSynchronization.afterCompletion threw exception", t)).then();
    }
