...
implements Iterable<Mapping> {
    ...
    public MimeMappings(Map<String, String> mappings) {
        Assert.notNull(mappings, (String)"Mappings must not be null");
        this.map = new LinkedHashMap<String, Mapping>();
        mappings.forEach(this::add);
    }

    private MimeMappings(MimeMappings mappings, boolean mutable) {
        Assert.notNull((Object)mappings, (String)"Mappings must not be null");
    ...
    static {
        ...
        DEFAULT = MimeMappings.unmodifiableMappings(mappings);
    }
