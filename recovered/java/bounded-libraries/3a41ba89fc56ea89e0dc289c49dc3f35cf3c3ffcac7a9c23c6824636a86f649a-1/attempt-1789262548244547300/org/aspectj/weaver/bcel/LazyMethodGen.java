...
implements Traceable {
        ...
        if (this.memberView == null) {
            if (this.newParameterAnnotations == null) {
                ...
                for (int i = 0; i < pcount; ++i) {
                    ...
                    this.newParameterAnnotations[i] = AnnotationAJ.EMPTY_ARRAY;
                }
            } else {
    ...
    public AnnotationAJ[] getAnnotations() {
        this.initialize();
        if (this.memberView == null && this.newAnnotations != null && this.newAnnotations.size() != 0) {
            return this.newAnnotations.toArray(AnnotationAJ.EMPTY_ARRAY);
        }
        return null;
    }
    ...
    private void printAspectAttributes(PrintStream out, AjAttribute.WeaverVersionInfo weaverVersion) {
        ...
        if (!(as = Utility.readAjAttributes(this.getClassName(), this.attributes.toArray(Attribute.NoAttributes), context, null, weaverVersion, new BcelConstantPoolReader(this.enclosingClass.getConstantPool()))).isEmpty()) {
    ...
    public MethodGen pack() {
        ...
        for (String declaredException : this.declaredExceptions) {
        ...
        if (this.newParameterAnnotations != null) {
            for (int i = 0; i < this.newParameterAnnotations.length; ++i) {
                AnnotationAJ[] annos;
                for (AnnotationAJ anno : annos = this.newParameterAnnotations[i]) {
        ...
        if (this.memberView != null && this.memberView.getAnnotations() != null && this.memberView.getAnnotations().length != 0) {
            AnnotationAJ[] ans;
            for (AnnotationAJ an : ans = this.memberView.getAnnotations()) {
    ...
    private void addLocalVariables(MethodGen gen, Map<LocalVariableTag, LVPosition> localVariables) {
        ...
        for (LocalVariableTag tag : localVariables.keySet()) {
            ...
            } else if (slots.contains(tag.getSlot())) continue;
    ...
    private static void assertTargetedBy(InstructionHandle target, InstructionTargeter targeter, String from) {
        for (InstructionTargeter instructionTargeter : target.getTargeters()) {
            if (instructionTargeter != targeter) continue;
            return;
        }
