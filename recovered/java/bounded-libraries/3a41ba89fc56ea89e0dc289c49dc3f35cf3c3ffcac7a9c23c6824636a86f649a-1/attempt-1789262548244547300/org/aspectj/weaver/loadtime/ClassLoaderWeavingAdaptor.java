...
extends WeavingAdaptor {
    ...
    private String looksLikeExactName(String typePattern) {
        if (this.hasSpaceAnnotationPlus(typePattern, 0) || typePattern.contains("*")) {
            return null;
        }
    ...
    protected boolean accept(String className, byte[] bytes) {
        ...
        if (!this.excludeSpecial.isEmpty()) {
            for (String[] stringArray : this.excludeSpecial) {
                ...
                String exceptThese = stringArray[1];
        ...
        if (this.excludeTypePattern.isEmpty()) {
            ...
            boolean bl2 = false;
            for (String s : this.includeStartsWith) {
                didSomeIncludeMatching = true;
                boolean bl3 = fastClassName.startsWith(s);
                if (!bl3) continue;
                return true;
            }
        ...
        try {
            ...
            boolean bl8 = !didSomeIncludeMatching;
    ...
    private boolean acceptAspect(String aspectClassName) {
        ...
        for (String string : this.aspectExcludeStartsWith) {
            if (!fastClassName.startsWith(string)) continue;
            return false;
        }
        for (String string : this.aspectIncludeStartsWith) {
            if (!fastClassName.startsWith(string)) continue;
            return true;
        }
        ...
        while (iterator.hasNext() && !(bl = (typePattern = iterator.next()).matchesStatically(classInfo))) {
        }
        return bl;
    }
    ...
    private void defineClass(ClassLoader loader, String name, byte[] bytes, ProtectionDomain protectionDomain) {
        ...
        if (LangUtil.is11VMOrGreater()) {
            ...
            try {
                ...
                clazz = defineClassMethod.invoke(this.getUnsafe(), name, bytes, 0, bytes.length, loader, protectionDomain);
            }
