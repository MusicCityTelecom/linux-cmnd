...
extends ResolvedType {
    ...
    public boolean hasAnnotation(UnresolvedType ofType) {
        ...
        if (this.annotationTypes != null) {
            for (ResolvedType annotationType : this.annotationTypes) {
                if (!annotationType.equals(ofType)) continue;
                return true;
            }
        }
        return false;
    ...
    public AnnotationAJ getAnnotationOfType(UnresolvedType ofType) {
        ...
        if (axs != null) {
            for (AnnotationAJ ax : axs) {
                if (!ax.getTypeSignature().equals(ofType.getSignature())) continue;
                return ax;
            }
        }
        if (this.annotations != null) {
            ...
            for (AnnotationAJ annotation : this.annotations) {
                if (!annotation.getTypeSignature().equals(searchSig)) continue;
                return annotation;
            }
        }
        return null;
    ...
    public boolean isCoerceableFrom(ResolvedType o) {
        ...
        for (ResolvedMember member : a) {
            for (ResolvedMember resolvedMember : b) {
                if (resolvedMember.isCompatibleWith(member)) continue;
                return false;
            }
        }
        return true;
    ...
    public boolean isAssignableFrom(ResolvedType other, boolean allowMissing) {
        ...
        if (this.isParameterizedType() && ((ReferenceType)this.getRawType()).isAssignableFrom(other)) {
            ...
            ResolvedType[] resolvedTypeArray = myParameters = this.getResolvedTypeParameters();
            int n = resolvedTypeArray.length;
            for (int i = 0; i < n; ++i) {
                ResolvedType myParameter = resolvedTypeArray[i];
    ...
    public TypeVariable[] getTypeVariables() {
        if (this.typeVariables == null) {
            for (TypeVariable typeVariable : this.typeVariables = this.getDelegate().getTypeVariables()) {
    ...
    private static String makeDeclaredSignature(ResolvedType aGenericType, UnresolvedType[] someParameters) {
        ...
        for (UnresolvedType someParameter : someParameters) {
            if (someParameter instanceof ReferenceType) {
                ret.append(((ReferenceType)someParameter).getSignatureForAttribute());
                continue;
            }
            if (someParameter instanceof ResolvedType.Primitive) {
                ret.append(((ResolvedType.Primitive)someParameter).getSignatureForAttribute());
                continue;
            }
            throw new IllegalStateException("DebugFor325731: expected a ReferenceType or Primitive but was " + someParameter + " of type " + someParameter.getClass().getName());
        }
        ret.append(">;");
        return ret.toString();
    }
    ...
    public void addParent(ResolvedType newParent) {
        ...
        if (newParent.isClass()) {
            ...
            if (this.newInterfaces == null) {
                ...
                if (existing != null) {
                    for (ResolvedType resolvedType : existing) {
                        if (!resolvedType.equals(newParent)) continue;
                        return;
                    }
                }
            ...
            if (this.isGenericType()) {
                List<WeakReference<ReferenceType>> list = this.derivativeTypes;
                synchronized (list) {
                    for (WeakReference weakReference : this.derivativeTypes) {
                        ReferenceType derivativeType = (ReferenceType)weakReference.get();
