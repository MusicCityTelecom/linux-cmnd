...
implements IReflectionWorld {
    ...
    private List<ResolvedType> typesForCompletion = new ArrayList<ResolvedType>();
    ...
    private static Map<?, ?> makeConcurrentMap() {
        if (concurrentMapClass != null) {
            try {
                return (Map)concurrentMapClass.getDeclaredConstructor(new Class[0]).newInstance(new Object[0]);
            }
    ...
    private static Class<?> makeConcurrentMapClass() {
        ...
        for (String betterChoice : betterChoices = new String[]{"java.util.concurrent.ConcurrentHashMap", "edu.emory.mathcs.backport.java.util.concurrent.ConcurrentHashMap", "EDU.oswego.cs.dl.util.concurrent.ConcurrentHashMap"}) {
            try {
        ...
        if (this.isLocallyDefined(ret.getName())) {
            if (this.typeCompletionInProgress) {
                ...
                while (this.typesForCompletion.size() != 0) {
                    ResolvedType rt = this.typesForCompletion.get(0);
