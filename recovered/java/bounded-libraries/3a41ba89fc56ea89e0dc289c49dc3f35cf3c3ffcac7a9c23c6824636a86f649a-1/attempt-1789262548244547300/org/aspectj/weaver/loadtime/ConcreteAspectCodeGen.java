...
public class ConcreteAspectCodeGen {
    ...
    public boolean validate() {
        ...
        if (parentAspectName.contains("<")) {
            ...
            if (this.parent.isParameterizedType()) {
                ...
                for (UnresolvedType typeParameter : typeParameters = this.parent.getTypeParameters()) {
    ...
    private boolean hasPointcutAnnotation(ResolvedMember member) {
        ...
        for (AnnotationAJ a : as) {
            if (!a.getTypeSignature().equals("Lorg/aspectj/lang/annotation/Pointcut;")) continue;
            return true;
        }
        return false;
    }
    ...
    public byte[] getBytes() {
        ...
        LazyMethodGen init = new LazyMethodGen(1, Type.VOID, "<init>", Type.NO_ARGS, EMPTY_STRINGS, cg);
        InstructionList cbody = init.getBody();
        cbody.append(InstructionConstants.ALOAD_0);
        cbody.append(cg.getFactory().createInvoke(parentName, "<init>", Type.VOID, Type.NO_ARGS, (short)183));
        cbody.append(InstructionConstants.RETURN);
        cg.addMethodGen(init);
        for (Definition.Pointcut pointcut : this.concreteAspect.pointcuts) {
            LazyMethodGen mg = new LazyMethodGen(1, Type.VOID, pointcut.name, Type.NO_ARGS, EMPTY_STRINGS, cg);
        ...
        for (String value : values) {
            ...
            for (ResolvedMember rm : rms) {
                ...
                block41: {
                    ...
                    block42: {
                        nvp = null;
                        if (!rm.getName().equals(key)) break block41;
                        keyIsOk = true;
                        rt = rm.getReturnType();
    ...
    private void generateAdviceMethod(Definition.PointcutAndAdvice paa, int adviceCounter, LazyClassGen cg) {
        ...
        LazyMethodGen advice = new LazyMethodGen(1, returnType, adviceName, paramTypes.toArray(Type.NO_ARGS), EMPTY_STRINGS, cg);
