...
ArgNameFinder {
    ...
    public void setClassLoader(ClassLoader aLoader) {
        ...
        if (useCachingClassLoaderRepository) {
            if (useSingleInstances && staticBcelRepository == null) {
                staticBcelRepository = new ClassLoaderRepository(this.getClassLoader());
            } else {
                this.bcelRepository = new ClassLoaderRepository(this.classLoaderRef);
            }
        } else if (useSingleInstances && staticBcelRepository == null) {
            staticBcelRepository = new NonCachingClassLoaderRepository(this.getClassLoader());
        } else {
    ...
    private Repository getBcelRepository() {
        return useSingleInstances ? staticBcelRepository : this.bcelRepository;
    }
    ...
    private ClassLoader getClassLoader() {
        return useSingleInstances ? staticClassLoaderRef.getClassLoader() : this.classLoaderRef.getClassLoader();
    }
    ...
    public AnnotationAJ getAnnotationOfType(UnresolvedType ofType, Member onMember) {
        ...
        try {
            ...
            AnnotationGen[] anns = AnnotationGen.NO_ANNOTATIONS;
            ...
            this.getBcelRepository().clear();
            if (anns == null) {
                anns = AnnotationGen.NO_ANNOTATIONS;
            }
            for (AnnotationGen ann : anns) {
                if (!ann.getTypeSignature().equals(ofType.getSignature())) continue;
                return new BcelAnnotation(ann, this.world);
            }
            return null;
        }
        catch (ClassNotFoundException classNotFoundException) {
    ...
    public String getAnnotationDefaultValue(Member onMember) {
        try {
            Method bcelMethod;
            JavaClass jc = this.getBcelRepository().loadClass(onMember.getDeclaringClass());
            if (onMember instanceof java.lang.reflect.Method && (bcelMethod = jc.getMethod((java.lang.reflect.Method)onMember)) != null) {
                Attribute[] attrs;
                for (Attribute attribute : attrs = bcelMethod.getAttributes()) {
    ...
    public ResolvedType[] getAnnotations(Class forClass, World inWorld) {
        try {
            JavaClass jc = this.getBcelRepository().loadClass(forClass);
    ...
    public String[] getParameterNames(Member forMember) {
        ...
        try {
            JavaClass jc = this.getBcelRepository().loadClass(forMember.getDeclaringClass());
    ...
    static {
        useSingleInstances = System.getProperty("org.aspectj.apache.bcel.useSingleRepositoryInstance", "false").equalsIgnoreCase("true");
        try {
