...
public class AtAjAttributes {
    ...
    public static List<AjAttribute> readAj5ClassAttributes(AsmManager model, JavaClass javaClass, ReferenceType type, ISourceContext context, IMessageHandler msgHandler, boolean isCodeStyleAspect) {
        ...
        for (Constant constant : cpool = javaClass.getConstantPool().getConstantPool()) {
        ...
        for (int i = 0; i < javaClass.getMethods().length; ++i) {
            ...
            for (Attribute mattribute : mattributes = method.getAttributes()) {
        ...
        for (Field field : fs = javaClass.getFields()) {
            ...
            for (Attribute fattribute : fattributes = field.getAttributes()) {
    ...
    private static boolean handleDeclareParentsAnnotation(RuntimeAnnos runtimeAnnotations, AjAttributeFieldStruct struct) {
        NameValuePair decpPatternNameValuePair;
        String decpPattern;
        AnnotationGen decpAnno = AtAjAttributes.getAnnotation(runtimeAnnotations, AjcMemberMaker.DECLAREPARENTS_ANNOTATION);
        if (decpAnno != null && (decpPattern = (decpPatternNameValuePair = AtAjAttributes.getAnnotationElement(decpAnno, VALUE)).getValue().stringifyValue()) != null) {
    ...
    private static boolean handleDeclareMixinAnnotation(RuntimeAnnos runtimeAnnotations, AjAttributeMethodStruct struct) {
        ...
        for (ResolvedType typeForDelegation : newInterfaceTypes) {
            ...
            for (ResolvedMember resolvedMember : methods = typeForDelegation.getMethodsWithoutIterator(true, false, false).toArray(ResolvedMember.NONE)) {
    ...
    public static int extractExtraArgument(String[] argumentSignatures) {
        ...
        for (String argumentSignature : argumentSignatures) {
            ...
            if (AjcMemberMaker.TYPEX_PROCEEDINGJOINPOINT.getSignature().equals(argumentSignature)) {
                extraArgument |= 2;
                continue;
            }
            if (AjcMemberMaker.TYPEX_STATICJOINPOINT.getSignature().equals(argumentSignature)) {
                extraArgument |= 4;
                continue;
            }
            if (!AjcMemberMaker.TYPEX_ENCLOSINGSTATICJOINPOINT.getSignature().equals(argumentSignature)) continue;
            extraArgument |= 8;
        }
    ...
    private static String[] getMethodArgumentNames(Method method, String argNamesFromAnnotation, AjAttributeMethodStruct methodStruct) {
        ...
        if (lt != null) {
            LocalVariable[] lvt;
            for (LocalVariable localVariable : lvt = lt.getLocalVariableTable()) {
    ...
    private static String[] extractArgNamesFromAnnotationValue(Method method, String argNamesFromAnnotation, AjAttributeMethodStruct methodStruct) {
        ...
        if (args.size() != method.getArgumentTypes().length) {
            StringBuilder shortString = new StringBuilder().append(AtAjAttributes.lastbit(method.getReturnType().toString())).append(" ").append(method.getName());
            if (method.getArgumentTypes().length > 0) {
                shortString.append("(");
    ...
    private static Pointcut parsePointcut(String pointcutString, AjAttributeStruct struct, boolean allowIf) {
        try {
            ...
            if (!allowIf && pointcutString.contains("if()") && AtAjAttributes.hasIf(pointcut)) {
