...
extends ResolvedMemberImpl {
    ...
    public void determineParameterNames() {
        ...
        if (varTable == null) {
            ...
            if (annos != null && annos.length != 0) {
                AnnotationAJ[] axs;
                for (AnnotationAJ annotationX : axs = this.getAnnotations()) {
                    AnnotationGen a;
    ...
    public String getAnnotationDefaultValue() {
        Attribute[] attrs;
        for (Attribute attribute : attrs = this.method.getAttributes()) {
    ...
    public String[] getAttributeNames(boolean onlyIncludeAjOnes) {
        ...
        for (Attribute a : as) {
            if (onlyIncludeAjOnes && !a.getName().startsWith("org.aspectj.weaver")) continue;
            names.add(a.getName());
        }
        ...
        if (jc.getMajor() < 49) {
            ...
            if (synthetics != null) {
                for (String synthetic : synthetics) {
                    if (!synthetic.equals("Synthetic")) continue;
                    this.bitflags |= 0x80;
                    break;
                }
            }
