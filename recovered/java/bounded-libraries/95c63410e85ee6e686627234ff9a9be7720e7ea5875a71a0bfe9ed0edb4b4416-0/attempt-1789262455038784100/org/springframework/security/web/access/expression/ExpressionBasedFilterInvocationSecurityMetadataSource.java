...
extends DefaultFilterInvocationSecurityMetadataSource {
    ...
    private static LinkedHashMap<RequestMatcher, Collection<ConfigAttribute>> processMap(LinkedHashMap<RequestMatcher, Collection<ConfigAttribute>> requestMap, ExpressionParser parser) {
        ...
        requestMap.forEach((request, value) -> ExpressionBasedFilterInvocationSecurityMetadataSource.process(parser, request, value, processed::put));
        return processed;
    }

    private static void process(ExpressionParser parser, RequestMatcher request, Collection<ConfigAttribute> value, BiConsumer<RequestMatcher, Collection<ConfigAttribute>> consumer) {
        String expression = ExpressionBasedFilterInvocationSecurityMetadataSource.getExpression(request, value);
        if (logger.isDebugEnabled()) {
            logger.debug((Object)LogMessage.format((String)"Adding web access control expression [%s] for %s", (Object)expression, (Object)request));
        }
        AbstractVariableEvaluationContextPostProcessor postProcessor = ExpressionBasedFilterInvocationSecurityMetadataSource.createPostProcessor(request);
    ...
    private static String getExpression(RequestMatcher request, Collection<ConfigAttribute> value) {
        Assert.isTrue((value.size() == 1 ? 1 : 0) != 0, () -> "Expected a single expression attribute for " + request);
