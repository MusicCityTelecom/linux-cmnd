...
- Room ID: stream 65 commands `IPCloneService` to fetch `RoomSpecificSettings.zip`; stream 69 retrieves it. The ZIP contains `RoomSpecificSettings.xml` plus `TVSettings.xml`; customer values are omitted. Stream 68 reports InProgress then Successful, but no later discovery readback proves application.
...
Observed transport had no HTTP Authorization and reported non-secured transit, while secure-command capability varied by TV. This does not establish universal lack of authentication. Room IDs are strings; preserve leading-zero semantics (synthetic example: `00042` versus `42`).
