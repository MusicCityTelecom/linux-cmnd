...
Evaluation releases are intended for a dedicated, access-restricted VM, not an
Internet-facing or production installation. Before changing a private development
repository to public, review its entire Git history as well as the current tree.
Excluding operational notes from current source archives does not redact older
commits; published history is never rewritten automatically.
